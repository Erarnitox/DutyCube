...
static const char* const animnames[] = {
	"mapmodel",		  "dead",		   "dying",			 "idle",		  "run N",			"run NE",
	"run E",		  "run SE",		   "run S",			 "run SW",		  "run W",			"run NW",
	"jump",			  "jump N",		   "jump NE",		 "jump E",		  "jump SE",		"jump S",
	"jump SW",		  "jump W",		   "jump NW",		 "sink",		  "swim",			"crouch",
	"crouch N",		  "crouch NE",	   "crouch E",		 "crouch SE",	  "crouch S",		"crouch SW",
	"crouch W",		  "crouch NW",	   "crouch jump",	 "crouch jump N", "crouch jump NE", "crouch jump E",
	"crouch jump SE", "crouch jump S", "crouch jump SW", "crouch jump W", "crouch jump NW", "crouch sink",
	"crouch swim",	  "shoot",		   "melee",			 "pain",		  "edit",			"lag",
	"taunt",		  "win",		   "lose",			 "gun idle",	  "gun shoot",		"gun melee",
...
static struct gamemodeinfo {
	...
	{"demo", "Demo", M_DEMO | M_LOCAL, nullptr},
	...
	{"rtdm",
	 "rTDM",
	 M_TEAM | M_RAIL,
	 "Railgun Team Deathmatch:\nFrag \fs\f3the enemy team\fr with railguns to score points for \fs\f1your team\fr."},
	{"ptdm",
	 "pTDM",
	 M_TEAM | M_PULSE,
	 "Pulse Rifle Team Deathmatch:\nFrag \fs\f3the enemy team\fr with pulse rifles to score points for \fs\f1your "
	 "team\fr."},
	{"rctf",
	 "rCTF",
	 M_CTF | M_TEAM | M_RAIL,
	 "Railgun Capture The Flag:\nCapture \fs\f3the enemy flag\fr and bring it back to \fs\f1your flag\fr to score "
	 "points for \fs\f1your team\fr."},
	{"pctf",
	 "pCTF",
	 M_CTF | M_TEAM | M_PULSE,
...
#define DROP_ENGINE_SERVER_PORT 42010
#define DROP_ENGINE_LANINFO_PORT 42008
#define DROP_ENGINE_MASTER_PORT 42009
#define PROTOCOL_VERSION 2	// bump when protocol changes
#define DEMO_VERSION 1	// bump when demo format changes
#define DEMO_MAGIC "DROP_ENGINE_DEMO\0\0"
...
static const struct attackinfo {
	int gun, action, anim, vwepanim, hudanim, sound, hudsound, attackdelay, damage, spread, margin, projspeed,
		kickamount, range, rays, hitpush, exprad, ttl, use;
} attacks[NUMATKS] = {{GUN_RAIL,
					   ACT_SHOOT,
					   ANIM_SHOOT,
					   ANIM_VWEP_SHOOT,
					   ANIM_GUN_SHOOT,
...
static const struct guninfo {
	const char *name, *file, *vwep;
	int attacks[NUMACTS];
} guns[NUMGUNS] = {{
					   "railgun",
					   "railgun",
					   "worldgun/railgun",
					   {-1, ATK_RAIL_SHOOT, ATK_RAIL_MELEE},
				   },
...
struct gamestate {
	...
	int ammo[NUMGUNS];
	int aitype{AI_NONE}, skill{0};

	gamestate()  {
	}

	auto canpickup(int type) -> bool {
	...
	auto dodamage(int damage) -> int {
		health -= damage;
		return damage;
	}

	auto hasammo(int gun, int exclude = -1) -> int {
...
static inline auto teamnumber(const char* name) -> int {
	loopi(MAXTEAMS) if (!strcmp(teamnames[1 + i], name)) return 1 + i;
	return 0;
}
...
struct gameent : dynent, gamestate {
	...
	int clientnum{-1}, privilege{PRIV_NONE}, lastupdate{0}, plag{0}, ping{0};
	...
	int frags{0}, flags{0}, deaths{0}, totaldamage{0}, totalshots{0};
	editinfo* edit{NULL};
	float deltayaw, deltapitch, deltaroll, newyaw, newpitch, newroll;
	int smoothmillis{-1};

	string name, info;
	int team{0}, playermodel{-1}, playercolor{0};
	ai::aiinfo* ai{NULL};
	int ownernum{-1}, lastnode;
...
struct teamscore {
	...
	static auto compare(const teamscore& x, const teamscore& y) -> bool {
		if (x.score > y.score)
			return true;
		if (x.score < y.score)
			return false;
...
static inline auto hthash(const teamscore& t) -> uint {
	return hthash(t.team);
}
static inline auto htcmp(int team, const teamscore& t) -> bool {
	return team == t.team;
}
...
extern auto entmdlname(int type) -> const char*;
extern auto itemname(int i) -> const char*;
extern auto itemicon(int i) -> int;
...
struct clientmode {
	virtual ~clientmode() = default;

	virtual void preload() {
	}
	virtual auto clipconsole(float w, float h) -> float {
		return 0;
	}
	...
	virtual auto respawnwait(gameent* d) -> int {
		return 0;
	}
	virtual void pickspawn(gameent* d) {
	...
	virtual auto aicheck(gameent* d, ai::aistate& b) -> bool {
		return false;
	}
	virtual auto aidefend(gameent* d, ai::aistate& b) -> bool {
		return false;
	}
	virtual auto aipursue(gameent* d, ai::aistate& b) -> bool {
		return false;
	}
};
...
extern auto clientoption(const char* arg) -> bool;
extern auto getclient(int cn) -> gameent*;
extern auto newclient(int cn) -> gameent*;
extern auto colorname(gameent* d, const char* name = nullptr, const char* alt = nullptr, const char* color = "") -> const char*;
extern auto teamcolorname(gameent* d, const char* alt = "you") -> const char*;
extern auto teamcolor(const char* prefix, const char* suffix, int team, const char* alt) -> const char*;
extern auto pointatplayer() -> gameent*;
extern auto hudplayer() -> gameent*;
extern auto followingplayer() -> gameent*;
...
extern auto parseplayer(const char* arg) -> int;
extern void ignore(int cn);
extern void unignore(int cn);
extern auto isignored(int cn) -> bool;
extern auto addmsg(int type, const char* fmt = nullptr, ...) -> bool;
...
// weapon
extern auto getweapon(const char* name) -> int;
...
extern auto intersect(dynent* d, const vec& from, const vec& to, float margin = 0, float& dist = intersectdist) -> bool;
extern auto intersectclosest(const vec& from,
								const vec& to,
								gameent* at,
								float margin = 0,
								float& dist = intersectdist) -> dynent*;
...
struct playermodelinfo {
	const char *model[1 + MAXTEAMS], *hudguns[1 + MAXTEAMS], *icon[1 + MAXTEAMS];
	bool ragdoll;
};
...
extern auto getplayermodelinfo(gameent* d) -> const playermodelinfo&;
extern auto getplayercolor(gameent* d, int team) -> int;
extern auto chooserandomplayermodel(int seed) -> int;
extern void syncplayer();
extern void swayhudgun(int curtime);
extern auto hudgunorigin(int gun, const vec& from, const vec& to, gameent* d) -> vec;
}  // namespace game

namespace server {
extern auto modename(int n, const char* unknown = "unknown") -> const char*;
extern auto modeprettyname(int n, const char* unknown = "unknown") -> const char*;
extern auto mastermodename(int n, const char* unknown = "unknown") -> const char*;
...
extern auto msgsizelookup(int msg) -> int;
extern auto serveroption(const char* arg) -> bool;
extern auto delayspawn(int type) -> bool;
}  // namespace server
