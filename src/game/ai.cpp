...
auto cansee(gameent* d, vec& x, vec& y, vec& targ) -> bool {
...
auto canshoot(gameent* d, int atk, gameent* e) -> bool {
	if (attackrange(d, atk, e->o.squaredist(d->o)) && targetable(d, e))
		return d->ammo[attacks[atk].gun] > 0 && lastmillis - d->lastaction >= d->gunwait;
	return false;
}

auto canshoot(gameent* d, int atk) -> bool {
	return !d->ai->becareful && d->ammo[attacks[atk].gun] > 0 && lastmillis - d->lastaction >= d->gunwait;
}

auto hastarget(gameent* d,
			   int atk,
			   aistate& b,
			   gameent* e,
			   float yaw,
			   float pitch,
			   float dist) -> bool {  // add margins of error
	if (attackrange(d, atk, dist) || (d->skill <= 100 && !rnd(d->skill))) {
		float skew =
				  clamp(float(lastmillis - d->ai->enemymillis) / float((d->skill * attacks[atk].attackdelay / 200.f)),
						0.f,
						attacks[atk].projspeed ? 0.25f : 1e16f),
...
auto getaimpos(gameent* d, int atk, gameent* e) -> vec {
	vec o = e->o;
...
auto checkothers(vector<int>& targets,
				 gameent* d,
				 int state,
				 int targtype,
				 int target,
				 bool teams,
				 int* members) -> bool {  // checks the states of other ai for a match
	targets.setsize(0);
	loopv(players) {
		gameent* e = players[i];
...
auto makeroute(gameent* d, aistate& b, int node, bool changed, int retries) -> bool {
...
auto makeroute(gameent* d, aistate& b, const vec& pos, bool changed, int retries) -> bool {
	int node = closestwaypoint(pos, SIGHTMIN, true);
	return makeroute(d, b, node, changed, retries);
}

auto randomnode(gameent* d, aistate& b, const vec& pos, float guard, float wander) -> bool {
...
auto randomnode(gameent* d, aistate& b, float guard, float wander) -> bool {
	return randomnode(d, b, d->feetpos(), guard, wander);
}

auto badhealth(gameent* d) -> bool {
	// if(d->skill <= 100) return d->health <= (111-d->skill)/4;
	return false;
}

auto enemy(gameent* d, aistate& b, const vec& pos, float guard = SIGHTMIN, int pursue = 0) -> bool {
	gameent* t = nullptr;
	vec dp = d->headpos();
...
auto patrol(gameent* d, aistate& b, const vec& pos, float guard, float wander, int walk, bool retry) -> bool {
	vec feet = d->feetpos();
	if (walk == 2 || b.override || (walk && feet.squaredist(pos) <= guard * guard) ||
		!makeroute(d, b, pos)) {  // run away and back to keep ourselves busy
...
auto defend(gameent* d, aistate& b, const vec& pos, float guard, float wander, int walk) -> bool {
...
auto violence(gameent* d, aistate& b, gameent* e, int pursue) -> bool {
	if (e && targetable(d, e)) {
		if (pursue) {
...
auto target(gameent* d, aistate& b, int pursue = 0, bool force = false, float mindist = 0.f) -> bool {
	static vector<gameent*> hastried;
	hastried.setsize(0);
	vec dp = d->headpos();
	while (true) {
		float dist = 1e16f;
...
auto isgoodammo(int gun) -> int {
	return gun == GUN_PULSE || gun == GUN_RAIL;
}

auto hasgoodammo(gameent* d) -> bool {
...
auto parseinterests(gameent* d, aistate& b, vector<interest>& interests, bool override, bool ignore) -> bool {
...
auto find(gameent* d, aistate& b, bool override = false) -> bool {
...
auto findassist(gameent* d, aistate& b, bool override = false) -> bool {
	static vector<interest> interests;
	interests.setsize(0);
	assist(d, b, interests);
	while (!interests.empty()) {
...
auto check(gameent* d, aistate& b) -> bool {
	if (cmode && cmode->aicheck(d, b))
		return true;
	return false;
}

auto dowait(gameent* d, aistate& b) -> int {
...
auto dodefend(gameent* d, aistate& b) -> int {
	if (d->state == CS_ALIVE) {
		switch (b.targtype) {
		case AI_T_NODE:
			if (check(d, b))
...
auto dointerest(gameent* d, aistate& b) -> int {
	if (d->state != CS_ALIVE)
		return 0;
	switch (b.targtype) {
...
auto dopursue(gameent* d, aistate& b) -> int {
	if (d->state == CS_ALIVE) {
		switch (b.targtype) {
		case AI_T_NODE: {
			if (check(d, b))
...
auto wpspot(gameent* d, int n, bool check = false) -> int {
	if (iswaypoint(n))
		loopk(2) {
...
auto randomlink(gameent* d, int n) -> int {
...
auto anynode(gameent* d, aistate& b, int len = NUMPREVNODES) -> bool {
	if (iswaypoint(d->lastnode))
		loopk(2) {
...
auto checkroute(gameent* d, int n) -> bool {
		...
		if (d->ai->hasprevnode(v) || obstacles.find(v, d))	// something is in the way, try to remap around it
		{
			...
			loopirev(m) {
				...
				if (!d->ai->hasprevnode(t) && !obstacles.find(t, d)) {
					...
					remap.setsize(0);
					if (route(d, w, t, remap, obstacles)) {	 // kill what we don't want and put the remap in
...
auto hunt(gameent* d, aistate& b) -> bool {
	if (!d->ai->route.empty()) {
		int n = closenode(d);
...
auto lockon(gameent* d, int atk, gameent* e, float maxdist) -> bool {
...
auto process(gameent* d, aistate& b) -> int {
	...
	if (!enemyok || d->skill >= 50) {
		auto* f = (gameent*)intersectclosest(dp, d->ai->target, d, 1);
		if (f) {
			if (targetable(d, f)) {
				if (!enemyok)
					violence(d, b, f);
	...
	if (d->ai->dontmove)
		d->move = d->strafe = 0;
	else {	// our guys move one way.. but turn another?! :)
		const struct aimdir {
...
auto hasrange(gameent* d, gameent* e, int weap) -> bool {
	if (!e)
		return true;
	if (targetable(d, e)) {
...
auto request(gameent* d, aistate& b) -> bool {
	gameent* e = getclient(d->ai->enemy);
...
void logic(gameent* d, aistate& b, bool run) {
	...
	if (d->state == CS_ALIVE) {
		if (allowmove) {
			if (!request(d, b))
				target(d, b, 0, b.idle ? true : false);
			shoot(d, d->ai->target);
		}
		if (!intermission) {
			if (d->ragdoll)
