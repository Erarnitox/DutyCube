...
struct waypoint {
	...
	int score() const {
		return int(curscore) + int(estscore);
	}
...
enum { AI_T_NODE, AI_T_PLAYER, AI_T_AFFINITY, AI_T_ENTITY, AI_T_MAX };
...
struct aiinfo {
	...
	void clearsetup() {
		...
		lastaction = lasthunt = lastcheck = enemyseen = enemymillis = blocktime = huntseq = blockseq = targtime =
			targseq = lastaimrnd = 0;
...
extern bool checkothers(vector<int>& targets,
						gameent* d = NULL,
						int state = -1,
						int targtype = -1,
						int target = -1,
...
extern bool patrol(gameent* d,
				   aistate& b,
				   const vec& pos,
				   float guard = SIGHTMIN,
				   float wander = SIGHTMAX,
				   int walk = 1,
				   bool retry = false);
extern bool defend(gameent* d,
				   aistate& b,
				   const vec& pos,
				   float guard = SIGHTMIN,
				   float wander = SIGHTMAX,
...
extern bool parseinterests(gameent* d,
						   aistate& b,
