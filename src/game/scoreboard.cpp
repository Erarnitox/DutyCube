...
static inline auto playersort(const gameent* a, const gameent* b) -> bool {
	if (a->state == CS_SPECTATOR) {
		if (b->state == CS_SPECTATOR)
			return strcmp(a->name, b->name) < 0;
		else
			return false;
	} else if (b->state == CS_SPECTATOR)
		return true;
	if (m_ctf) {
		if (a->flags > b->flags)
			return true;
...
ICOMMAND(scoreboardhighlight,
		 "i",
		 (int* cn),
		 intret(*cn == player1->clientnum && highlightscore &&
...
ICOMMAND(scoreboardmode, "", (), { result(server::modeprettyname(gamemode)); });
