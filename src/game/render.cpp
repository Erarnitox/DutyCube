...
void saveragdoll(gameent* d) {
	if (!d->ragdoll || !ragdollmillis || (!ragdollfade && lastmillis > d->lastpain + ragdollmillis))
		return;
	auto* r = new gameent(*d);
	r->lastupdate = ragdollfade && lastmillis > d->lastpain + max(ragdollmillis - ragdollfade, 0)
		? lastmillis - max(ragdollmillis - ragdollfade, 0)
		: d->lastpain;
	r->edit = nullptr;
	r->ai = nullptr;
	if (d == player1)
...
static const int playercolors[] =
	{0xA12020, 0xA15B28, 0xB39D52, 0x3E752F, 0x3F748C, 0x214C85, 0xB3668C, 0x523678, 0xB3ADA3};

static const int playercolorsazul[] = {0x27508A, 0x3F748C, 0x3B3B80, 0x5364B5};

static const int playercolorsrojo[] = {0xAC2C2A, 0x992417, 0x802438, 0xA3435B};
...
static const playermodelinfo playermodels[] = {{{"player/bones", "player/bones", "player/bones"},
...
auto chooserandomplayermodel(int seed) -> int {
	return (seed & 0xFFFF) % (sizeof(playermodels) / sizeof(playermodels[0]));
}

auto getplayermodelinfo(int n) -> const playermodelinfo* {
	if (size_t(n) >= sizeof(playermodels) / sizeof(playermodels[0]))
		return nullptr;
	return &playermodels[n];
}

auto getplayermodelinfo(gameent* d) -> const playermodelinfo& {
...
auto getplayercolor(int team, int color) -> int {
#define GETPLAYERCOLOR(playercolors) return playercolors[color % (sizeof(playercolors) / sizeof(playercolors[0]))];
	switch (team) {
	case 1:
...
auto getplayercolor(gameent* d, int team) -> int {
	if (d == player1)
		switch (team) {
		case 1:
...
void renderplayer(gameent* d,
	...
	} else if (d->state == CS_ALIVE && d->lasttaunt && lastmillis - d->lasttaunt < 1000 &&
			   lastmillis - d->lastaction > delay) {
	...
	if (guns[d->gunselect].vwep) {
		...
		if (lastaction && d->lastattack >= 0 && attacks[d->lastattack].gun == d->gunselect &&
			lastmillis < lastaction + delay) {
	...
	float yaw = testanims && d == player1 ? 0 : d->yaw, pitch = testpitch && d == player1 ? testpitch : d->pitch;
	vec o = d->feetpos();
	int basetime = 0;
	if (animoverride)
		...
		if (d->inwater && d->physstate <= PHYS_FALL)
			anim |= (((game::allowmove(d) && (d->move || d->strafe)) || d->vel.z + d->falling.z > 0 ? ANIM_SWIM
																									: ANIM_SINK) |
					 ANIM_LOOP)
				<< ANIM_SECONDARY;
		else {
			static const int dirs[9] = {
				ANIM_RUN_SE, ANIM_RUN_S, ANIM_RUN_SW, ANIM_RUN_E, 0, ANIM_RUN_W, ANIM_RUN_NE, ANIM_RUN_N, ANIM_RUN_NW};
		...
		if (d->crouching)
			...
			case ANIM_JUMP:
				anim &= ~(ANIM_INDEX << ANIM_SECONDARY);
				anim |= ANIM_CROUCH_JUMP << ANIM_SECONDARY;
				break;
			case ANIM_SWIM:
				anim &= ~(ANIM_INDEX << ANIM_SECONDARY);
				anim |= ANIM_CROUCH_SWIM << ANIM_SECONDARY;
				break;
			case ANIM_SINK:
				anim &= ~(ANIM_INDEX << ANIM_SECONDARY);
				anim |= ANIM_CROUCH_SINK << ANIM_SECONDARY;
				break;
			case 0:
	...
	if (d->type == ENT_PLAYER)
		flags |= MDL_FULLBRIGHT;
	else
		flags |= MDL_CULL_DIST;
	if (!mainpass)
		flags &= ~(MDL_FULLBRIGHT | MDL_CULL_VFC | MDL_CULL_OCCLUDED | MDL_CULL_QUERY | MDL_CULL_DIST);
...
void rendergame() {
	...
	gameent *f = followingplayer(), *exclude = third ? nullptr : f;
	loopv(players) {
		gameent* d = players[i];
		if (d == player1 || d->state == CS_SPECTATOR || d->state == CS_SPAWNING || d->lifesequence < 0 ||
			d == exclude || (d->state == CS_DEAD && hidedead))
			continue;
		renderplayer(d);
	...
	loopv(ragdolls) {
		...
		if (ragdollmillis && ragdollfade)
			fade -= clamp(float(lastmillis - (d->lastupdate + max(ragdollmillis - ragdollfade, 0))) /
							  min(ragdollmillis, ragdollfade),
						  0.0f,
						  1.0f);
		renderplayer(d, fade);
	}
	if (exclude)
		renderplayer(exclude, 1, MDL_ONLYSHADOW);
			 ...
			 (player1->state == CS_ALIVE || (player1->state == CS_EDITING && third) ||
			  (player1->state == CS_DEAD && !hidedead)))
...
void drawhudmodel(gameent* d, int anim, int basetime) {
	...
	rendermodel(
		gunname, anim, sway, d->yaw, d->pitch, 0, MDL_NOBATCH, nullptr, a, basetime, 0, 1, vec4(vec::hexcolor(color), 1));
	if (d->muzzle.x >= 0)
...
void drawhudgun() {
	...
	if (d->lastaction && d->lastattack >= 0 && attacks[d->lastattack].gun == d->gunselect &&
		lastmillis - d->lastaction < attacks[d->lastattack].attackdelay) {
...
void renderplayerpreview(int model, int color, int team, int weap) {
	static gameent* previewent = nullptr;
	if (!previewent) {
		previewent = new gameent;
		loopi(NUMGUNS) previewent->ammo[i] = 1;
	}
	float height = previewent->eyeheight + previewent->aboveeye, zrad = height / 2;
...
auto hudgunorigin(int gun, const vec& from, const vec& to, gameent* d) -> vec {
	if (d->muzzle.x >= 0)
		return d->muzzle;
	vec offset(from);
