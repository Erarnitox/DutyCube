...
auto getweapon(const char* name) -> int {
	if (isdigit(name[0]))
...
void setweapon(const char* name, bool force = false) {
	...
	if (player1->state != CS_ALIVE || !validgun(gun))
		return;
	if (force || player1->ammo[gun])
		gunselect(gun, player1);
	else
...
void cycleweapon(int numguns, int* guns, bool force = false) {
	if (numguns <= 0 || player1->state != CS_ALIVE)
		return;
	int offset = 0;
	loopi(numguns) if (guns[i] == player1->gunselect) {
		offset = i + 1;
		break;
	}
	loopi(numguns) {
...
void offsetray(const vec& from, const vec& to, int spread, float range, vec& dest) {
	vec offset;
	do
		offset = vec(rndscale(1), rndscale(1), rndscale(1)).sub(0.5f);
...
struct bouncer : physent {
	int lifetime, bounces{0};
	float lastyaw, roll{0};
	bool local;
	gameent* owner;
	int bouncetype, variant{0};
	vec offset;
	int offsetmillis;
	int id;
...
void bounced(physent* d, const vec& surface) {
	if (d->type != ENT_BOUNCE)
		return;
	auto* b = (bouncer*)d;
	if (b->bouncetype != BNC_GIBS || b->bounces >= 2)
		return;
	b->bounces++;
	addstain(STAIN_BLOOD,
			 vec(b->o).sub(vec(surface).mul(b->radius)),
			 surface,
			 2.96f / b->bounces,
			 bvec(0x60, 0xFF, 0xFF),
			 rnd(4));
...
void damageeffect(int damage, gameent* d, bool thirdperson) {
	...
	if (blood)
		particle_splash(PART_BLOOD, max(damage / 1000, rnd(3) + 1), 1000, p, 0x60FFFF, 2.96f);
}
...
static auto sameteam(dynent* other) -> bool {
...
auto projdist(dynent* o, vec& dir, const vec& v, const vec& vel) -> float {
	vec middle = o->o;
...
void radialeffect(dynent* o, const vec& v, const vec& vel, int qdam, gameent* at, int atk) {
	...
	if (dist < attacks[atk].exprad) {
		...
		if (damage > 0)
			hit(max(int(damage), 1), o, at, dir, atk, dist);
	}
}
...
void explode(bool local, gameent* owner, const vec& v, const vec& vel, dynent* safe, int damage, int atk) {
	...
	adddynlight(safe ? v : debrisorigin,
				2 * attacks[atk].exprad,
				vec(1.0f, 3.0f, 4.0f),
				350,
				40,
				0,
				attacks[atk].exprad / 2,
				vec(0.5f, 1.5f, 2.0f));
...
auto projdamage(dynent* o, projectile& p, const vec& v) -> bool {
	if (o->state != CS_ALIVE)
		return false;
		...
		if (!exploded) {
			if (dist < 4) {
				...
				if (p.owner != noside) {
					...
					vec dir = vec(dv).normalize(), tail = vec(dir).mul(-len).add(pos),
		...
		if (exploded) {
			if (p.local)
				addmsg(N_EXPLODE,
					   ...
					   p.id - maptime,
					   hits.length(),
					   hits.length() * sizeof(hitmsg) / sizeof(int),
					   hits.getbuf());
			projs.remove(i--);
		} else
			p.o = v;
...
void shoteffects(int atk,
				 const vec& from,
				 const vec& to,
				 gameent* d,
				 bool local,
				 int id,
				 int prevaction)  // create visual effect from a shot
{
...
auto intersect(dynent* d,
			   const vec& from,
			   const vec& to,
			   float margin,
			   float& dist) -> bool	 // if lineseg hits entity bounding box
{
...
auto intersectclosest(const vec& from, const vec& to, gameent* at, float margin, float& bestdist) -> dynent* {
	dynent* best = nullptr;
	bestdist = 1e16f;
	loopi(numdynents()) {
		dynent* o = iterdynents(i);
...
void shoot(gameent* d, const vec& targ) {
	...
	if (attacks[atk].action != ACT_MELEE && d->ai)
		d->gunwait += int(d->gunwait * (((101 - d->skill) + rnd(111 - d->skill)) / 100.f));
...
void updateweapons(int curtime) {
	...
	updatebouncers(
		curtime);  // need to do this after the player shoots so bouncers don't end up inside player's BB next frame
}
