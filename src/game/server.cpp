...
#endif
			if (!server::serveroption(args[i])) conoutf(CON_ERROR, "unknown command-line option: %s", args[i]);
}

auto gameident() -> const char* {
	return "DropEngine";
}
}  // namespace game
...
struct gameevent {
	...
	virtual auto flush(clientinfo* ci, int fmillis) -> bool;
	virtual void process(clientinfo* ci) {
	}

	[[nodiscard]] virtual auto keepable() const -> bool {
		return false;
	}
};
...
struct explodeevent : timedevent {
	...
	[[nodiscard]] bool keepable() const override {
		return true;
	}
...
struct servstate : gamestate {
	...
	auto isalive(int gamemillis) -> bool {
		return state == CS_ALIVE || (state == CS_DEAD && gamemillis - lastdeath <= DEATHMILLIS);
	}

	auto waitexpired(int gamemillis) -> bool {
...
struct clientinfo {
	...
	ENetPacket *getdemo{NULL}, *getmap{NULL}, *clipboard{NULL};
	int lastclipboard, needclipboard;
	int connectauth;
	uint authreq;
	string authname, authdesc;
	void* authchallenge{NULL};
	int authkickvictim;
	char* authkickreason{NULL};
	...
	auto calcpushrange() -> int {
		ENetPeer* peer = getclientpeer(ownernum);
		return PUSHMILLIS +
			(peer ? peer->roundTripTime + peer->roundTripTimeVariance : ENET_PEER_DEFAULT_ROUND_TRIP_TIME);
	}

	auto checkpushed(int millis, int range) -> bool {
		return millis >= pushed - range && millis <= pushed + range;
	}

	void scheduleexceeded() {
		if (state.state != CS_ALIVE || !exceeded)
			return;
	...
	auto geteventmillis(int servmillis, int clientmillis) -> int {
...
void addban(uint ip, int expire) {
	...
	b.ip = ip;
	loopv(bannedips) if (bannedips[i].expire - b.expire > 0) {
...
struct maprotation {
	...
	[[nodiscard]] auto calcmodemask() const -> int {
		return modes & (1 << NUMGAMEMODES) ? modes & ~exclude : modes;
	}
	[[nodiscard]] auto hasmode(int mode, int offset = STARTGAMEMODE) const -> bool {
		return (calcmodemask() & (1 << (mode - offset))) != 0;
	}

	[[nodiscard]] auto findmode(int mode) const -> int {
		if (!hasmode(mode))
			loopi(NUMGAMEMODES) if (hasmode(i, 0)) return i + STARTGAMEMODE;
		return mode;
	}

	auto match(int reqmode, const char* reqmap) const -> bool {
		return hasmode(reqmode) && (!map[0] || !reqmap[0] || !strcmp(map, reqmap));
	}

	[[nodiscard]] auto includes(const maprotation& rot) const -> bool {
...
auto findmaprotation(int mode, const char* map) -> int {
	...
	int start;
	for (start = max(curmaprotation, 0) - 1; start >= 0; start--)
		if (!maprotations[start].modes)
			break;
	start++;
...
auto searchmodename(const char* haystack, const char* needle) -> bool {
	if (!needle[0])
		return true;
	do {
		if (needle[0] != '.') {
...
auto genmodemask(vector<char*>& modes) -> int {
	int modemask = 0;
	loopv(modes) {
...
auto addmaprotation(int modemask, const char* map) -> bool {
	if (!map[0])
		loopk(NUMGAMEMODES) if (modemask & (1 << k) && !m_check(k + STARTGAMEMODE, M_EDIT)) modemask &= ~(1 << k);
	if (!modemask)
		return false;
...
void addmaprotations(tagval* args, int numargs) {
	...
	for (int i = 0; i + 1 < numargs; i += 2) {
		...
		if (maps.length())
			loopvj(maps) addmaprotation(modemask, maps[j]);
		else
...
struct teamkillkick {
	...
	[[nodiscard]] auto match(int mode) const -> bool {
		return (modes & (1 << (mode - STARTGAMEMODE))) != 0;
	}

	[[nodiscard]] auto includes(const teamkillkick& tk) const -> bool {
...
void addteamkill(clientinfo* actor, clientinfo* victim, int n) {
	if (!m_timed || actor->state.aitype != AI_NONE || actor->local || actor->privilege ||
		(victim && victim->state.aitype != AI_NONE))
		return;
...
void checkteamkills() {
	...
	if (m_timed)
		loopv(teamkillkicks) if (teamkillkicks[i].match(gamemode) && (!kick || kick->includes(teamkillkicks[i]))) kick =
			&teamkillkicks[i];
	if (kick)
		loopvrev(teamkills) {
...
auto msgsizelookup(int msg) -> int {
...
auto modename(int n, const char* unknown) -> const char* {
	if (m_valid(n))
		return gamemodes[n - STARTGAMEMODE].name;
	return unknown;
}

auto modeprettyname(int n, const char* unknown) -> const char* {
	if (m_valid(n))
		return gamemodes[n - STARTGAMEMODE].prettyname;
	return unknown;
}

auto mastermodename(int n, const char* unknown) -> const char* {
	return (n >= MM_START && size_t(n - MM_START) < sizeof(mastermodenames) / sizeof(mastermodenames[0]))
		? mastermodenames[n - MM_START]
		: unknown;
}

auto privname(int type) -> const char* {
	switch (type) {
	case PRIV_ADMIN:
		return "admin";
	case PRIV_AUTH:
...
auto serveroption(const char* arg) -> bool {
	return false;
}
...
auto numclients(int exclude = -1, bool nospec = true, bool noai = true, bool priv = false) -> int {
	int n = 0;
	loopv(clients) {
		clientinfo* ci = clients[i];
		if (ci->clientnum != exclude &&
			(!nospec || ci->state.state != CS_SPECTATOR || (priv && (ci->privilege || ci->local))) &&
...
auto duplicatename(clientinfo* ci, const char* name) -> bool {
	if (!name)
		name = ci->name;
...
auto colorname(clientinfo* ci, const char* name = nullptr) -> const char* {
	if (!name)
		name = ci->name;
	...
	cidx = (cidx + 1) % 3;
	formatstring(
		cname[cidx], ci->state.aitype == AI_NONE ? "%s \fs\f5(%d)\fr" : "%s \fs\f5[%d]\fr", name, ci->clientnum);
	return cname[cidx];
}

struct servmode {
	virtual ~servmode() = default;
	...
	virtual auto canspawn(clientinfo* ci, bool connecting = false) -> bool {
		return true;
	}
	virtual void spawned(clientinfo* ci) {
	}
	virtual auto fragvalue(clientinfo* victim, clientinfo* actor) -> int {
		if (victim == actor || isteam(victim->team, actor->team))
			return -1;
		return 1;
	}
	virtual void died(clientinfo* victim, clientinfo* actor) {
	}
	virtual auto canchangeteam(clientinfo* ci, int oldteam, int newteam) -> bool {
		return true;
	}
	...
	virtual auto hidefrags() -> bool {
		return false;
	}
	virtual auto getteamscore(int team) -> int {
		return 0;
	}
	virtual void getteamscores(vector<teamscore>& scores) {
	}
	virtual auto extinfoteam(int team, ucharbuf& p) -> bool {
		return false;
	}
};
...
auto pickup(int i, int sender) -> bool	// server side item pickup, acknowledge first client that gets it
{
...
auto choosebestclient(float& bestrank) -> clientinfo* {
...
auto chooseworstteam(clientinfo* exclude = nullptr) -> int {
	teamrank teamranks[MAXTEAMS];
	loopv(clients) {
		clientinfo* ci = clients[i];
		if (ci == exclude || ci->state.aitype != AI_NONE || ci->state.state == CS_SPECTATOR || !validteam(ci->team))
			continue;
...
auto welcomepacket(packetbuf& p, clientinfo* ci) -> int;
void sendwelcome(clientinfo* ci);

void setupdemorecord() {
	if (!m_mp(gamemode) || m_edit)
		return;
...
void setupdemoplayback() {
	...
	if (!demoplayback)
		...
		if (hdr.version != DEMO_VERSION)
			formatstring(msg,
						 "demo \"%s\" requires an %s version of DropEngine",
						 file,
...
void checkpausegame() {
	if (!gamepaused)
		return;
	int admins = 0;
	loopv(clients) if (clients[i]->privilege >= (restrictpausegame ? PRIV_ADMIN : PRIV_MASTER) || clients[i]->local)
		admins++;
	if (!admins)
		pausegame(false);
}
...
static inline auto hthash(const userkey& k) -> uint {
	return ::hthash(k.name);
}
static inline auto htcmp(const userkey& x, const userkey& y) -> bool {
	return !strcmp(x.name, y.name) && !strcmp(x.desc, y.desc);
}
...
auto checkpassword(clientinfo* ci, const char* wanted, const char* given) -> bool {
	string hash;
...
auto setmaster(clientinfo* ci,
			   bool val,
			   const char* pass = "",
			   const char* authname = nullptr,
			   const char* authdesc = nullptr,
			   int authpriv = PRIV_MASTER,
			   bool force = false,
			   bool trial = false) -> bool {
	if (authname && !val)
		return false;
	const char* name = "";
	if (val) {
...
auto trykick(clientinfo* ci,
			 int victim,
			 const char* reason = nullptr,
			 const char* authname = nullptr,
			 const char* authdesc = nullptr,
			 int authpriv = PRIV_NONE,
			 bool trial = false) -> bool {
	...
	if ((priv || ci->local) && ci->clientnum != victim) {
		auto* vinfo = (clientinfo*)getclientinfo(victim);
		if (vinfo && vinfo->connected && (priv >= vinfo->privilege || ci->local) && vinfo->privilege < PRIV_ADMIN &&
			!vinfo->local) {
			if (trial)
				return true;
			string kicker;
...
auto findscore(clientinfo* ci, bool insert) -> savedscore* {
...
static struct msgfilter {
	...
	auto operator[](int msg) const -> uchar {
		return msg >= 0 && msg < NUMMSG ? msgmask[msg] : 0;
	}
} msgfilter(-1,
			N_CONNECT,
			N_SERVINFO,
...
auto checktype(int type, clientinfo* ci) -> int {
	if (ci) {
		if (!ci->connected)
...
struct worldstate {
	...
	auto contains(const uchar* p) const -> bool {
...
static void sendmessages(worldstate& ws, ucharbuf& wsbuf) {
	...
	loopv(clients) {
		...
		ENetPacket* packet = enet_packet_create(
			data, size, (reliablemessages ? ENET_PACKET_FLAG_RELIABLE : 0) | ENET_PACKET_FLAG_NO_ALLOCATE);
...
auto sendpackets(bool force) -> bool {
...
auto hasmap(clientinfo* ci) -> bool {
	return (m_edit && (clients.length() > 0 || ci->local)) ||
		(smapname[0] &&
		 (!m_timed || gamemillis < gamelimit || (ci->state.state == CS_SPECTATOR && !ci->privilege && !ci->local) ||
		  numclients(ci->clientnum, true, true, true)));
}
...
auto restorescore(clientinfo* ci) -> bool {
...
void changemap(const char* s, int mode) {
	...
	loopv(clients) {
		...
		ci->mapchange();
		ci->state.lasttimeplayed = lastmillis;
		if (m_mp(gamemode) && ci->state.state != CS_SPECTATOR)
			sendspawn(ci);
	}
...
void checkvotes(bool force = false) {
	...
	loopv(clients) {
		clientinfo* oi = clients[i];
		if (oi->state.state == CS_SPECTATOR && !oi->privilege && !oi->local)
			continue;
		if (oi->state.aitype != AI_NONE)
			continue;
		maxvotes++;
		if (!m_valid(oi->modevote))
			continue;
	...
	loopv(votes) if (!best || votes[i].count > best->count || (votes[i].count == best->count && rnd(2))) best =
		&votes[i];
...
void vote(const char* map, int reqmode, int sender) {
	clientinfo* ci = getinfo(sender);
	if (!ci || (ci->state.state == CS_SPECTATOR && !ci->privilege && !ci->local) || (!ci->local && !m_mp(reqmode)))
		return;
	if (!m_valid(reqmode))
		return;
	if (!map[0] && !m_check(reqmode, M_EDIT)) {
		...
		if (idx < 0 && smapname[0])
			idx = findmaprotation(reqmode, "");
		if (idx < 0)
			return;
		map = maprotations[idx].map;
	}
	if (lockmaprotation && !ci->local && ci->privilege < (lockmaprotation > 1 ? PRIV_ADMIN : PRIV_MASTER) &&
		findmaprotation(reqmode, map) < 0) {
	...
	if (ci->local || (ci->privilege && mastermode >= MM_VETO)) {
		...
		if (!ci->local || hasnonlocalclients())
			sendservmsgf("%s forced %s on map %s",
						 colorname(ci),
						 modeprettyname(ci->modevote),
						 ci->mapvote[0] ? ci->mapvote : "[new map]");
		changemap(ci->mapvote, ci->modevote);
	} else {
		sendservmsgf("%s suggests %s on map %s (select map to vote)",
					 colorname(ci),
					 modeprettyname(reqmode),
					 map[0] ? map : "[new map]");
		checkvotes();
	}
}
...
void dodamage(clientinfo* target, clientinfo* actor, int damage, int atk, const vec& hitpush = vec(0, 0, 0)) {
	...
	if (target == actor)
		...
		sendf(
			ts.health <= 0 ? -1 : target->ownernum, 1, "ri7", N_HITPUSH, target->clientnum, atk, damage, v.x, v.y, v.z);
		target->setpushed();
	}
	if (ts.health <= 0) {
		target->state.deaths++;
		int fragvalue =
			smode ? smode->fragvalue(target, actor) : (target == actor || isteam(target->team, actor->team) ? -1 : 1);
		actor->state.frags += fragvalue;
		if (fragvalue > 0) {
			int friends = 0, enemies = 0;  // note: friends also includes the fragger
			if (m_teammode)
				loopv(clients) if (clients[i]->team != actor->team) enemies++;
			else
				friends++;
			else {
				friends = 1;
		...
		teaminfo* t = m_teammode && validteam(actor->team) ? &teaminfos[actor->team - 1] : nullptr;
		if (t)
			t->frags += fragvalue;
...
void suicide(clientinfo* ci) {
	...
	ci->state.deaths++;
	teaminfo* t = m_teammode && validteam(ci->team) ? &teaminfos[ci->team - 1] : nullptr;
	if (t)
		t->frags += fragvalue;
...
void explodeevent::process(clientinfo* ci) {
	...
	loopv(hits) {
		...
		if (!target || target->state.state != CS_ALIVE || h.lifesequence != target->state.lifesequence || h.dist < 0 ||
			h.dist > attacks[atk].exprad)
			continue;

		bool dup = false;
		loopj(i) if (hits[j].target == h.target) {
			dup = true;
			break;
		}
		if (dup)
...
void shotevent::process(clientinfo* ci) {
	...
	default: {
		...
		loopv(hits) {
			hitinfo& h = hits[i];
			clientinfo* target = getinfo(h.target);
			if (!target || target->state.state != CS_ALIVE || h.lifesequence != target->state.lifesequence ||
				h.rays < 1 || h.dist > attacks[atk].range + 1)
				continue;
...
auto gameevent::flush(clientinfo* ci, int fmillis) -> bool {
	process(ci);
	return true;
}

auto timedevent::flush(clientinfo* ci, int fmillis) -> bool {
	if (millis > fmillis)
		return false;
...
void serverupdate() {
	...
	while (bannedips.length() && bannedips[0].expire - totalmillis <= 0)
		bannedips.remove(0);
	...
	if (shouldcheckteamkills)
		checkteamkills();

	if (shouldstep && !gamepaused) {
		if (m_timed && smapname[0] && gamemillis - curtime > 0)
			checkintermission();
...
struct crcinfo {
	...
	static auto compare(const crcinfo& x, const crcinfo& y) -> bool {
...
void checkmaps(int req = -1) {
	...
	loopv(clients) {
		clientinfo* ci = clients[i];
		if (ci->state.state == CS_SPECTATOR || ci->state.aitype != AI_NONE)
			continue;
		total++;
		if (!ci->clientmap[0]) {
			if (ci->mapcrc < 0)
			...
			loopvj(crcs) if (crcs[j].crc == ci->mapcrc) {
				match = &crcs[j];
				break;
			}
			if (!match)
	...
	loopv(clients) {
		clientinfo* ci = clients[i];
		if (ci->state.state == CS_SPECTATOR || ci->state.aitype != AI_NONE || ci->clientmap[0] || ci->mapcrc >= 0 ||
			(req < 0 && ci->warned))
			continue;
		...
		loopv(crcs) {
			...
			if (i || info.matches <= crcs[i + 1].matches)
				loopvj(clients) {
					...
					if (ci->state.state == CS_SPECTATOR || ci->state.aitype != AI_NONE || !ci->clientmap[0] ||
						ci->mapcrc != info.crc || (req < 0 && ci->warned))
						continue;
					formatstring(msg, "%s has modified map \"%s\"", colorname(ci), smapname);
					sendf(req, 1, "ris", N_SERVMSG, msg);
					if (req < 0)
...
auto shouldspectate(clientinfo* ci) -> bool {
...
auto clientconnect(int n, uint ip) -> int {
	clientinfo* ci = getinfo(n);
	ci->clientnum = ci->ownernum = n;
	ci->connectmillis = totalmillis;
	ci->sessionid = (rnd(0x1000000) * ((totalmillis % 10000) + 1)) & 0xFFFFFF;
...
auto allowconnect(clientinfo* ci, const char* pwd = "") -> int {
	if (ci->local)
		return DISC_NONE;
	if (!m_mp(gamemode))
		return DISC_LOCAL;
	if (serverpass[0]) {
		if (!checkpassword(ci, serverpass, pwd))
	...
	if (numclients(-1, false, true) >= maxclients)
		return DISC_MAXCLIENTS;
	uint ip = getclientip(ci->clientnum);
	if (checkbans(ip))
		return DISC_IPBAN;
	if (mastermode >= MM_PRIVATE && allowedips.find(ip) < 0)
		return DISC_PRIVATE;
	return DISC_NONE;
}

auto allowbroadcast(int n) -> bool {
	clientinfo* ci = getinfo(n);
	return ci && ci->connected;
}

auto findauth(uint id) -> clientinfo* {
...
void authsucceeded(uint id) {
	...
	if (ci->authkickvictim >= 0) {
		if (setmaster(ci, true, "", ci->authname, nullptr, PRIV_AUTH, false, true))
			trykick(ci, ci->authkickvictim, ci->authkickreason, ci->authname, nullptr, PRIV_AUTH);
		ci->cleanauthkick();
	} else
		setmaster(ci, true, "", ci->authname, nullptr, PRIV_AUTH);
}
...
auto tryauth(clientinfo* ci, const char* user, const char* desc) -> bool {
	ci->cleanauth();
	if (!nextauthreq)
		nextauthreq = 1;
	...
	if (ci->authdesc[0]) {
		...
		if (u) {
			uint seed[3] = {::hthash(serverauth) + detrnd(size_t(ci) + size_t(user) + size_t(desc), 0x10000),
							uint(totalmillis),
							randomMT()};
			vector<char> buf;
...
auto answerchallenge(clientinfo* ci, uint id, char* val, const char* desc) -> bool {
...
void receivefile(int sender, uchar* data, int len) {
	if (!m_edit || len <= 0 || len > 4 * 1024 * 1024)
		return;
	clientinfo* ci = getinfo(sender);
	if (ci->state.state == CS_SPECTATOR && !ci->privilege && !ci->local)
		return;
	if (mapdata)
		DELETEP(mapdata);
	mapdata = opentempfile("mapdata", "w+b");
	if (!mapdata) {
		sendf(sender, 1, "ris", N_SERVMSG, "failed to open temporary file for map");
		return;
	}
...
void connected(clientinfo* ci) {
	...
	if (servermotd[0])
		sendf(ci->clientnum, 1, "ris", N_SERVMSG, servermotd);
}
		...
		if (chan == 0)
			...
			while (p.length() < p.maxlen)
				...
				case N_CONNECT: {
					...
					if (disc) {
						if (disc == DISC_LOCAL || !serverauth[0] || strcmp(serverauth, authdesc) ||
							!tryauth(ci, authname, authdesc)) {
...
#define QUEUE_STR(text) QUEUE_BUF(sendstring(text, cm->messages))
	...
	while ((curmsg = p.length()) < p.maxlen)
		...
		case N_POS: {
			...
			if (cp) {
				if ((!ci->local || demorecord || hasnonlocalclients()) &&
					(cp->state.state == CS_ALIVE || cp->state.state == CS_EDITING)) {
		...
		case N_TELEPORT: {
			...
			if (cp && pcn != sender && cp->ownernum != sender)
				cp = nullptr;
			if (cp && (!ci->local || demorecord || hasnonlocalclients()) &&
				(cp->state.state == CS_ALIVE || cp->state.state == CS_EDITING)) {
		...
		case N_EDITMODE: {
			...
			if (!ci->local && !m_edit)
				break;
			if (val ? ci->state.state != CS_ALIVE && ci->state.state != CS_DEAD : ci->state.state != CS_EDITING)
				break;
			if (smode) {
				if (val)
					smode->leavegame(ci);
		...
		case N_TRYSPAWN:
			if (!ci || !cq || cq->state.state != CS_DEAD || cq->state.lastspawn >= 0 || (smode && !smode->canspawn(cq)))
				break;
			if (!ci->clientmap[0] && !ci->mapcrc) {
				ci->mapcrc = -1;
				checkmaps();
		...
		case N_GUNSELECT: {
			...
			if (!cq || cq->state.state != CS_ALIVE || !validgun(gunselect))
				break;
			QUEUE_AI;
			QUEUE_MSG;
			break;
		...
		case N_SPAWN: {
			...
			if (!cq || (cq->state.state != CS_ALIVE && cq->state.state != CS_DEAD && cq->state.state != CS_EDITING) ||
				ls != cq->state.lifesequence || cq->state.lastspawn < 0 || !validgun(gunselect))
				break;
		...
		case N_SAYTEAM: {
			getstring(text, p);
			if (!ci || !cq || (ci->state.state == CS_SPECTATOR && !ci->local && !ci->privilege) || !m_teammode ||
				!validteam(cq->team))
				break;
			filtertext(text, text, true, true);
			loopv(clients) {
				clientinfo* t = clients[i];
				if (t == cq || t->state.state == CS_SPECTATOR || t->state.aitype != AI_NONE || cq->team != t->team)
					continue;
		...
		case N_SWITCHTEAM: {
			int team = getint(p);
			if (m_teammode && validteam(team) && ci->team != team &&
		...
		case N_ITEMLIST: {
			if ((ci->state.state == CS_SPECTATOR && !ci->privilege && !ci->local) || !notgotitems ||
			...
			while ((n = getint(p)) >= 0 && n < MAXENTS && !p.overread()) {
				...
				if (canspawnitem(sents[n].type)) {
					if (m_mp(gamemode) && delayspawn(sents[n].type))
		...
		case N_MASTERMODE: {
			...
			if ((ci->privilege || ci->local) && mm >= MM_OPEN && mm <= MM_PRIVATE) {
				if ((ci->privilege >= PRIV_ADMIN || ci->local) || (mastermask & (1 << mm))) {
					...
					sendf(
						sender, 1, "ris", N_SERVMSG, tempformatstring("mastermode %d is disabled on this server", mm));
				}
			}
			break;
		}
		...
		case N_SPECTATOR: {
			...
			if (!ci->privilege && !ci->local &&
				(spectator != sender || (ci->state.state == CS_SPECTATOR && mastermode >= MM_LOCKED)))
				break;
			auto* spinfo = (clientinfo*)getclientinfo(spectator);	 // no bots
			if (!spinfo || !spinfo->connected || (spinfo->state.state == CS_SPECTATOR ? val : !val))
				break;
		...
		case N_SETTEAM: {
			...
			if (!m_teammode || !validteam(team) || !wi || !wi->connected || wi->team == team)
				break;
		...
		case N_LISTDEMOS:
			if (!ci->privilege && !ci->local && ci->state.state == CS_SPECTATOR)
				break;
			listdemos(sender);
			break;
		...
		case N_GETMAP:
			if (!mapdata)
				...
				sendf(sender, 1, "ris", N_SERVMSG, "already sending map");
			else {
		...
		case N_SETMASTER: {
			...
			if (mn != ci->clientnum) {
				if (!ci->privilege && !ci->local)
					break;
				auto* minfo = (clientinfo*)getclientinfo(mn);
				if (!minfo || !minfo->connected || (!ci->local && minfo->privilege >= ci->privilege) ||
					(val && minfo->privilege))
					break;
		...
		genericmsg: {
			...
			if (ci)
				...
				case 3:
					...
					break;
				default:
					if (cq && (ci != cq || ci->state.state != CS_SPECTATOR)) {
						QUEUE_AI;
						QUEUE_MSG;
					}
					break;
...
auto laninfoport() -> int {
	return DROP_ENGINE_LANINFO_PORT;
}
auto serverport() -> int {
	return DROP_ENGINE_SERVER_PORT;
}
auto defaultmaster() -> const char* {
	// TODO(m): Use a custom DropEngine server some day. This is just a dummy address.
	return "master.dropengine.foo";
}
auto masterport() -> int {
	return DROP_ENGINE_MASTER_PORT;
}
...
void serverinforeply(ucharbuf& req, ucharbuf& p) {
	...
	putint(p,
		   serverpass[0]
			   ? MM_PASSWORD
			   : (!m_mp(gamemode) ? MM_PRIVATE : (mastermode || mastermask & MM_AUTOAPPROVE ? mastermode : MM_AUTH)));
