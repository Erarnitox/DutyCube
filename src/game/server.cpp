...
namespace game
{
    ...
    const char *gameident() { return "DropEngine"; }
}
...
namespace server
{
    ...
    void setupdemoplayback()
    {
        ...
        {
            lilswap(&hdr.version, 2);
            if(hdr.version!=DEMO_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of DropEngine", file, hdr.version<DEMO_VERSION ? "older" : "newer");
            else if(hdr.protocol!=PROTOCOL_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of DropEngine", file, hdr.protocol<PROTOCOL_VERSION ? "older" : "newer");
        }
        if(msg[0])
        {
            DELETEP(demoplayback);
    ...
    int laninfoport() { return DROP_ENGINE_LANINFO_PORT; }
    int serverport() { return DROP_ENGINE_SERVER_PORT; }
    ...
    int masterport() { return DROP_ENGINE_MASTER_PORT; }
