...
auto getweight(const vec& o) -> int {
	vec pos = o;
	pos.z += ai::JUMPMIN;
	...
	if (isliquid(posmat & MATF_VOLUME))
		weight *= 5;
	if (dist >= 0) {
		weight = int(dist / ai::JUMPMIN);
		pos.z -= clamp(dist - 8.0f, 0.0f, pos.z);
		int trgmat = lookupmaterial(pos);
		if (trgmat & MAT_DEATH || (trgmat & MATF_VOLUME) == MAT_LAVA)
			weight *= 10;
		else if (isliquid(trgmat & MATF_VOLUME))
			weight *= 2;
	}
	return weight;
}
...
struct wpcache {
	struct node {
		...
		[[nodiscard]] auto axis() const -> int {
			return child[0] >> 30;
		}
		[[nodiscard]] auto childindex(int which) const -> int {
			return child[which] & 0x3FFFFFFF;
		}
		[[nodiscard]] auto isleaf(int which) const -> bool {
	...
	void build(int* indices, int numindices, const vec& vmin, const vec& vmax) {
		...
		vec leftmin(1e16f, 1e16f, 1e16f), leftmax(-1e16f, -1e16f, -1e16f), rightmin(1e16f, 1e16f, 1e16f),
		...
		if (numindices - right <= 1)
			curnode.child[1] =
				(1 << 31) | (left <= 1 ? 1 << 30 : 0) | (numindices - right > 0 ? indices[right] : 0x3FFFFFFF);
		else {
...
static inline void invalidatewpcache(int wp) {
	if (++numinvalidatewpcaches >= 1000) {
		...
		loopi(WPCACHE_DYNAMIC) if (wp >= wpcaches[i].firstwp && wp <= wpcaches[i].lastwp) {
...
void clearwpcache(bool full = true) {
	loopi(NUMWPCACHES) if (full || invalidatedwpcaches & (1 << i)) {
		wpcaches[i].clear();
		clearedwpcaches |= 1 << i;
	}
...
void buildwpcache() {
	loopi(NUMWPCACHES) if (wpcaches[i].firstwp < 0) wpcaches[i].build(
		i > 0 ? wpcaches[i - 1].lastwp + 1 : 1,
		i + 1 >= NUMWPCACHES || wpcaches[i + 1].firstwp < 0 ? -1 : wpcaches[i + 1].firstwp);
	clearedwpcaches = 0;
	lastwpcache = waypoints.length();

	wpavoid.clear();
	loopv(waypoints) if (waypoints[i].weight < 0)
		wpavoid.avoidnear(nullptr, waypoints[i].o.z + WAYPOINTRADIUS, waypoints[i].o, WAYPOINTRADIUS);
}
...
auto closestwaypoint(const vec& pos, float mindist, bool links, gameent* d) -> int {
	if (waypoints.empty())
		return -1;
	if (clearedwpcaches)
		buildwpcache();
...
#define CHECKCLOSEST(index)                       \
	...
	loop(which, NUMWPCACHES) if (wpcaches[which].firstwp >=
								 0) for (curnode = &wpcaches[which].nodes[0], wpcachestack.setsize(0);;) {
...
auto avoidset::remap(gameent* d, int n, vec& pos, bool retry) -> int {
	if (!obstacles.empty()) {
		int cur = 0;
		loopv(obstacles) {
			...
			if (ob.owner != d) {
				for (; cur < next; cur++)
					if (waypoints[cur] == n) {
						...
						if (ai::iswaypoint(node) && node != n) {  // try to reroute above their head?
							if (!find(node, d)) {
...
static inline auto heapscore(waypoint* q) -> float {
	return q->score();
}

auto route(gameent* d, int node, int goal, vector<int>& route, const avoidset& obstacles, int retries) -> bool {
	...
	if (d) {
		if (retries <= 1 && d->ai)
			loopi(ai::NUMPREVNODES) if (d->ai->prevnodes[i] != node && iswaypoint(d->ai->prevnodes[i])) {
		...
		if (retries <= 0) {
			loopavoid(obstacles, d, {
				if (iswaypoint(wp) && wp != node && wp != goal && waypoints[node].find(wp) < 0 &&
					waypoints[goal].find(wp) < 0) {
...
auto addwaypoint(const vec& o, int weight = -1) -> int {
...
static inline auto shouldnavigate() -> bool {
	if (dropwaypoints)
		return true;
...
static inline auto shoulddrop(gameent* d) -> bool {
...
void navigate(gameent* d) {
	...
	if ((mat & MATF_CLIP) == MAT_CLIP || (mat & MATF_VOLUME) == MAT_LAVA || mat & MAT_DEATH)
		dropping = false;
...
auto getwaypointfile(const char* mname, char* wptname) -> bool {
	if (!mname || !*mname)
		mname = getclientmap();
	if (!*mname)
		return false;
...
void movewaypoints(const vec& d) {
	...
	if (d.x < -worldsize || d.x > worldsize || d.y < -worldsize || d.y > worldsize || d.z < -worldsize ||
		d.z > worldsize) {
		clearwaypoints();
		return;
	}
