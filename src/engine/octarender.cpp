...
enum { VBO_VBUF = 0, VBO_EBUF, VBO_SKYBUF, VBO_DECALBUF, NUMVBO };
...
void genvbo(int type, void* buf, int len, vtxarray** vas, int numva) {
	...
	if (printvbo)
		conoutf(CON_DEBUG, "vbo %d: type %d, size %d, %d uses", vbo, type, len, numva);
...
auto addvbo(vtxarray* va, int type, int numelems, int elemsize) -> uchar* {
	switch (type) {
	case VBO_VBUF:
...
struct verthash {
	...
	auto addvert(const vec& pos,
				const vec& tc = vec(0, 0, 0),
				const bvec& norm = bvec(128, 128, 128),
				const bvec4& tangent = bvec4(128, 128, 128, 128)) -> int {
		vertex vtx;
		vtx.pos = pos;
		vtx.tc = tc;
		vtx.norm = norm;
...
enum { NO_ALPHA = 0, ALPHA_BACK, ALPHA_FRONT, ALPHA_REFRACT };
...
struct sortkey {
	...
	auto operator==(const sortkey& o) const -> bool {
		return tex == o.tex && envmap == o.envmap && orient == o.orient && layer == o.layer && alpha == o.alpha;
	}

	static inline auto sort(const sortkey& x, const sortkey& y) -> bool {
		if (x.alpha < y.alpha)
			return true;
		if (x.alpha > y.alpha)
			return false;
...
static inline auto htcmp(const sortkey& x, const sortkey& y) -> bool {
	return x == y;
}

static inline auto hthash(const sortkey& k) -> uint {
	return k.tex;
}
...
struct decalkey {
	...
	decalkey() = default;
	decalkey(ushort tex, ushort envmap = EMID_NONE, ushort reuse = 0) : tex(tex), envmap(envmap), reuse(reuse) {
	}

	auto operator==(const decalkey& o) const -> bool {
		return tex == o.tex && envmap == o.envmap && reuse == o.reuse;
	}

	static inline auto sort(const decalkey& x, const decalkey& y) -> bool {
		if (x.tex == y.tex) {
			if (x.envmap < y.envmap)
				return true;
			if (x.envmap > y.envmap)
...
static inline auto htcmp(const decalkey& x, const decalkey& y) -> bool {
	return x == y;
}

static inline auto hthash(const decalkey& k) -> uint {
	return k.tex;
}
...
#define GENVERTS(type, ptr, body)       \
	...
	void gendecal(const extentity& e, DecalSlot& s, const decalkey& key) {
		...
		if (!s.sts.empty()) {
			Texture* t = s.sts[0].t;
			if (t->xs < t->ys)
				size.x *= t->xs / float(t->ys);
			else if (t->xs > t->ys)
				size.z *= t->ys / float(t->xs);
		}
		...
		loopv(texs) {
			...
			if (k.layer == LAYER_BLEND || k.alpha != NO_ALPHA)
				continue;
			...
			for (int j = 0; j < t.tris.length(); j += 3) {
				...
				if (tmin.x >= bbmax.x || tmin.y >= bbmax.y || tmin.z >= bbmax.z || tmax.x <= bbmin.x ||
					tmax.y <= bbmin.y || tmax.z <= bbmin.z)
					continue;
				float f0 = t0.norm.tonormal().dot(orient.b), f1 = t1.norm.tonormal().dot(orient.b),
				...
				bvec4 n0 = t0.norm, n1 = t1.norm, n2 = t2.norm, x0 = t0.tangent, x1 = t1.tangent, x2 = t2.tangent;
				...
				loopk(nump) {
					...
					float dp1 = ep.dot(e1), dp2 = ep.dot(e2), denom = d11 * d22 - d12 * d12,
						  b1 = (d22 * dp1 - d12 * dp2) / denom, b2 = (d11 * dp2 - d12 * dp1) / denom, b0 = 1 - b1 - b2;
	...
	void gendecals() {
		...
		loopv(extdecals) {
			...
			loopvj(oe->decals) {
				...
				if (!s.shader)
					continue;
				ushort envmap = s.shader->type & SHADER_ENVMAP
					? (s.texmask & (1 << TEX_ENVMAP) ? EMID_CUSTOM : closestenvmap(e.o))
					: EMID_NONE;
	...
	void setupdata(vtxarray* va) {
		...
		va->voffset = 0;
		if (va->verts) {
			if (vbosize[VBO_VBUF] + verts.length() > maxvbosize || vbosize[VBO_EBUF] + worldtris > USHRT_MAX ||
				vbosize[VBO_SKYBUF] + skytris > USHRT_MAX || vbosize[VBO_DECALBUF] + decaltris > USHRT_MAX)
				flushvbo();
		...
		if (va->sky) {
			auto* skydata = (ushort*)addvbo(va, VBO_SKYBUF, va->sky, sizeof(ushort));
			memcpy(skydata, skyindices.getbuf(), va->sky * sizeof(ushort));
			if (va->voffset)
				loopi(va->sky) skydata[i] += va->voffset;
		}
...
#define progress(s)                      \
	if ((recalcprogress++ & 0xFFF) == 0) \
		renderprogress(recalcprogress / (float)allocnodes, s);
			...
			if (ctj >= 0) {
				...
				while (ctj >= 0) {
					...
					if (v1.tangent.w != v2.tangent.w)
						vt.tangent.w = orientation_bitangent[vslot.rotation][orient].scalartriple(
										   vt.norm.tonormal(), vt.tangent.tonormal()) < 0
							? 0
							: 255;
...
static inline void calctexgen(VSlot& vslot, int orient, vec4& sgen, vec4& tgen) {
	...
	float k = TEX_SCALE / vslot.scale, xs = vslot.rotation >= 2 && vslot.rotation <= 4 ? -tex->xs : tex->xs,
		  ys = (vslot.rotation >= 1 && vslot.rotation <= 2) || vslot.rotation == 5 ? -tex->ys : tex->ys, sk = k / xs,
		  tk = k / ys, soff = -((vslot.rotation & 5) == 1 ? vslot.offset.y : vslot.offset.x) / xs,
...
auto encodenormal(const vec& n) -> ushort {
	if (n.iszero())
		return 0;
...
auto decodenormal(ushort norm) -> vec {
	if (!norm)
		return {0, 0, 1};
	norm--;
	const vec2 &yaw = sincos360[norm % 360], &pitch = sincos360[norm / 360 + 270];
	return {-yaw.y * pitch.x, yaw.x * pitch.x, pitch.y};
}
...
void addcubeverts(VSlot& vslot,
	...
	if (texture == DEFAULT_SKY)
		loopi(numverts) if (pos[i][orient >> 1] != ((orient & 1) << worldscale)) {
			loopk(numverts) {
				vc.skymin.min(pos[k]);
				vc.skymax.max(pos[k]);
			}
	...
	sortkey key(
		texture,
		vslot.scroll.iszero() ? O_ANY : orient,
		layer & LAYER_BOTTOM ? layer : LAYER_TOP,
		envmap,
		alpha ? (vslot.refractscale > 0 ? ALPHA_REFRACT : (vslot.alphaback ? ALPHA_BACK : ALPHA_FRONT)) : NO_ALPHA);
	...
	if (grassy) {
		for (int i = 0; i < numverts - 2; i += 2) {
			...
			if (index[0] != index[i + 1] && index[i + 1] != index[i + 2] && index[i + 2] != index[0])
				faces |= 1;
			if (i + 3 < numverts && index[0] != index[i + 2] && index[i + 2] != index[i + 3] &&
				index[i + 3] != index[0])
				faces |= 2;
			if (grassy > 1 && faces == 3)
...
static inline auto hthash(const edgegroup& g) -> uint {
	return g.slope.x ^ g.slope.y ^ g.slope.z ^ g.origin.x ^ g.origin.y ^ g.origin.z;
}

static inline auto htcmp(const edgegroup& x, const edgegroup& y) -> bool {
...
void gencubeedges(cube& c, const ivec& co, int size) {
	...
	loopi(6) if ((vis = visibletris(c, i, co, size))) {
		...
		loopj(numverts) {
			...
			int t1 = pos[e1][axis] / d[axis], t2 = pos[e2][axis] / d[axis];
			edgegroup g;
			...
			if (exists) {
				...
				while (cur >= 0) {
					...
					if (p.flags & CE_DUP ? ce.offset >= p.offset && ce.offset + ce.size <= p.offset + p.size
...
void gencubeverts(cube& c, const ivec& co, int size, int csi) {
	...
	loopi(6) if (vismask & (1 << i) && (vis = visibletris(c, i, co, size))) {
		...
		ushort envmap = vslot.slot->shader->type & SHADER_ENVMAP
			? (vslot.slot->texmask & (1 << TEX_ENVMAP) ? EMID_CUSTOM : closestenvmap(i, co, size))
			: EMID_NONE,
			   envmap2 = layer && layer->slot->shader->type & SHADER_ENVMAP
			? (layer->slot->texmask & (1 << TEX_ENVMAP) ? EMID_CUSTOM : closestenvmap(i, co, size))
			: EMID_NONE;
		while (tj >= 0 && tjoints[tj].edge < i * (MAXFACEVERTS + 1))
			tj = tjoints[tj].next;
		...
		if (!c.ext)
			...
			if (surf.numverts & LAYER_BOTTOM)
				addcubeverts(layer ? *layer : vslot,
							 i,
							 size,
							 pos,
							 convex,
							 ...
							 0,
							 false,
							 surf.numverts & LAYER_TOP ? LAYER_BOTTOM : LAYER_TOP);
		}
	}
}
...
auto newva(const ivec& o, int size) -> vtxarray* {
...
auto genmergedfaces(cube& c, const ivec& co, int size, int minlevel = -1) -> int {
	if (!c.ext || isempty(c))
		return -1;
	...
	loopi(6) if (c.merged & (1 << i)) {
		...
		if (level > minlevel) {
			...
			while (tj >= 0 && tjoints[tj].edge < i * (MAXFACEVERTS + 1))
				tj = tjoints[tj].next;
			if (tj >= 0 && tjoints[tj].edge < (i + 1) * (MAXFACEVERTS + 1))
				mf.tjoints = tj;
			...
			ushort envmap2 = layer && layer->slot->shader->type & SHADER_ENVMAP
				? (layer->slot->texmask & (1 << TEX_ENVMAP) ? EMID_CUSTOM : closestenvmap(i, co, size))
				: EMID_NONE;

			if (surf.numverts & LAYER_TOP)
...
auto findmergedfaces(cube& c, const ivec& co, int size, int csi, int minlevel) -> int {
...
void rendercube(cube& c,
				...
				int size,
				int csi,
				int& maxlevel)	// creates vertices and indices ready to be put into a va
{
	// if(size<=16) return;
	if (c.ext && c.ext->va) {
...
static inline auto setcubevisibility(cube& c, const ivec& co, int size) -> int {
	...
	loopi(6) {
		...
		if (facemask & 1) {
			vismask |= 1 << i;
			if (c.merged & (1 << i)) {
				if (c.ext && c.ext->surfaces[i].numverts & MAXFACEVERTS)
					numvis++;
			} else {
				numvis++;
				if (c.texture[i] != DEFAULT_SKY && !(c.ext && c.ext->surfaces[i].numverts & MAXFACEVERTS))
					checkmask |= 1 << i;
			}
		}
		if (facemask & 2 && collideface(c, i))
			collidemask |= 1 << i;
	}
...
auto updateva(cube* c, const ivec& co, int size, int csi) -> int {
	...
	loopi(8)  // counting number of semi-solid/solid children cubes
	{
		...
		if (c[i].ext && c[i].ext->va) {
			...
			if (tcount > vafacemax || (tcount >= vafacemin && size >= vacubesize) ||
...
void addtjoint(const edgegroup& g, const cubeedge& e, int offset) {
	...
	while (cur >= 0) {
		...
		if (tj.edge < o.edge ||
			(tj.edge == o.edge && (e.flags & CE_FLIP ? tj.offset > o.offset : tj.offset < o.offset)))
			break;
		prev = cur;
		cur = o.next;
	}
	...
	loopv(valist) {
		...
		loopj(va->texs + va->blends) {
			...
			if (texs.find(tex) < 0) {
				...
				if (vslot.layer && texs.find(vslot.layer) < 0)
					texs.add(vslot.layer);
				if (vslot.detail && texs.find(vslot.detail) < 0)
					texs.add(vslot.detail);
			}
		}
	}
	loopv(texs) {
