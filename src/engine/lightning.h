...
static void setuplightning() {
	if (!lastlnjitter || lastmillis - lastlnjitter > lnjittermillis) {
		if (!lastlnjitter)
			calclightningjitter(lnjitterframe);
...
static void renderlightning(Texture* tex, const vec& o, const vec& d, float sz) {
	...
	loopj(numsteps) {
		...
		if (j + 1 == numsteps)
			...
			next.add(
				vec(right).mul((jitter1 * lnjitterx[lnjitterframe][lj] + jitter0 * lnjitterx[lnjitterframe ^ 1][lj])));
			next.add(
				vec(up).mul((jitter1 * lnjittery[lnjitterframe][lj] + jitter0 * lnjittery[lnjitterframe ^ 1][lj])));
		}
...
struct lightningrenderer : listrenderer {
	lightningrenderer() : listrenderer("media/particle/lightning.jpg", 2, PT_LIGHTNING | PT_BRIGHT | PT_TRACK) {
	}
