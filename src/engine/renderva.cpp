...
static inline void drawvatris(vtxarray* va, GLsizei numindices, int offset) {
	drawtris(numindices, (ushort*)nullptr + va->eoffset + offset, va->minvert, va->maxvert);
}

static inline void drawvaskytris(vtxarray* va) {
	drawtris(va->sky, (ushort*)nullptr + va->skyoffset, va->minvert, va->maxvert);
}
...
auto isvisiblesphere(float rad, const vec& cv) -> int {
	...
	dist -= vfcDfog;
	if (dist > rad)
		return VFC_FOGGED;	// VFC_NOT_VISIBLE;    // culling when fog is closer than size of world results in HOM
	if (dist > -rad)
		v = VFC_PART_VISIBLE;
...
static inline auto ishiddencube(const ivec& o, int size) -> int {
	loopi(5) if (o.dist(vfcP[i]) < -vfcDfar[i] * size) return true;
	return false;
}

static inline auto isfoggedcube(const ivec& o, int size) -> int {
...
auto isvisiblecube(const ivec& o, int size) -> int {
	int v = VFC_FULL_VISIBLE;
	float dist;
...
auto isvisiblebb(const ivec& bo, const ivec& br) -> int {
	int v = VFC_FULL_VISIBLE;
	float dnear, dfar;
...
static inline auto vadist(vtxarray* va, const vec& p) -> float {
...
struct queryframe {
	...
	auto newquery(void* owner) -> occludequery* {
		if (cur >= max) {
			if (max >= MAXQUERY)
				return nullptr;
			if (deferquery) {
	...
	void reset() {
		loopi(max) queries[i].owner = nullptr;
	}
...
static inline auto querytarget() -> GLenum {
...
auto checkquery(occludequery* query, bool nowait) -> bool {
...
void startbb(bool mask) {
	...
	gle::bindvbo(bbvbo);
	gle::bindebo(bbebo);
	gle::vertexpointer(sizeof(vec), (const vec*)nullptr);
	gle::enablevertex();
	SETSHADER(bbquery);
	if (mask) {
...
void drawbb(const ivec& bo, const ivec& br) {
	...
	glDrawRangeElements_(GL_TRIANGLES, 0, 8 - 1, 3 * 2 * 6, GL_UNSIGNED_SHORT, (ushort*)nullptr);
	xtraverts += 8;
}
	...
	for (vtxarray* va = visibleva; va; va = va->next)
		if (va->occluded < OCCLUDE_BB && va->curvfc < VFC_FOGGED)
			loopv(va->mapmodels) {
				...
				if (isfoggedcube(oe->o, oe->size) || pvsoccluded(oe->bbmin, oe->bbmax))
					continue;
...
static inline auto bbinsideva(const ivec& bo, const ivec& br, vtxarray* va) -> bool {
	return bo.x >= va->bbmin.x && bo.y >= va->bbmin.y && bo.z >= va->bbmin.z && br.x <= va->bbmax.x &&
		br.y <= va->bbmax.y && br.z <= va->bbmax.z;
}

static inline auto bboccluded(const ivec& bo, const ivec& br, cube* c, const ivec& o, int size) -> bool {
	loopoctabox(o, size, bo, br) {
		ivec co(i, o, size);
		if (c[i].ext && c[i].ext->va) {
			vtxarray* va = c[i].ext->va;
			if (va->curvfc >= VFC_FOGGED || (va->occluded >= OCCLUDE_BB && bbinsideva(bo, br, va)))
				continue;
		}
...
auto bboccluded(const ivec& bo, const ivec& br) -> bool {
	...
	for (vtxarray* va = visibleva; va; va = va->next)
		if (va->occluded < OCCLUDE_BB) {
			...
			if (va->alphaback || va->alphafront || va->refract) {
				drawvatris(
					va, 3 * (va->alphabacktris + va->alphafronttris + va->refracttris), 3 * (va->tris + va->blendtris));
...
void renderblendbrush(GLuint tex, float x, float y, float w, float h) {
	...
	for (vtxarray* va = visibleva; va; va = va->next)
		if (va->texs && va->occluded < OCCLUDE_GEOM) {
			if (va->o.x + va->size <= x || va->o.y + va->size <= y || va->o.x >= x + w || va->o.y >= y + h)
				continue;
...
auto calcbbsidemask(const ivec& bbmin, const ivec& bbmax, const vec& lightpos, float lightradius, float bias) -> int {
	vec pmin = vec(bbmin).sub(lightpos).div(lightradius), pmax = vec(bbmax).sub(lightpos).div(lightradius);
	int mask = 0x3F;
	float dp1 = pmax.x + pmax.y, dn1 = pmax.x - pmin.y, ap1 = fabs(dp1), an1 = fabs(dn1), dp2 = pmin.x + pmin.y,
		  dn2 = pmin.x - pmax.y, ap2 = fabs(dp2), an2 = fabs(dn2);
	if (ap1 > bias * an1 && ap2 > bias * an2)
		mask &= (3 << 4) | (dp1 >= 0 ? (1 << 0) | (1 << 2) : (2 << 0) | (2 << 2)) |
			(dp2 >= 0 ? (1 << 0) | (1 << 2) : (2 << 0) | (2 << 2));
	if (an1 > bias * ap1 && an2 > bias * ap2)
		mask &= (3 << 4) | (dn1 >= 0 ? (1 << 0) | (2 << 2) : (2 << 0) | (1 << 2)) |
			(dn2 >= 0 ? (1 << 0) | (2 << 2) : (2 << 0) | (1 << 2));

	dp1 = pmax.y + pmax.z, dn1 = pmax.y - pmin.z, ap1 = fabs(dp1), an1 = fabs(dn1), dp2 = pmin.y + pmin.z,
	dn2 = pmin.y - pmax.z, ap2 = fabs(dp2), an2 = fabs(dn2);
	if (ap1 > bias * an1 && ap2 > bias * an2)
		mask &= (3 << 0) | (dp1 >= 0 ? (1 << 2) | (1 << 4) : (2 << 2) | (2 << 4)) |
			(dp2 >= 0 ? (1 << 2) | (1 << 4) : (2 << 2) | (2 << 4));
	if (an1 > bias * ap1 && an2 > bias * ap2)
		mask &= (3 << 0) | (dn1 >= 0 ? (1 << 2) | (2 << 4) : (2 << 2) | (1 << 4)) |
			(dn2 >= 0 ? (1 << 2) | (2 << 4) : (2 << 2) | (1 << 4));

	dp1 = pmax.z + pmax.x, dn1 = pmax.z - pmin.x, ap1 = fabs(dp1), an1 = fabs(dn1), dp2 = pmin.z + pmin.x,
	dn2 = pmin.z - pmax.x, ap2 = fabs(dp2), an2 = fabs(dn2);
	if (ap1 > bias * an1 && ap2 > bias * an2)
		mask &= (3 << 2) | (dp1 >= 0 ? (1 << 4) | (1 << 0) : (2 << 4) | (2 << 0)) |
			(dp2 >= 0 ? (1 << 4) | (1 << 0) : (2 << 4) | (2 << 0));
	if (an1 > bias * ap1 && an2 > bias * ap2)
		mask &= (3 << 2) | (dn1 >= 0 ? (1 << 4) | (2 << 0) : (2 << 4) | (1 << 0)) |
			(dn2 >= 0 ? (1 << 4) | (2 << 0) : (2 << 4) | (1 << 0));

	return mask;
}

auto calcspheresidemask(const vec& p, float radius, float bias) -> int {
	// p is in the cubemap's local coordinate system
	// bias = border/(size - border)
	float dxyp = p.x + p.y, dxyn = p.x - p.y, axyp = fabs(dxyp), axyn = fabs(dxyn), dyzp = p.y + p.z, dyzn = p.y - p.z,
		  ayzp = fabs(dyzp), ayzn = fabs(dyzn), dzxp = p.z + p.x, dzxn = p.z - p.x, azxp = fabs(dzxp),
		  azxn = fabs(dzxn);
	int mask = 0x3F;
	radius *= SQRT2;
...
auto calctrisidemask(const vec& p1, const vec& p2, const vec& p3, float bias) -> int {
	// p1, p2, p3 are in the cubemap's local coordinate system
	// bias = border/(size - border)
	int mask = 0x3F;
	float dp1 = p1.x + p1.y, dn1 = p1.x - p1.y, ap1 = fabs(dp1), an1 = fabs(dn1), dp2 = p2.x + p2.y, dn2 = p2.x - p2.y,
		  ap2 = fabs(dp2), an2 = fabs(dn2), dp3 = p3.x + p3.y, dn3 = p3.x - p3.y, ap3 = fabs(dp3), an3 = fabs(dn3);
	...
	dp1 = p1.y + p1.z, dn1 = p1.y - p1.z, ap1 = fabs(dp1), an1 = fabs(dn1), dp2 = p2.y + p2.z, dn2 = p2.y - p2.z,
	ap2 = fabs(dp2), an2 = fabs(dn2), dp3 = p3.y + p3.z, dn3 = p3.y - p3.z, ap3 = fabs(dp3), an3 = fabs(dn3);
	if (ap1 > bias * an1 && ap2 > bias * an2 && ap3 > bias * an3)
		mask &= (3 << 0) | (dp1 >= 0 ? (1 << 2) | (1 << 4) : (2 << 2) | (2 << 4)) |
			(dp2 >= 0 ? (1 << 2) | (1 << 4) : (2 << 2) | (2 << 4)) |
	...
	dp1 = p1.z + p1.x, dn1 = p1.z - p1.x, ap1 = fabs(dp1), an1 = fabs(dn1), dp2 = p2.z + p2.x, dn2 = p2.z - p2.x,
	ap2 = fabs(dp2), an2 = fabs(dn2), dp3 = p3.z + p3.x, dn3 = p3.z - p3.x, ap3 = fabs(dp3), an3 = fabs(dn3);
	if (ap1 > bias * an1 && ap2 > bias * an2 && ap3 > bias * an3)
		mask &= (3 << 2) | (dp1 >= 0 ? (1 << 4) | (1 << 0) : (2 << 4) | (2 << 0)) |
			(dp2 >= 0 ? (1 << 4) | (1 << 0) : (2 << 4) | (2 << 0)) |
...
auto cullfrustumsides(const vec& lightpos, float lightradius, float size, float border) -> int {
	...
	// this next test usually clips off more sides than the former, but occasionally clips fewer/different ones, so do
	// both and combine results check if frustum corners/origin cross plane sides infinite version, assumes frustum
	// corners merely give direction and extend to infinite distance
	...
	loopv(vas) {
		...
		if (dist < shadowradius || !smdistcull) {
			v.shadowmask = !smbbcull
				? 0x3F
				: (v.children.length() || v.mapmodels.length()
		...
		if (dist < shadowradius || !smdistcull) {
			v.shadowmask = !smbbcull ||
					(v.children.length() || v.mapmodels.length()
						 ...
						 : bbinsidespot(shadoworigin, shadowdir, shadowspot, v.geommin, v.geommax))
				? 1
				: 0;
			addshadowva(&v, dist);
			if (v.children.length())
...
struct renderstate {
	bool colormask{true}, depthmask{true};
	int alphaing{0};
	GLuint vbuf{0};
	bool vattribs{false}, vquery{false};
	vec colorscale;
	float alphascale{0};
	float refractscale{0};
	vec refractcolor;
	bool blend{false};
	int blendx{-1}, blendy{-1};
	int globals{-1}, tmu{-1};
	GLuint textures[7];
	Slot *slot{NULL}, *texgenslot{NULL};
	VSlot *vslot{NULL}, *texgenvslot{NULL};
	vec2 texgenscroll;
	int texgenorient{-1}, texgenmillis;
...
struct geombatch {
	...
	[[nodiscard]] auto compare(const geombatch& b) const -> int {
		...
		if (es.layer & LAYER_BOTTOM) {
			...
			if (y1 > y2)
				return 1;
		} else if (b.es.layer & LAYER_BOTTOM)
			return -1;
...
static void changeslottmus(renderstate& cur, int pass, Slot& slot, VSlot& vslot) {
	...
	if (cur.alphaing) {
		...
		changecolorparams:
			...
			GLOBALPARAMF(
				colorparams, alpha * vslot.colorscale.x, alpha * vslot.colorscale.y, alpha * vslot.colorscale.z, alpha);
		}
...
static void changetexgen(renderstate& cur, int orient, Slot& slot, VSlot& vslot) {
	if (cur.texgenslot != &slot || cur.texgenvslot != &vslot) {
		...
		if (!cur.texgenvslot || slot.sts.empty() ||
			(curtex->xs != tex->xs || curtex->ys != tex->ys || cur.texgenvslot->rotation != vslot.rotation ||
			 cur.texgenvslot->scale != vslot.scale || cur.texgenvslot->offset != vslot.offset ||
			 cur.texgenvslot->scroll != vslot.scroll)) {
...
static void renderbatch(renderstate& cur, int pass, geombatch& b) {
	...
	for (geombatch* curbatch = &b;; curbatch = &geombatches[curbatch->batch]) {
		...
		if (len) {
			drawtris(
				len, (ushort*)nullptr + curbatch->va->eoffset + curbatch->offset, curbatch->es.minvert, curbatch->es.maxvert);
			vtris += len / 3;
		}
		if (curbatch->batch < 0)
			break;
...
static void renderbatches(renderstate& cur, int pass) {
	...
	while (curbatch >= 0) {
		...
		if (cur.vslot != &b.vslot) {
			...
			if (cur.texgenorient != b.es.orient || (cur.texgenorient < O_ANY && cur.texgenvslot != &b.vslot))
...
void renderva(renderstate& cur, vtxarray* va, int pass = RENDERPASS_GBUFFER, bool doquery = false) {
	...
	case RENDERPASS_RSM_BLEND:
		mergetexs(cur, va, &va->texelems[va->texs], va->blends, 3 * va->tris);
		if (!batchgeom && geombatches.length())
			renderbatches(cur, RENDERPASS_RSM);
		break;
	}
}
...
void rendergeom() {
	...
	if (blends) {
		...
		cur.texgenorient = -1;
		for (vtxarray* va = visibleva; va; va = va->next)
			if (va->blends && va->occluded < OCCLUDE_GEOM && va->curvfc != VFC_FOGGED) {
...
auto dynamicshadowvabounds(int mask, vec& bbmin, vec& bbmax) -> int {
	int vis = 0;
	for (vtxarray* va = shadowva; va; va = va->rnext)
...
float alphafrontsx1 = -1, alphafrontsx2 = 1, alphafrontsy1 = -1, alphafrontsy2 = -1, alphabacksx1 = -1,
	  alphabacksx2 = 1, alphabacksy1 = -1, alphabacksy2 = -1, alpharefractsx1 = -1, alpharefractsx2 = 1,
	  alpharefractsy1 = -1, alpharefractsy2 = 1;
uint alphatiles[LIGHTTILE_MAXH];

auto findalphavas() -> int {
	alphavas.setsize(0);
	...
	for (vtxarray* va = visibleva; va; va = va->next)
		if (va->alphabacktris || va->alphafronttris || va->refracttris) {
			if (va->occluded >= OCCLUDE_BB)
				continue;
			if (va->occluded >= OCCLUDE_GEOM && pvsoccluded(va->alphamin, va->alphamax))
				continue;
			if (va->curvfc == VFC_FOGGED)
				continue;
...
auto renderexplicitsky(bool outline) -> bool {
	vtxarray* prev = nullptr;
	for (vtxarray* va = visibleva; va; va = va->next)
...
struct decalrenderer {
	...
	decalrenderer() :  colorscale(1, 1, 1) {
...
struct decalbatch {
	...
	[[nodiscard]] auto compare(const decalbatch& b) const -> int {
		if (va->vbuf < b.va->vbuf)
			return -1;
		if (va->vbuf > b.va->vbuf)
			return 1;
...
static void renderdecalbatch(decalrenderer& cur, int pass, decalbatch& b) {
	...
	for (decalbatch* curbatch = &b;; curbatch = &decalbatches[curbatch->batch]) {
		...
		if (len) {
			drawtris(len,
					 (ushort*)nullptr + curbatch->va->decaloffset + curbatch->offset,
					 curbatch->es.minvert,
					 curbatch->es.maxvert);
			vtris += len / 3;
		}
...
static void flushshadowmeshdraws(shadowmesh& m, int sides, shadowdrawinfo draws[6]) {
	...
	auto* indexes = new ushort[numindexes];
	int offset = 0;
	...
	gle::bindvbo(vbuf);
	glBufferData_(
		GL_ARRAY_BUFFER, shadowverts.verts.length() * sizeof(vec), shadowverts.verts.getbuf(), GL_STATIC_DRAW);
	gle::clearvbo();
	shadowverts.clear();
...
static inline void addshadowmeshtri(shadowmesh& m,
									int sides,
	...
	if (l0.squaredlen() > shadowradius * shadowradius && l1.squaredlen() > shadowradius * shadowradius &&
		l2.squaredlen() > shadowradius * shadowradius)
		return;
	int sidemask = 0;
	switch (m.type) {
	case SM_SPOT:
		sidemask = bbinsidespot(shadoworigin,
								shadowdir,
								shadowspot,
								ivec(vec(v0).min(v1).min(v2)),
								ivec(vec(v0).max(v1).max(v2).add(1)))
			? 1
			: 0;
		break;
	case SM_CUBEMAP:
...
static void genshadowmeshtris(shadowmesh& m,
							  int sides,
							  shadowdrawinfo draws[6],
							  ushort* edata,
							  int numtris,
...
static void genshadowmeshmapmodels(shadowmesh& m, int sides, shadowdrawinfo draws[6]) {
	...
	for (octaentities* oe = shadowmms; oe; oe = oe->rnext)
		loopvk(oe->mapmodels) {
			extentity& e = *ents[oe->mapmodels[k]];
			if (e.flags & (EF_NOVIS | EF_NOSHADOW))
				continue;
			e.flags |= EF_RENDER;
		}
...
auto findshadowmesh(int idx, extentity& e) -> shadowmesh* {
	shadowmesh* m = shadowmeshes.access(idx);
	if (!m || m->type != shadowmapping || m->origin != shadoworigin || m->radius < shadowradius)
		return nullptr;
	switch (m->type) {
	case SM_SPOT:
		if (!e.attached || e.attached->type != ET_SPOTLIGHT || m->spotloc != e.attached->o ||
...
void rendershadowmesh(shadowmesh* m) {
	...
	while (draw >= 0) {
		...
		drawtris(3 * d.tris, (ushort*)nullptr + d.offset, d.minvert, d.maxvert);
