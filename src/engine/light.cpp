...
static const surfaceinfo brightsurfaces[6] =
	{brightsurface, brightsurface, brightsurface, brightsurface, brightsurface, brightsurface};
...
auto PackNode::insert(ushort& tx, ushort& ty, ushort tw, ushort th) -> bool {
	if ((available < tw && available < th) || w < tw || h < th)
		return false;
	if (child1) {
		bool inserted = child1->insert(tx, ty, tw, th) || child2->insert(tx, ty, tw, th);
...
void PackNode::reserve(ushort tx, ushort ty, ushort tw, ushort th) {
	...
	int dx1 = tx - x, dx2 = x + w - tx - tw, dx = max(dx1, dx2), dy1 = ty - y, dy2 = y + h - ty - th,
		dy = max(dy1, dy2), split;
	if (dx > dy) {
		if (dx1 > dx2)
			split = min(dx1, int(w));
		else
...
void clearlightcache(int id) {
	if (id >= 0) {
		...
		if (radius <= 0)
			return;
		for (int x = int(max(light.o.x - radius, 0.0f)) >> lightcachesize,
				 ex = int(min(light.o.x + radius, worldsize - 1.0f)) >> lightcachesize;
			 x <= ex;
			 x++)
			for (int y = int(max(light.o.y - radius, 0.0f)) >> lightcachesize,
					 ey = int(min(light.o.y + radius, worldsize - 1.0f)) >> lightcachesize;
				 y <= ey;
				 y++) {
...
auto checklightcache(int x, int y) -> const vector<int>& {
	x >>= lightcachesize;
	y >>= lightcachesize;
	lightcacheentry& lce = lightcache[LIGHTCACHEHASH(x, y)];
	if (lce.x == x && lce.y == y)
	...
	loopv(ents) {
		...
		case ET_LIGHT: {
			int radius = light.attr1;
			if (radius <= 0 || light.o.x + radius < cx || light.o.x - radius > cx + csize || light.o.y + radius < cy ||
				light.o.y - radius > cy + csize)
				continue;
			break;
		}
		default:
...
static void calcsurfaces(cube& c, const ivec& co, int size, int usefacemask, int preview = 0) {
	...
	loopi(6) {
		...
		if (numverts < 4 || !convex)
			loopk(numverts) findnormal(pos[k], smooth, planes[0], n[k]);
		else {
...
static void calcsurfaces(cube* c, const ivec& co, int size) {
	...
	loopi(8) {
		...
		if (c[i].children)
			...
			int usefacemask = 0;
			loopj(6) if (c[i].texture[j] != DEFAULT_SKY &&
						 (!(c[i].merged & (1 << j)) || (c[i].ext && c[i].ext->surfaces[j].numverts & MAXFACEVERTS))) {
...
static inline auto previewblends(cube& c, const ivec& o, int size) -> bool {
	if (isempty(c) || c.material & MAT_ALPHA)
		return false;
	int usefacemask = 0;
	loopj(6) if (c.texture[j] != DEFAULT_SKY && lookupvslot(c.texture[j], false).layer) usefacemask |=
		visibletris(c, j, o, size) << (4 * j);
	if (!usefacemask)
		return false;
...
static auto previewblends(cube* c, const ivec& co, int size, const ivec& bo, const ivec& bs) -> bool {
	bool changed = false;
...
static auto calclighttimer(Uint32 interval, void* param) -> Uint32 {
