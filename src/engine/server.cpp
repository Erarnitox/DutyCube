...
void fatal(const char* fmt, ...) {
	...
	if (logfile)
		logoutf("%s", msg);
#ifdef WIN32
	MessageBox(NULL, msg, "DropEngine fatal error", MB_OK | MB_SYSTEMMODAL);
#else
...
auto hasnonlocalclients() -> bool {
	return nonlocalclients != 0;
}
auto haslocalclients() -> bool {
	return localclients != 0;
}

auto addclient(int type) -> client& {
	client* c = nullptr;
...
auto getservermtu() -> int {
	return serverhost ? serverhost->mtu : -1;
}
auto getclientinfo(int i) -> void* {
	return !clients.inrange(i) || clients[i]->type == ST_EMPTY ? nullptr : clients[i]->info;
}
auto getclientpeer(int i) -> ENetPeer* {
	return clients.inrange(i) && clients[i]->type == ST_TCPIP ? clients[i]->peer : nullptr;
}
auto getnumclients() -> int {
	return clients.length();
}
auto getclientip(int n) -> uint {
...
auto sendf(int cn, int chan, const char* format, ...) -> ENetPacket* {
	int exclude = -1;
	bool reliable = false;
	if (*format == 'r') {
		reliable = true;
...
auto sendfile(int cn, int chan, stream* file, const char* format, ...) -> ENetPacket* {
	if (cn < 0) {
#ifdef STANDALONE
		return nullptr;
#endif
...
auto disconnectreason(int reason) -> const char* {
	switch (reason) {
	case DISC_EOP:
...
#ifdef STANDALONE
auto resolverwait(const char* name, ENetAddress* address) -> bool {
	return enet_address_set_host(address, name) >= 0;
}

auto connectwithtimeout(ENetSocket sock, const char* hostname, const ENetAddress& remoteaddress) -> int {
...
auto connectmaster(bool wait) -> ENetSocket {
	if (!mastername[0])
...
auto requestmaster(const char* req) -> bool {
	if (mastersock == ENET_SOCKET_NULL) {
...
auto requestmasterf(const char* fmt, ...) -> bool {
...
static auto serverinfointercept(ENetHost* host, ENetEvent* event) -> int {
	if (host->receivedDataLength < 2 || host->receivedData[0] != 0xFF || host->receivedData[1] != 0xFF ||
		host->receivedDataLength - 2 > MAXPINGDATA)
		return 0;
	serverinfoaddress = host->receivedAddress;
	ucharbuf req(host->receivedData + 2, host->receivedDataLength - 2),
		p(host->receivedData + 2, sizeof(host->packetData[0]) - 2);
...
void serverslice(bool dedicated,
				 uint timeout)	// main server update, called from main loop in sp, or from below in dedicated server
{
	if (!serverhost) {
		server::serverupdate();
		server::sendpackets();
	...
	if (!lastupdatemaster ||
		totalmillis - lastupdatemaster > 60 * 60 * 1000)  // send alive signal to masterserver every hour of uptime
		updatemasterserver();
	...
	if (totalmillis - laststatus > 60 * 1000)  // display bandwidth stats, useful for server ops
	{
		...
		if (nonlocalclients || serverhost->totalSentData || serverhost->totalReceivedData)
			logoutf("status: %d remote clients, %.1f send, %.1f rec (K/sec)",
					nonlocalclients,
...
enum { MENU_OPENCONSOLE = 0, MENU_SHOWCONSOLE, MENU_HIDECONSOLE, MENU_EXIT };
...
static void setupwindow(const char* title) {
	...
	appicon = LoadIcon(
		appinstance,
		MAKEINTRESOURCE(
			IDI_ICON1));  //(HICON)LoadImage(appinstance, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 0, 0, LR_DEFAULTSIZE);
	if (!appicon)
	...
	appwindow = CreateWindow(
		MAKEINTATOM(wndclass), title, 0, CW_USEDEFAULT, CW_USEDEFAULT, 0, 0, HWND_MESSAGE, NULL, appinstance, NULL);
	if (!appwindow)
...
auto servererror(bool dedicated, const char* desc) -> bool {
#ifndef STANDALONE
	if (!dedicated) {
...
auto setuplistenserver(bool dedicated) -> bool {
	...
	if (*serverip) {
		if (enet_address_set_host(&address, serverip) < 0)
			conoutf(CON_WARN, "WARNING: server ip not resolved");
		else
			serveraddress.host = address.host;
	}
	serverhost = enet_host_create(
		&address, min(maxclients + server::reserveclients(), MAXCLIENTS), server::numchannels(), 0, serveruprate);
	if (!serverhost)
	...
	if (lansock == ENET_SOCKET_NULL)
		conoutf(CON_WARN, "WARNING: could not create LAN server info socket");
	else
...
void startlistenserver(int* usemaster) {
	if (serverhost) {
		conoutf(CON_ERROR, "listen server is already running");
		return;
	}
	...
	conoutf("listen server started for %d clients%s",
			maxclients,
			allowupdatemaster ? " and listed with master server" : "");
}
COMMAND(startlistenserver, "i");

void stoplistenserver() {
	if (!serverhost) {
		conoutf(CON_ERROR, "listen server is not running");
		return;
	}
...
auto serveroption(char* opt) -> bool {
	switch (opt[1]) {
#ifdef STANDALONE
	case 'u':
		logoutf("Using home directory: %s", opt);
		sethomedir(opt + 2);
		return true;
	case 'k':
		logoutf("Adding package directory: %s", opt);
		addpackagedir(opt + 2);
		return true;
	case 'g':
		logoutf("Setting log file: %s", opt);
		setlogfile(opt + 2);
		return true;
#endif
	default:
