...
auto BIH::triintersect(const mesh& m, int tidx, const vec& mo, const vec& mray, float maxdist, float& dist, int mode)
	-> bool {
	const tri& t = m.tris[tidx];
	vec a = m.getpos(t.vert[0]), b = m.getpos(t.vert[1]).sub(a), c = m.getpos(t.vert[2]).sub(a), n = vec().cross(b, c),
		r = vec(a).sub(mo), e = vec().cross(r, mray);
	...
	if (m.flags & MESH_ALPHA && (mode & RAY_ALPHAPOLY) == RAY_ALPHAPOLY && (m.tex->alphamask || loadalphamask(m.tex))) {
		vec2 at = m.gettc(t.vert[0]), bt = m.gettc(t.vert[1]).sub(at).mul(v * invdet),
			 ct = m.gettc(t.vert[2]).sub(at).mul(w * invdet);
		at.add(bt).add(ct);
		int si = clamp(int(m.tex->xs * at.x), 0, m.tex->xs - 1), ti = clamp(int(m.tex->ys * at.y), 0, m.tex->ys - 1);
...
inline auto BIH::traverse(const mesh& m,
						  const vec& o,
						  const vec& ray,
						  const vec& invray,
						  float maxdist,
...
inline auto BIH::traverse(const vec& o, const vec& ray, float maxdist, float& dist, int mode) -> bool {
	...
	loopi(nummeshes) {
		...
		if (!(m.flags & MESH_RENDER) || (!(mode & RAY_SHADOW) && m.flags & MESH_NOCLIP))
			continue;
		float t1 = (m.bbmin.x - o.x) * invray.x, t2 = (m.bbmax.x - o.x) * invray.x, tmin, tmax;
		if (invray.x > 0) {
			tmin = t1;
			tmax = t2;
		} else {
...
void BIH::build(mesh& m, ushort* indices, int numindices, const ivec& vmin, const ivec& vmax) {
	...
	loopk(3) {
		...
		for (left = 0, right = numindices, splitleft = SHRT_MIN, splitright = SHRT_MAX; left < right;) {
			...
			ivec trimin = ivec(tri.center).sub(ivec(tri.radius)), trimax = ivec(tri.center).add(ivec(tri.radius));
...
BIH::BIH(vector<mesh>& buildmeshes)
	...
	loopi(nummeshes) {
		...
		loopj(m.numtris) {
			vec s0 = m.getpos(srctri->vert[0]), s1 = m.getpos(srctri->vert[1]), s2 = m.getpos(srctri->vert[2]),
				...
				vmin = vec(v0).min(v1).min(v2), vmax = vec(v0).max(v1).max(v2);
			mmin.min(vmin);
			mmax.max(vmax);
	...
	node* curnode = nodes;
	auto* indices = new ushort[numtris];
	loopi(nummeshes) {
		mesh& m = meshes[i];
		m.nodes = curnode;
...
auto mmintersect(const extentity& e, const vec& o, const vec& ray, float maxdist, int mode, float& dist) -> bool {
	model* m = loadmapmodel(e.attr1);
	if (!m)
		return false;
	if (mode & RAY_SHADOW) {
		if (!m->shadow || e.flags & EF_NOSHADOW)
			return false;
	} else if ((mode & RAY_ENTS) != RAY_ENTS && (!m->collide || e.flags & EF_NOCOLLIDE))
		return false;
	if (!m->bih && !m->setBIH())
		return false;
...
static inline auto segmentdistance(const vec& d1, const vec& d2, const vec& r) -> float {
	...
	vec c1 = vec(d1).mul(s), c2 = vec(d2).mul(t);
	return vec(c2).sub(c1).add(r).squaredlen();
}

static inline auto trisegmentdistance(const vec& a, const vec& b, const vec& c, const vec& p, const vec& q) -> float {
	vec pq = vec(q).sub(p), ab = vec(b).sub(a), bc = vec(c).sub(b), ca = vec(a).sub(c), ap = vec(p).sub(a),
		bp = vec(p).sub(b), cp = vec(p).sub(c), aq = vec(q).sub(a), bq = vec(q).sub(b), n, nab, nbc, nca;
	n.cross(ab, bc);
	nab.cross(n, ab);
	nbc.cross(n, bc);
	nca.cross(n, ca);
...
static inline auto triboxoverlap(const vec& radius, const vec& a, const vec& b, const vec& c) -> bool {
	vec ab = vec(b).sub(a), bc = vec(c).sub(b), ca = vec(a).sub(c);

#define TESTAXIS(v0, v1, v2, e, s, t)                                     \
	{                                                                     \
		float p = v0.s * v1.t - v0.t * v1.s, q = v2.s * e.t - v2.t * e.s, \
...
template <>
inline void BIH::tricollide<COLLIDE_ELLIPSE>(const mesh& m,
											 int tidx,
											 physent* d,
											 const vec& dir,
											 float cutoff,
	...
	if (trisegmentdistance(a, b, c, vec(center).sub(zdir), vec(center).add(zdir)) >
		m.invscale * m.invscale * radius.x * radius.x)
		return;
	...
	if (!dir.iszero()) {
		...
		if (d->type == ENT_PLAYER &&
			pdist < (dir.z * n.z < 0 ? 2 * radius.z *
							 (d->zmargin / (d->aboveeye + d->eyeheight) - (dir.z < 0 ? 1 / 3.0f : 1 / 4.0f))
...
template <>
inline void BIH::tricollide<COLLIDE_OBB>(const mesh& m,
										 int tidx,
										 physent* d,
										 const vec& dir,
										 float cutoff,
	...
	const tri& t = m.tris[tidx];
	vec a = orient.transform(m.getpos(t.vert[0])), b = orient.transform(m.getpos(t.vert[1])),
...
template <int C>
inline void BIH::collide(const mesh& m,
						 physent* d,
						 const vec& dir,
						 float cutoff,
						 const vec& center,
	...
	for (;;) {
		...
		int nearsplit = bmin[axis] - curnode->split[nearidx], farsplit = curnode->split[faridx] - bmax[axis];
				...
				if (!curnode->isleaf(faridx)) {
					if (stacksize < int(sizeof(stack) / sizeof(stack[0]))) {
						...
						collide<C>(m,
								   ...
								   orient,
								   dist,
								   &nodes[curnode->childindex(nearidx)],
								   bo,
								   br);
						curnode += curnode->childindex(faridx);
						continue;
...
auto BIH::ellipsecollide(physent* d,
						 const vec& dir,
						 float cutoff,
						 const vec& o,
						 int yaw,
	...
	if (bo.x + br.x < bbmin.x || bo.y + br.y < bbmin.y || bo.z + br.z < bbmin.z || bo.x - br.x > bbmax.x ||
		bo.y - br.y > bbmax.y || bo.z - br.z > bbmax.z)
		return false;

	ivec imin = ivec::floor(vec(bo).sub(br)), imax = ivec::ceil(vec(bo).add(br)), icenter = ivec(imin).add(imax).div(2),
	...
	loopi(nummeshes) {
		...
		collide<COLLIDE_ELLIPSE>(
			m, d, dir, cutoff, m.invxform.transform(bo), radius, morient, dist, m.nodes, icenter, iradius);
	}
	return dist > -1e9f;
}

auto BIH::boxcollide(physent* d, const vec& dir, float cutoff, const vec& o, int yaw, int pitch, int roll, float scale)
	-> bool {
	if (!numnodes)
		return false;
...
inline void BIH::genstaintris(stainrenderer* s,
							  const mesh& m,
							  int tidx,
							  const vec& center,
							  float radius,
	...
	const tri& t = m.tris[tidx];
	vec v[3] = {orient.transform(m.getpos(t.vert[0])),
...
void BIH::genstaintris(stainrenderer* s,
					   const mesh& m,
					   const vec& center,
					   float radius,
					   const matrix4x3& orient,
...
void BIH::genstaintris(stainrenderer* s,
	...
	vec bo = orient.transposedtransform(center);
	if (bo.x + radius < bbmin.x || bo.y + radius < bbmin.y || bo.z + radius < bbmin.z || bo.x - radius > bbmax.x ||
		bo.y - radius > bbmax.y || bo.z - radius > bbmax.z)
		return;

	orient.scale(scale);

	ivec imin = ivec::floor(vec(bo).sub(radius)), imax = ivec::ceil(vec(bo).add(radius)),
		 icenter = ivec(imin).add(imax).div(2), iradius = ivec(imax).sub(imin).add(1).div(2);
