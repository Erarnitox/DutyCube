...
bool hdrfloat = false;
GLuint msfbo = 0, msdepthtex = 0, mscolortex = 0, msnormaltex = 0, msglowtex = 0, msdepthrb = 0, msstencilrb = 0,
	   mshdrfbo = 0, mshdrtex = 0, msrefractfbo = 0, msrefracttex = 0;
...
auto gethdrformat(int prec, int fallback = GL_RGB) -> int {
	if (prec >= 3 && hasTF)
		return GL_RGB16F;
	if (prec >= 2 && hasPF && !amd_pf_bug)
...
void setupbloom(int w, int h) {
	...
	if (hwvtexunits < 4) {
		...
		glBufferData_(GL_PIXEL_PACK_BUFFER,
					  4 * (hasTF ? sizeof(GLfloat) : sizeof(GLushort)) * (hasTRG ? 1 : 3),
					  nullptr,
					  GL_DYNAMIC_COPY);
	...
	static const float grayf[12] = {
		0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f, 0.125f};
	createtexture(bloomtex[4],
				  bloompbo ? 4 : 1,
				  1,
				  hasTF ? (const void*)grayf : (const void*)gray,
				  3,
				  1,
				  hasTF ? (hasTRG ? GL_R16F : GL_RGB16F) : (hasTRG ? GL_R16 : GL_RGB16));

	loopi(5 + (bloomformat != GL_RGB ? 1 : 0)) {
		glBindFramebuffer_(GL_FRAMEBUFFER, bloomfbo[i]);
		glFramebufferTexture2D_(
			GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, i == 4 ? GL_TEXTURE_2D : GL_TEXTURE_RECTANGLE, bloomtex[i], 0);
...
void cleanupbloom() {
	...
	loopi(6) if (bloomfbo[i]) {
		glDeleteFramebuffers_(1, &bloomfbo[i]);
		bloomfbo[i] = 0;
	}
	loopi(6) if (bloomtex[i]) {
...
extern int ao, aotaps, aoreduce, aoreducedepth, aonoise, aobilateral, aobilateralupscale, aopackdepth, aodepthformat,
	aoprec, aoderivnormal;

static Shader* bilateralshader[2] = {nullptr, nullptr};

auto loadbilateralshader(int pass) -> Shader* {
	if (!aobilateral)
		return nullshader;
...
auto loadambientobscuranceshader() -> Shader* {
	string opts;
	int optslen = 0;
...
void setupao(int w, int h) {
	...
	loopi(upscale ? 3 : 2) {
		...
		createtexture(aotex[i],
					  upscale && i ? w : aow,
					  upscale && i >= 2 ? h : aoh,
					  nullptr,
					  3,
					  i < 2 ? packfilter : 1,
					  i < 2 ? packformat : format,
	...
	if (aoreducedepth && (aoreduce || aoreducedepth > 1)) {
		...
		createtexture(aotex[3],
					  aow,
					  aoh,
					  nullptr,
					  3,
					  0,
					  aodepthformat > 1 ? GL_R32F : (aodepthformat ? GL_R16F : GL_RGBA8),
					  GL_TEXTURE_RECTANGLE);
...
void renderao() {
	...
	LOCALPARAMF(tapparams,
				aoradius * eyematrix.d.z / xscale,
				aoradius * eyematrix.d.z / yscale,
				aoradius * aoradius * aocutoff * aocutoff);
	LOCALPARAMF(contrastparams, (2.0f * aodark) / aotaps, aosharp);
	LOCALPARAMF(offsetscale,
				xscale / eyematrix.d.z,
				yscale / eyematrix.d.z,
				eyematrix.d.x / eyematrix.d.z,
				eyematrix.d.y / eyematrix.d.z);
...
void cleanupscale() {
	loopi(2) if (scalefbo[i]) {
		glDeleteFramebuffers_(1, &scalefbo[i]);
		scalefbo[i] = 0;
	}
	loopi(2) if (scaletex[i]) {
...
void setupscale(int sw, int sh, int w, int h) {
	...
	loopi(gscalecubic ? 2 : 1) {
		...
		createtexture(
			scaletex[i], sw, i ? h : sh, nullptr, 3, gscalecubic || !gscalenearest ? 1 : 0, GL_RGB, GL_TEXTURE_RECTANGLE);
...
void initgbuffer() {
	...
	if (lineardepth > 1 && (!hasAFBO || !hasTF || !hasTRG))
		gdepthformat = 1;
	else
...
void cleanupmsbuffer() {
	if (msfbo) {
		glDeleteFramebuffers_(1, &msfbo);
		msfbo = 0;
	}
	if (msdepthtex) {
		glDeleteTextures(1, &msdepthtex);
		msdepthtex = 0;
	}
	if (mscolortex) {
	...
	if (msnormaltex) {
		glDeleteTextures(1, &msnormaltex);
		msnormaltex = 0;
	}
	if (msglowtex) {
		glDeleteTextures(1, &msglowtex);
		msglowtex = 0;
	}
	if (msstencilrb) {
		glDeleteRenderbuffers_(1, &msstencilrb);
		msstencilrb = 0;
	}
	if (msdepthrb) {
		glDeleteRenderbuffers_(1, &msdepthrb);
		msdepthrb = 0;
	}
	if (mshdrfbo) {
		glDeleteFramebuffers_(1, &mshdrfbo);
		mshdrfbo = 0;
	}
	if (mshdrtex) {
		glDeleteTextures(1, &mshdrtex);
		mshdrtex = 0;
	}
	if (msrefractfbo) {
		glDeleteFramebuffers_(1, &msrefractfbo);
		msrefractfbo = 0;
	}
	if (msrefracttex) {
		glDeleteTextures(1, &msrefracttex);
		msrefracttex = 0;
	}
}
...
void setupmsbuffer(int w, int h) {
	...
	if (gdepthformat) {
		...
		glRenderbufferStorageMultisample_(
			GL_RENDERBUFFER, msaasamples, ghasstencil > 1 ? stencilformat : GL_DEPTH_COMPONENT24, w, h);
	...
	GLenum depthformat =
		gdepthformat ? depthformats[gdepthformat - 1] : (ghasstencil > 1 ? stencilformat : GL_DEPTH_COMPONENT24);
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, msdepthtex);
...
void cleanupgbuffer() {
	...
	if (gglowtex) {
		glDeleteTextures(1, &gglowtex);
		gglowtex = 0;
	}
	if (gstencilrb) {
		glDeleteRenderbuffers_(1, &gstencilrb);
		gstencilrb = 0;
	}
	if (gdepthrb) {
		glDeleteRenderbuffers_(1, &gdepthrb);
		gdepthrb = 0;
	}
	if (hdrfbo) {
		glDeleteFramebuffers_(1, &hdrfbo);
		hdrfbo = 0;
	}
	if (hdrtex) {
	...
	if (refractfbo) {
		glDeleteFramebuffers_(1, &refractfbo);
		refractfbo = 0;
	}
	if (refracttex) {
...
void processhdr(GLuint outfbo, int aa) {
	...
	int b0w = max(vieww / 4, bloomw), b0h = max(viewh / 4, bloomh), b1w = max(vieww / 2, bloomw),
		b1h = max(viewh / 2, bloomh), pw = vieww, ph = viewh;
	if (msaalight) {
	...
	if (!lasthdraccum || lastmillis - lasthdraccum >= hdraccummillis) {
		...
		LOCALPARAMF(accumscale,
					lasthdraccum ? pow(hdraccumscale, float(lastmillis - lasthdraccum) / hdraccummillis) : 0);
		screenquad(2, 2);
		glDisable(GL_BLEND);
	...
	if (bloompbo) {
		...
		gle::enablecolor();
		gle::colorpointer(
			hasTF ? sizeof(GLfloat) : sizeof(GLushort), (const void*)nullptr, hasTF ? GL_FLOAT : GL_UNSIGNED_SHORT, 1);
		gle::clearvbo();
	}
...
auto loadradiancehintsshader() -> Shader* {
...
void setupradiancehints() {
	...
	loopi(!rhrect && rhcache ? 8 : 4) {
		...
		create3dtexture(rhtex[i],
						rhgrid + 2 * rhborder,
						rhgrid + 2 * rhborder,
						(rhgrid + 2 * rhborder) * rhsplits,
						nullptr,
						7,
						1,
						rhformat);
	...
	if (rhrect)
		loopi(4) {
			...
			glRenderbufferStorage_(GL_RENDERBUFFER,
								   rhformat,
								   (rhgrid + 2 * rhborder) * (rhgrid + 2 * rhborder),
								   (rhgrid + 2 * rhborder) * rhsplits);
			glBindRenderbuffer_(GL_RENDERBUFFER, 0);
			glFramebufferRenderbuffer_(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_RENDERBUFFER, rhrb[i]);
		}
	else
		loopi(4) glFramebufferTexture3D_(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_3D, rhtex[i], 0, 0);
	...
	createtexture(
		...
		3,
		0,
		rsmdepthprec > 1 ? GL_DEPTH_COMPONENT32 : (rsmdepthprec ? GL_DEPTH_COMPONENT24 : GL_DEPTH_COMPONENT16),
		GL_TEXTURE_RECTANGLE);
...
void cleanupradiancehints() {
	...
	loopi(4) if (rhrb[i]) {
		glDeleteRenderbuffers_(1, &rhrb[i]);
		rhrb[i] = 0;
	}
	if (rhfbo) {
...
VARF(rhsplits, 1, 2, RH_MAXSPLITS, {
	cleardeferredlightshaders();
...
struct lightinfo {
	...
	lightinfo(int i, const extentity& e)
		: ent(i),
		  ...
		  flags(e.attr5),
		  o(e.o),
		  color(vec(e.attr2, e.attr3, e.attr4).max(0)),
		  radius(e.attr1),
	...
	[[nodiscard]] auto noshadow() const -> bool {
		return flags & L_NOSHADOW || radius <= smminradius;
	}
	...
	[[nodiscard]] auto validscissor() const -> bool {
	...
	[[nodiscard]] auto checkquery() const -> bool {
	...
	void calcbb(vec& bbmin, vec& bbmax) {
		if (spot > 0) {
			...
			vec up = vec(spotx).mul(spotscale).abs(), right = vec(spoty).mul(spotscale).abs(),
...
struct shadowcachekey {
	...
	shadowcachekey() = default;
	shadowcachekey(const lightinfo& l) : o(l.o), radius(l.radius), dir(l.dir), spot(l.spot) {
	}
};

static inline auto hthash(const shadowcachekey& k) -> uint {
	return hthash(k.o);
}

static inline auto htcmp(const shadowcachekey& x, const shadowcachekey& y) -> bool {
...
static inline auto usegatherforsm() -> bool {
	return smfilter > 1 && smgather && hasTG && usetexgather;
}
static inline auto usesmcomparemode() -> bool {
	return !usegatherforsm() || (hasTG && hasGPU5 && usetexgather > 1);
}
...
void setupshadowatlas() {
	...
	createtexture(shadowatlastex,
				  shadowatlaspacker.w,
				  shadowatlaspacker.h,
				  nullptr,
				  3,
				  1,
				  smdepthprec > 1 ? GL_DEPTH_COMPONENT32 : (smdepthprec ? GL_DEPTH_COMPONENT24 : GL_DEPTH_COMPONENT16),
				  shadowatlastarget);
	glTexParameteri(shadowatlastarget, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE);
...
void cleanupshadowatlas() {
	if (shadowatlastex) {
		glDeleteTextures(1, &shadowatlastex);
		shadowatlastex = 0;
	}
	if (shadowatlasfbo) {
...
VARFP(smfilter, 0, 2, 3, {
	cleardeferredlightshaders();
	cleanupshadowatlas();
	cleanupvolumetric();
});
VARFP(smgather, 0, 0, 1, {
	cleardeferredlightshaders();
	cleanupshadowatlas();
	cleanupvolumetric();
});
...
struct lightrect {
	...
	[[nodiscard]] auto outside(const lightrect& o) const -> bool {
		return x1 >= o.x2 || x2 <= o.x1 || y1 >= o.y2 || y2 <= o.y1;
	}

	[[nodiscard]] auto inside(const lightrect& o) const -> bool {
	...
	auto overlaps(int tx1, int ty1, int tx2, int ty2, const uint* tilemask) const -> bool {
		if (int(x2) <= tx1 || int(x1) >= tx2 || int(y2) <= ty1 || int(y1) >= ty2)
			return false;
		if (!tilemask)
			return true;
		uint xmask = (1 << x2) - (1 << x1);
		for (int y = max(int(y1), ty1), end = min(int(y2), ty2); y < end; y++)
			if (tilemask[y] & xmask)
				return true;
		return false;
	}
...
enum { BF_SPOTLIGHT = 1 << 0, BF_NOSHADOW = 1 << 1, BF_NOSUN = 1 << 2 };
...
static inline auto hthash(const lightbatchkey& l) -> uint {
	uint h = 0;
	loopi(l.numlights) h = ((h << 8) + h) ^ l.lights[i];
	return h;
}

static inline auto htcmp(const lightbatchkey& x, const lightbatchkey& y) -> bool {
	return x.flags == y.flags && x.numlights == y.numlights &&
...
static auto addshadowmap(ushort x,
								   ushort y,
								   int size,
								   int& idx,
								   int light = -1,
								   shadowcacheval* cached = nullptr) -> shadowmapinfo* {
...
VARF(csmsplits, 1, 3, CSM_MAXSPLITS, {
	cleardeferredlightshaders();
	clearshadowcache();
});
FVAR(csmsplitweight, 0.20f, 0.75f, 0.95f);
VARF(csmshadowmap, 0, 1, 1, {
	cleardeferredlightshaders();
	clearshadowcache();
});
...
void cascadedshadowmap::getprojmatrix() {
	...
	loopi(csmsplits) {
		...
		split.proj.identity();
		split.proj.settranslation(
			2 * split.offset.x / sm.size - 1, 2 * split.offset.y / sm.size - 1, 2 * split.offset.z - 1);
...
void cascadedshadowmap::bindparams() {
	...
	loopi(csmsplits) {
		...
		const float bias = (smfilter > 2 ? csmbias2 : csmbias) * (-512.0f / sm.size) *
			(split.farplane - split.nearplane) / (splits[0].farplane - splits[0].nearplane);
...
auto calcbbcsmsplits(const ivec& bbmin, const ivec& bbmax) -> int {
...
auto calcspherecsmsplits(const vec& center, float radius) -> int {
	int mask = (1 << csmsplits) - 1;
	if (!csmcull)
		return mask;
	loopi(csmsplits) {
...
auto calcbbrsmsplits(const ivec& bbmin, const ivec& bbmax) -> int {
	if (!rsmcull)
		return 1;
	loopk(4) {
...
auto calcspherersmsplits(const vec& center, float radius) -> int {
	if (!rsmcull)
		return 1;
	loopk(4) {
		const plane& p = rsm.cull[k];
...
struct radiancehints {
	struct splitinfo {
		...
		splitinfo() : center(-1e16f, -1e16f, -1e16f),  cached(-1e16f, -1e16f, -1e16f) {
		}
	...
	radiancehints()
		: dynmin(1e16f, 1e16f, 1e16f),
		  dynmax(-1e16f, -1e16f, -1e16f),
		  prevdynmin(1e16f, 1e16f, 1e16f),
		  prevdynmax(-1e16f, -1e16f, -1e16f) {
	}
	...
	void clearcache() {
		loopi(RH_MAXSPLITS) splits[i].clearcache();
	}
	[[nodiscard]] auto allcached() const -> bool {
		loopi(rhsplits) if (splits[i].cached != splits[i].center) return false;
		return true;
	}
} rh;
...
void radiancehints::setup() {
	...
	loopi(rhsplits) {
		...
		// modify mvp with a scale and offset
		// now compute the update model view matrix for this split
		split.scale = vec(1 / (step * (rhgrid + 2 * rhborder)),
						  1 / (step * (rhgrid + 2 * rhborder)),
						  1 / (step * (rhgrid + 2 * rhborder) * rhsplits));
		split.offset = vec(-(offset.x - rhborder) / (rhgrid + 2 * rhborder),
						   -(offset.y - rhborder) / (rhgrid + 2 * rhborder),
						   (i - (offset.z - rhborder) / (rhgrid + 2 * rhborder)) / float(rhsplits));
	}
}
...
void radiancehints::bindparams() {
	...
	loopi(rhsplits) {
		splitinfo& split = splits[i];
		rhtcv[i] =
			vec4(vec(split.center).mul(-split.scale.x), split.scale.x);	 // split.bounds*(1 + rhborder*2*0.5f/rhgrid));
	}
	GLOBALPARAMF(rhbounds, 0.5f * (rhgrid + rhborder) / float(rhgrid + 2 * rhborder));
}

auto useradiancehints() -> bool {
...
auto loadvolumetricshader() -> Shader* {
	...
	return generateshader(name, R"(volumetricshader "%s" "%s" %d)", common, shadow, volsteps);
}
...
void loadvolumetricshaders() {
	...
	if (volbilateral)
		loopi(2) {
			...
			volumetricbilateralshader[i] =
				generateshader(name, "volumetricbilateralshader %d %d", volbilateral, volreduce);
		}
}
...
static Shader *deferredlightshader = nullptr, *deferredminimapshader = nullptr, *deferredmsaapixelshader = nullptr,
...
auto loaddeferredlightshader(const char* type = nullptr) -> Shader* {
	...
	return generateshader(name,
						  R"(deferredlightshader "%s" "%s" "%s" %d %d %d)",
						  common,
						  shadow,
						  sun,
						  usecsm,
						  userh,
						  !minimap ? lighttilebatch : 0);
}
...
static inline auto sortlights(int x, int y) -> bool {
...
void init(int slices, int stacks) {
	...
	loopi(stacks + 1) {
		float rho = M_PI * (1 - t), s = 0.0f, sinrho = i && i < stacks ? sin(rho) : 0,
...
static inline void lightquads(float z, float sx1, float sy1, float sx2, float sy2, int tx1, int ty1, int tx2, int ty2) {
	int vx1 = max(int(floor((sx1 * 0.5f + 0.5f) * vieww)), ((tx1 * lighttilevieww) / lighttilew) * lighttilealignw),
		...
		vx2 = min(int(ceil((sx2 * 0.5f + 0.5f) * vieww)),
				  min(((tx2 * lighttilevieww) / lighttilew) * lighttilealignw, vieww)),
		vy2 = min(int(ceil((sy2 * 0.5f + 0.5f) * viewh)),
				  min(((ty2 * lighttileviewh) / lighttileh) * lighttilealignh, viewh));
	lightquads(z,
			   (vx1 * 2.0f) / vieww - 1.0f,
			   (vy1 * 2.0f) / viewh - 1.0f,
			   (vx2 * 2.0f) / vieww - 1.0f,
...
static inline void
lightquads(float z, float sx1, float sy1, float sx2, float sy2, int x1, int y1, int x2, int y2, const uint* tilemask) {
	if (!tilemask)
...
static inline void setlightglobals(bool transparent = false) {
	...
	if (!drawtex && editmode && fullbright)
		GLOBALPARAMF(lightscale,
					 fullbrightlevel * lightscale,
					 fullbrightlevel * lightscale,
					 fullbrightlevel * lightscale,
					 255 * lightscale);
	else
		GLOBALPARAMF(lightscale,
					 ambient.x * lightscale * ambientscale,
					 ambient.y * lightscale * ambientscale,
					 ambient.z * lightscale * ambientscale,
					 255 * lightscale);
	...
	if (!sunlight.iszero() && csmshadowmap) {
		...
		if (!drawtex && editmode && fullbright) {
			...
			GLOBALPARAMF(sunlightcolor,
						 sunlight.x * lightscale * sunlightscale,
						 sunlight.y * lightscale * sunlightscale,
						 sunlight.z * lightscale * sunlightscale);
			GLOBALPARAMF(giscale, 2 * giscale);
			GLOBALPARAMF(skylightcolor,
						 2 * giaoscale * skylight.x * lightscale * skylightscale,
						 2 * giaoscale * skylight.y * lightscale * skylightscale,
						 2 * giaoscale * skylight.z * lightscale * skylightscale);
		}
	}
...
static LocalShaderParam lightpos("lightpos"), lightcolor("lightcolor"), spotparams("spotparams"),
...
static inline void setlightparams(int i, const lightinfo& l) {
	...
	if (l.shadowmap >= 0) {
		...
		float smnearclip = SQRT3 / l.radius, smfarclip = SQRT3,
			  bias = (smfilter > 2 || shadowatlaspacker.w > SHADOWATLAS_SIZE ? smbias2 : smbias) *
			(smcullside ? 1 : -1) * smnearclip * (1024.0f / sm.size);
		int border = smfilter > 2 ? smborder2 : smborder;
		if (l.spot > 0) {
			shadowparamsv[i] = vec4(-0.5f * sm.size * cotan360(l.spot),
									(-smnearclip * smfarclip / (smfarclip - smnearclip) - 0.5f * bias),
									1 / (1 + fabs(l.dir.z)),
									0.5f + 0.5f * (smfarclip + smnearclip) / (smfarclip - smnearclip));
		} else {
			shadowparamsv[i] = vec4(-0.5f * (sm.size - border),
									-smnearclip * smfarclip / (smfarclip - smnearclip) - 0.5f * bias,
									sm.size,
									0.5f + 0.5f * (smfarclip + smnearclip) / (smfarclip - smnearclip));
		}
...
static inline void setlightshader(Shader* s,
								  int n,
								  bool baselight,
								  bool shadowmap,
								  bool spotlight,
	...
	s->setvariant(
		n - 1,
		(shadowmap ? 1 : 0) + (baselight ? 0 : 2) + (spotlight ? 4 : 0) + (transparent ? 8 : 0) + (avatar ? 24 : 0));
...
static void rendersunpass(Shader* s,
						  int stencilref,
						  bool transparent,
						  float bsx1,
						  float bsy1,
...
static void
renderlightsnobatch(Shader* s, int stencilref, bool transparent, float bsx1, float bsy1, float bsx2, float bsy2) {
	lightsphere::enable();
	...
	loop(avatarpass, stencilref >= 0 ? 2 : 1) {
		...
		loopv(lightorder) {
			...
			float sx1 = max(bsx1, l.sx1), sy1 = max(bsy1, l.sy1), sx2 = min(bsx2, l.sx2), sy2 = min(bsy2, l.sy2);
...
static void renderlightbatches(Shader* s,
							   int stencilref,
							   bool transparent,
							   float bsx1,
							   float bsy1,
	...
	loopv(lightbatches) {
		...
		loopvj(batch.rects) {
			...
			int x1 = max(int(r.x1), btx1), y1 = max(int(r.y1), bty1), x2 = min(int(r.x2), btx2),
...
void renderlights(float bsx1 = -1,
				  float bsy1 = -1,
				  float bsx2 = 1,
				  float bsy2 = 1,
				  const uint* tilemask = nullptr,
				  int stencilmask = 0,
				  int msaapass = 0,
				  bool transparent = false) {
	Shader* s = drawtex == DRAWTEX_MINIMAP
		? deferredminimapshader
		: (msaapass <= 0 ? deferredlightshader : (msaapass > 1 ? deferredmsaasampleshader : deferredmsaapixelshader));
	if (!s || s == nullshader)
		return;
	...
	bool sunpass = !lighttilebatch || drawtex == DRAWTEX_MINIMAP ||
		(!sunlight.iszero() && csmshadowmap && batchsunlight <= (gi && giscale && gidist ? 1 : 0));
	if (sunpass) {
		if (depthtestlights && depth) {
			glDisable(GL_DEPTH_TEST);
			depth = false;
...
void rendervolumetric() {
	...
	loopv(lightorder) {
		const lightinfo& l = lights[lightorder[i]];
		if (!(l.flags & L_VOLUMETRIC) || l.checkquery())
			continue;
	...
	int cx1 = int(floor((bsx1 * 0.5f + 0.5f) * volw)) & ~1, cy1 = int(floor((bsy1 * 0.5f + 0.5f) * volh)) & ~1,
	...
	loopv(entgroup) {
		...
		if (ents.inrange(idx) && ents[idx]->type == ET_LIGHT) {
			...
			loopvj(lights) if (lights[j].o == e.o) {
				...
				float x1 = (l.sx1 + 1) / 2 * hudw, x2 = (l.sx2 + 1) / 2 * hudw, y1 = (1 - l.sy1) / 2 * hudh,
					  y2 = (1 - l.sy2) / 2 * hudh;
				gle::begin(GL_TRIANGLE_STRIP);
				gle::attribf(x1, y1);
				gle::attribf(x2, y1);
...
void collectlights() {
	...
	if (!editmode || !fullbright)
		loopv(ents) {
			...
			if (smviscull) {
				if (isfoggedsphere(e->attr1, e->o))
					continue;
				if (pvsoccludedsphere(e->o, e->attr1))
					continue;
			}
	...
	if (!drawtex && smquery && oqfrags && oqlights)
		loopv(lightorder) {
			...
			if ((l.noshadow() && (!oqvol || !(l.flags & L_VOLUMETRIC))) || l.radius >= worldsize)
				continue;
			vec bbmin, bbmax;
	...
	if (smcache && !smnoshadow && shadowcache.numelems)
		loop(mismatched, 2) loopv(lightorder) {
			int idx = lightorder[i];
			lightinfo& l = lights[idx];
			if (l.noshadow())
				continue;
...
static inline auto shouldworkinoq() -> bool {
...
struct batchrect : lightrect {
	...
	batchrect(const lightinfo& l, ushort idx)
		: lightrect(l), group((l.shadowmap < 0 ? BF_NOSHADOW : 0) | (l.spot > 0 ? BF_SPOTLIGHT : 0)), idx(idx) {
	}
};
...
struct batchstack : lightrect {
	...
	batchstack() = default;
	batchstack(uchar x1, uchar y1, uchar x2, uchar y2, ushort offset, ushort numrects, uchar flags = 0)
		: lightrect(x1, y1, x2, y2), offset(offset), numrects(numrects), flags(flags) {
	}
};
...
static void batchlights(const batchstack& initstack) {
	...
	while (numstack > 0) {
		...
		loop(g, BF_NOSUN) while (groups[g] >= lighttilebatch ||
								 (inside == outside && (groups[g] || !(flags & BF_NOSUN)))) {
			lightbatchkey key;
			key.flags = flags | g;
			flags |= BF_NOSUN;
...
static inline auto sortlightbatches(const lightbatch* x, const lightbatch* y) -> bool {
	if (x->flags < y->flags)
		return true;
	if (x->flags > y->flags)
		return false;
...
static inline auto rendernogi(cube* c, const ivec& o, int size, const ivec& bbmin, const ivec& bbmax, int minsize) -> bool {
	ivec mid = ivec(o).add(size);
	uchar overlap = 0;
	if (bbmin.y < mid.y) {
		if (bbmin.x < mid.x) {
			if ((bbmin.z < mid.z &&
				 (c[0].children ? rendernogi(c[0].children, ivec(o.x, o.y, o.z), size >> 1, bbmin, bbmax, minsize)
								: c[0].material & MAT_NOGI)) ||
				(bbmax.z > mid.z &&
				 (c[4].children ? rendernogi(c[4].children, ivec(o.x, o.y, mid.z), size >> 1, bbmin, bbmax, minsize)
								: c[4].material & MAT_NOGI)))
				overlap |= 1;
		}
		if (bbmax.x > mid.x) {
			if ((bbmin.z < mid.z &&
				 (c[1].children ? rendernogi(c[1].children, ivec(mid.x, o.y, o.z), size >> 1, bbmin, bbmax, minsize)
								: c[1].material & MAT_NOGI)) ||
				(bbmax.z > mid.z &&
				 (c[5].children ? rendernogi(c[5].children, ivec(mid.x, o.y, mid.z), size >> 1, bbmin, bbmax, minsize)
								: c[5].material & MAT_NOGI)))
				overlap |= 2;
		}
	}
	if (bbmax.y > mid.y) {
		if (bbmin.x < mid.x) {
			if ((bbmin.z < mid.z &&
				 (c[2].children ? rendernogi(c[2].children, ivec(o.x, mid.y, o.z), size >> 1, bbmin, bbmax, minsize)
								: c[2].material & MAT_NOGI)) ||
				(bbmax.z > mid.z &&
				 (c[6].children ? rendernogi(c[6].children, ivec(o.x, mid.y, mid.z), size >> 1, bbmin, bbmax, minsize)
								: c[6].material & MAT_NOGI)))
				overlap |= 4;
		}
		if (bbmax.x > mid.x) {
			if ((bbmin.z < mid.z &&
				 (c[3].children ? rendernogi(c[3].children, ivec(mid.x, mid.y, o.z), size >> 1, bbmin, bbmax, minsize)
								: c[3].material & MAT_NOGI)) ||
				(bbmax.z > mid.z &&
				 (c[7].children ? rendernogi(c[7].children, ivec(mid.x, mid.y, mid.z), size >> 1, bbmin, bbmax, minsize)
								: c[7].material & MAT_NOGI)))
				overlap |= 8;
		}
	}
	if (!overlap)
		return false;
	if (overlap == 0xF || size <= minsize)
		return true;
	if (overlap & 1) {
		if (overlap & 2)
			nogiquad(o.x, o.y, 2 * size, size);
		else
			nogiquad(o.x, o.y, size, size);
	} else if (overlap & 2)
		nogiquad(o.x + size, o.y, size, size);
	if (overlap & 4) {
		if (overlap & 8)
			nogiquad(o.x, o.y + size, 2 * size, size);
		else
			nogiquad(o.x, o.y + size, size, size);
	} else if (overlap & 8)
		nogiquad(o.x + size, o.y + size, size, size);
	return false;
}

static inline void rendernogi(const ivec& bbmin, const ivec& bbmax, int minsize) {
	if (rendernogi(
			worldroot, ivec(0, 0, 0), worldsize >> 1, ivec(bbmin).max(nogimin), ivec(bbmax).min(nogimax), minsize))
		nogiquad(0, 0, worldsize, worldsize);
}

static inline void
rhquad(float x1, float y1, float x2, float y2, float tx1, float ty1, float tx2, float ty2, float tz) {
	gle::begin(GL_TRIANGLE_STRIP);
	gle::attribf(x2, y1);
	gle::attribf(tx2, ty1, tz);
	gle::attribf(x1, y1);
	gle::attribf(tx1, ty1, tz);
	gle::attribf(x2, y2);
	gle::attribf(tx2, ty2, tz);
	gle::attribf(x1, y2);
	gle::attribf(tx1, ty2, tz);
	gle::end();
}

static inline void rhquad(float dx1,
						  float dy1,
						  float dx2,
						  float dy2,
						  float dtx1,
	...
	gle::begin(GL_TRIANGLE_STRIP);
	gle::attribf(dx2, dy1);
	gle::attribf(dtx2, dty1, dtz);
	gle::attribf(px2, py1);
	gle::attribf(ptx2, pty1, ptz);
	gle::attribf(dx1, dy1);
	gle::attribf(dtx1, dty1, dtz);
	gle::attribf(px1, py1);
	gle::attribf(ptx1, pty1, ptz);
	gle::attribf(dx1, dy2);
	gle::attribf(dtx1, dty2, dtz);
	gle::attribf(px1, py2);
	gle::attribf(ptx1, pty2, ptz);
	gle::attribf(dx2, dy2);
	gle::attribf(dtx2, dty2, dtz);
	gle::attribf(px2, py2);
	gle::attribf(ptx2, pty2, ptz);
	gle::attribf(dx2, dy1);
	gle::attribf(dtx2, dty1, dtz);
	gle::attribf(px2, py1);
	gle::attribf(ptx2, pty1, ptz);
...
void radiancehints::renderslices() {
	...
	loopirev(rhsplits) {
		...
		vec cmin, cmax, dmin(1e16f, 1e16f, 1e16f), dmax(-1e16f, -1e16f, -1e16f), bmin(1e16f, 1e16f, 1e16f),
			bmax(-1e16f, -1e16f, -1e16f);
		loopk(3) {
			cmin[k] = floor((worldmin[k] - nudge - (split.center[k] - split.bounds)) / step) * step + split.center[k] -
				split.bounds;
			cmax[k] = ceil((worldmax[k] + nudge - (split.center[k] - split.bounds)) / step) * step + split.center[k] -
				split.bounds;
		}
		if (prevdynmin.z < prevdynmax.z)
			loopk(3) {
				dmin[k] =
					min(dmin[k],
						(float)floor((prevdynmin[k] - gidist - cellradius - (split.center[k] - split.bounds)) / step) *
								step +
							split.center[k] - split.bounds);
				dmax[k] =
					max(dmax[k],
						(float)ceil((prevdynmax[k] + gidist + cellradius - (split.center[k] - split.bounds)) / step) *
								step +
							split.center[k] - split.bounds);
			}
		if (dynmin.z < dynmax.z)
			loopk(3) {
				dmin[k] = min(
					dmin[k],
					(float)floor((dynmin[k] - gidist - cellradius - (split.center[k] - split.bounds)) / step) * step +
						split.center[k] - split.bounds);
				dmax[k] = max(
					dmax[k],
					(float)ceil((dynmax[k] + gidist + cellradius - (split.center[k] - split.bounds)) / step) * step +
						split.center[k] - split.bounds);
			}
		...
		if ((rhrect || !rhcache || hasCI) && split.cached == split.center && (!rhborder || prevcached) && !rhforce &&
			...
			if (rhrect || !rhcache || split.copied)
				continue;
			split.copied = true;
			loopk(4) glCopyImageSubData_(
				rhtex[4 + k], GL_TEXTURE_3D, 0, 0, 0, i * sh, rhtex[k], GL_TEXTURE_3D, 0, 0, 0, i * sh, sw, sh, sh);
			continue;
		}
		...
		if (rhborder && i + 1 < rhsplits) {
			...
			GLOBALPARAMF(borderrange,
						 0.5f - 0.5f / (rhgrid + 2),
						 0.5f - 0.5f / (rhgrid + 2),
						 (0.5f - 0.5f / (rhgrid + 2)) / rhsplits);
			GLOBALPARAMF(borderscale, rhgrid + 2, rhgrid + 2, (rhgrid + 2) * rhsplits);

			splitinfo& next = splits[i + 1];
			loopk(3) {
				bmin[k] = floor((max(float(worldmin[k] - nudge), next.center[k] - next.bounds) -
								 (split.center[k] - split.bounds)) /
								step) *
						step +
					split.center[k] - split.bounds;
				bmax[k] = ceil((min(float(worldmax[k] + nudge), next.center[k] + next.bounds) -
								(split.center[k] - split.bounds)) /
							   step) *
						step +
					split.center[k] - split.bounds;
	...
	do {                                                                                                          \
		if (rhrect) {                                                                                             \
			...
			float x1 = split.center.x - split.bounds, x2 = split.center.x + split.bounds,
				  ...
				  vx1 = -1 + rhborder * 2.0f / (rhgrid + 2), vx2 = 1 - rhborder * 2.0f / (rhgrid + 2),
				  vy1 = -1 + rhborder * 2.0f / (rhgrid + 2), vy2 = 1 - rhborder * 2.0f / (rhgrid + 2), tx1 = x1,
				  tx2 = x2, ty1 = y1, ty2 = y2;
			bool clipped = false;

			if (rhborder && i + 1 < rhsplits) {
				splitinfo& next = splits[i + 1];
				float bx1 = x1 - step, bx2 = x2 + step, by1 = y1 - step, by2 = y2 + step, bz = z, bvx1 = -1, bvx2 = 1,
					  bvy1 = -1, bvy2 = 1, btx1 = bx1, btx2 = bx2, bty1 = by1, bty2 = by2;
		...
		noborder:
			...
			skipped:
				if (clearmasks[j / 32] & (1 << (j % 32)) && (!rhrect || cx < 0) &&
			...
			if (rhcache && z > split.cached.z - split.bounds && z < split.cached.z + split.bounds) {
				...
				if (px1 < px2 && py1 < py2) {
					...
					if (px1 != tx1 || px2 != tx2 || py1 != ty1 || py2 != ty2) {
						...
						rhquad(
							pvx1, pvy1, pvx2, pvy2, px1, py1, px2, py2, z, vx1, vy1, vx2, vy2, tx1, ty1, tx2, ty2, z);
					}

					if (z > dmin.z && z < dmax.z) {
						float dx1 = max(px1, dmin.x), dx2 = min(px2, dmax.x), dy1 = max(py1, dmin.y),
							  dy2 = min(py2, dmax.y);
						if (dx1 < dx2 && dy1 < dy2) {
							float dvx1 = -1 + rhborder * 2.0f / (rhgrid + 2) +
								2 * rhgrid / float(sw) * (dx1 - x1) / (x2 - x1),
								  dvx2 = 1 - rhborder * 2.0f / (rhgrid + 2) +
								2 * rhgrid / float(sw) * (dx2 - x2) / (x2 - x1),
								  dvy1 = -1 + rhborder * 2.0f / (rhgrid + 2) +
								2 * rhgrid / float(sh) * (dy1 - y1) / (y2 - y1),
								  dvy2 = 1 - rhborder * 2.0f / (rhgrid + 2) +
								2 * rhgrid / float(sh) * (dy2 - y2) / (y2 - y1),
...
auto calcshadowinfo(const extentity& e, vec& origin, float& radius, vec& spotloc, int& spotangle, float& bias) -> int {
	if (e.attr5 & L_NOSHADOW || e.attr1 <= smminradius)
		return SM_NONE;
		...
		if (shadowmapping == SM_SPOT) {
			...
			if (!cachemask) {
				int cx1 = sidemask & 0x03 ? 0 : (sidemask & 0xC ? sm.size : 2 * sm.size),
					...
					cy1 = sidemask & 0x15 ? 0 : sm.size, cy2 = sidemask & 0x2A ? 2 * sm.size : sm.size;
	...
	loop(layer, 4) {
		...
		if (msaalight) {
			...
			if ((ghasstencil && msaaedgedetect) || msaalight == 2)
				loopi(2) renderlights(sx1, sy1, sx2, sy2, tiles, layer + 1, i + 1, true);
			else
...
void preparegbuffer(bool depthclear) {
	...
	if (gcolorclear)
		glClearColor(0, 0, 0, 0);
	glClear((depthclear ? GL_DEPTH_BUFFER_BIT : 0) | (gcolorclear ? GL_COLOR_BUFFER_BIT : 0) |
			(depthclear && ghasstencil && (!msaasamples || msaalight || ghasstencil > 1) ? GL_STENCIL_BUFFER_BIT : 0));
	if (gdepthformat && gdepthclear)
		maskgbuffer("cnd");
	if (drawtex && gdepthinit)
		glDisable(GL_SCISSOR_TEST);
	...
	if (drawtex == DRAWTEX_MINIMAP) {
		...
		float xscale = eyematrix.a.x, yscale = eyematrix.b.y, xoffset = eyematrix.d.x, yoffset = eyematrix.d.y,
...
void shademodelpreview(int x, int y, int w, int h, bool background, bool scissor) {
	...
	int sx = clamp(x, 0, hudw), sy = clamp(y, 0, hudh), sw = clamp(x + w, 0, hudw) - sx,
...
void shadegbuffer() {
	...
	if (msaasamples && (msaalight || !drawtex)) {
		if ((ghasstencil && msaaedgedetect) || msaalight == 2)
			loopi(2) renderlights(-1, -1, 1, 1, nullptr, 0, i + 1);
		else
...
void setuplights() {
	...
	if (drawtex == DRAWTEX_MINIMAP && !deferredminimapshader)
		deferredminimapshader = loaddeferredlightshader(msaalight ? "mM" : "m");
	setupaa(gw, gh);
	GLERROR;
}

auto debuglights() -> bool {
	if (debugshadowatlas)
		viewshadowatlas();
	else if (debugao)
		viewao();
