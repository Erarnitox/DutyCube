...
auto getentboundingbox(const extentity& e, ivec& o, ivec& r) -> bool {
	switch (e.type) {
	case ET_EMPTY:
		return false;
	case ET_DECAL: {
...
enum { MODOE_ADD = 1 << 0, MODOE_UPDATEBB = 1 << 1, MODOE_CHANGED = 1 << 2 };
...
static auto modifyoctaent(int flags, int id, extentity& e) -> bool {
	if (flags & MODOE_ADD ? e.flags & EF_OCTA : !(e.flags & EF_OCTA))
		return false;
...
static inline auto modifyoctaent(int flags, int id) -> bool {
	vector<extentity*>& ents = entities::getents();
	return ents.inrange(id) && modifyoctaent(flags, id, *ents[id]);
}

static inline void addentity(int id) {
	modifyoctaent(MODOE_ADD | MODOE_UPDATEBB, id);
}
static inline void addentityedit(int id) {
	modifyoctaent(MODOE_ADD | MODOE_UPDATEBB | MODOE_CHANGED, id);
}
static inline void removeentity(int id) {
	modifyoctaent(MODOE_UPDATEBB, id);
}
static inline void removeentityedit(int id) {
	modifyoctaent(MODOE_UPDATEBB | MODOE_CHANGED, id);
}
...
static inline void findents(octaentities& oe,
							int low,
							int high,
							bool notspawned,
							const vec& pos,
	...
	loopv(oe.other) {
		int id = oe.other[i];
		extentity& e = *ents[id];
		if (e.type >= low && e.type <= high && (e.spawned() || notspawned) &&
			vec(e.o).sub(pos).mul(invradius).squaredlen() <= 1)
			found.add(id);
	}
}

static inline void findents(cube* c,
							const ivec& o,
							int size,
							const ivec& bo,
							const ivec& br,
...
void findents(int low, int high, bool notspawned, const vec& pos, const vec& radius, vector<int>& found) {
	...
	ivec bo(vec(pos).sub(radius).sub(1)), br(vec(pos).add(radius).add(1));
	int diff = (bo.x ^ br.x) | (bo.y ^ br.y) | (bo.z ^ br.z) | octaentsize, scale = worldscale - 1;
...
auto pointinsel(const selinfo& sel, const vec& o) -> bool {
	return (o.x <= sel.o.x + sel.s.x * sel.grid && o.x >= sel.o.x && o.y <= sel.o.y + sel.s.y * sel.grid &&
			o.y >= sel.o.y && o.z <= sel.o.z + sel.s.z * sel.grid && o.z >= sel.o.z);
}

vector<int> entgroup;

auto haveselent() -> bool {
...
auto newundoent() -> undoblock* {
	int numents = entgroup.length();
	if (numents <= 0)
		return nullptr;
	auto* u = (undoblock*)new uchar[sizeof(undoblock) + numents * sizeof(undoent)];
	u->numents = numents;
	auto* e = (undoent*)(u + 1);
	loopv(entgroup) {
		e->i = entgroup[i];
...
void attachentity(extentity& e) {
	...
	default:
		if (e.type < ET_GAMESPECIFIC || !entities::mayattach(e))
			return;
		break;
	}
	...
	loopv(ents) {
		...
		default:
			if (e.type < ET_GAMESPECIFIC || !entities::attachent(e, *a))
				continue;
			break;
		}
...
#define addgroup(exp)                                       \
	{                                                       \
		vector<extentity*>& ents = entities::getents();     \
		loopv(ents) entfocusv(i, if (exp) entadd(n), ents); \
	}
#define setgroup(exp)  \
	{                  \
		entcancel();   \
...
#define groupeditloop(f)                                \
	{                                                   \
		...
		entlooplevel++;                                 \
		int _ = efocus;                                 \
		loopv(entgroup) enteditv(entgroup[i], f, ents); \
		efocus = _;                                     \
		entlooplevel--;                                 \
	}
...
auto copyundoents(undoblock* u) -> undoblock* {
	entcancel();
	undoent* e = u->ents();
	loopi(u->numents) entadd(e[i].i);
	undoblock* c = newundoent();
	loopi(u->numents) if (e[i].e.type == ET_EMPTY) entgroup.removeobj(e[i].i);
	return c;
}
...
void pasteundoent(int idx, const entity& ue) {
	...
	while (ents.length() < idx)
		ents.add(entities::newentity())->type = ET_EMPTY;
	int efocus = -1;
...
void entrotate(int* cw) {
	...
	vec s(sel.o.v);
	groupeditundo(e.o[dd] -= (e.o[dd] - mid) * 2; e.o.sub(s); swap(e.o[R[d]], e.o[C[d]]); e.o.add(s););
}
...
extern auto editmoveplane(const vec& o, const vec& ray, int d, float off, vec& handle, vec& dest, bool first) -> bool;
...
void entdrag(const vec& ray) {
	...
	vec eo, es;
	int d = dimension(entorient), dc = dimcoord(entorient);

	entfocus(entgroup.last(), entselectionbox(e, eo, es);

			 if (!editmoveplane(e.o, ray, d, eo[d] + (dc ? es[d] : 0), handle, dest, entmoving == 1)) return;
...
void renderentradius(extentity& e, bool color) {
	...
	case ET_DECAL: {
		...
		float size = max(float(e.attr5), 1.0f);
		renderentbox(
			e, vec(0, s.depth * size / 2, 0), vec(size / 2, s.depth * size / 2, size / 2), e.attr2, e.attr3, e.attr4);
		break;
	}
...
static void renderentbox(const vec& eo, vec es) {
	...
	// bottom quad
	gle::attrib(eo.x, eo.y, eo.z);
	gle::attrib(es.x, eo.y, eo.z);
	gle::attrib(es.x, eo.y, eo.z);
	gle::attrib(es.x, es.y, eo.z);
	gle::attrib(es.x, es.y, eo.z);
	gle::attrib(eo.x, es.y, eo.z);
	gle::attrib(eo.x, es.y, eo.z);
	gle::attrib(eo.x, eo.y, eo.z);

	// top quad
	gle::attrib(eo.x, eo.y, es.z);
	gle::attrib(es.x, eo.y, es.z);
	gle::attrib(es.x, eo.y, es.z);
	gle::attrib(es.x, es.y, es.z);
	gle::attrib(es.x, es.y, es.z);
	gle::attrib(eo.x, es.y, es.z);
	gle::attrib(eo.x, es.y, es.z);
	gle::attrib(eo.x, eo.y, es.z);
...
void renderentselection(const vec& o, const vec& ray, bool entmoving) {
	...
	if (entgroup.length()) {
		...
		loopv(entgroup) entfocus(entgroup[i], entselectionbox(e, eo, es); renderentbox(eo, es););
		xtraverts += gle::end();
	}
	...
	if (enthover >= 0) {
		...
		if (entmoving && entmovingshadow == 1) {
			...
			(a = eo).x = eo.x - fmod(eo.x, worldsize);
			(b = es).x = a.x + worldsize;
			boxs3D(a, b, 1);
			(a = eo).y = eo.y - fmod(eo.y, worldsize);
			(b = es).y = a.x + worldsize;
			boxs3D(a, b, 1);
			(a = eo).z = eo.z - fmod(eo.z, worldsize);
			(b = es).z = a.x + worldsize;
			boxs3D(a, b, 1);
		}
		gle::colorub(200, 0, 0);
	...
	if (showentradius) {
		...
		if (enthover >= 0)
			entfocus(enthover, renderentradius(e, false));
		glDepthFunc(GL_LESS);
		loopv(entgroup) entfocus(entgroup[i], renderentradius(e, true));
		if (enthover >= 0)
			entfocus(enthover, renderentradius(e, true));
	}
}

auto enttoggle(int id) -> bool {
	undonext = true;
...
auto hoveringonent(int ent, int orient) -> bool {
	if (noentedit())
		return false;
	entorient = orient;
...
void entautoview(int* dir) {
	...
	if (t < 0 && s > 0)
		s = entgroup.length() - s;
	entfocus(entgroup[s], v.add(e.o); player->o = v; player->resetinterp(););
}
...
auto dropentity(entity& e, int drop = -1) -> bool {
...
auto
newentity(bool local, const vec& o, int type, int v1, int v2, int v3, int v4, int v5, int& idx, bool fix = true) -> extentity* {
	vector<extentity*>& ents = entities::getents();
	if (local) {
		idx = -1;
...
void entcopy() {
	...
	entcopybuf.shrink(0);
	addimplicit({ loopv(entgroup) entfocus(entgroup[i], entcopybuf.add(e).o.sub(vec(sel.o))); });
}
...
void printent(extentity& e, char* buf, int len) {
	...
	default:
		if (e.type >= ET_GAMESPECIFIC && entities::printent(e, buf, len))
			return;
		break;
	}
	nformatstring(
		buf, len, "%s %d %d %d %d %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3, e.attr4, e.attr5);
}
...
void enttype(char* type, int* numargs) {
	if (*numargs >= 1) {
		...
		if (typeidx != ET_EMPTY)
			groupedit(e.type = typeidx);
	} else
		entfocus(efocus, { result(entities::entname(e.type)); })
}

void entattr(int* attr, int* val, int* numargs) {
	if (*numargs >= 2) {
		if (*attr >= 0 && *attr <= 4)
			groupedit(switch (*attr) {
				case 0:
					e.attr1 = *val;
					break;
				case 1:
...
auto findentity(int type, int index, int attr1, int attr2) -> int {
...
void splitocta(cube* c, int size) {
	...
	loopi(8) {
		if (!c[i].children)
			c[i].children = newcubes(isempty(c[i]) ? F_EMPTY : F_SOLID);
...
auto emptymap(int scale, bool force, const char* mname, bool usecfg) -> bool  // main empty world creation routine
{
	if (!force && !editmode) {
...
auto enlargemap(bool force) -> bool {
	if (!force && !editmode) {
...
static auto isallempty(cube& c) -> bool {
	if (!c.children)
		return isempty(c);
...
auto getworldsize() -> int {
	return worldsize;
}
auto getmapversion() -> int {
	return mapversion;
}
