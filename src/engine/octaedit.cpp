...
void boxsgrid(int orient, vec o, vec s, int g) {
	int d = dimension(orient), dc = dimcoord(orient);
	float ox = o[R[d]], oy = o[C[d]], xs = s[R[d]], ys = s[C[d]], f = boxoutline ? (dc > 0 ? 0.2f : -0.2f) : 0;
...
VARF(dragging, 0, 0, 1, if (!dragging || cor[0] < 0) return; lastcur = cur; lastcor = cor; sel.grid = gridsize;
	 sel.orient = orient;);
...
void toggleedit(bool force) {
	if (!force) {
		if (!isconnected())
			return;
		if (player->state != CS_ALIVE && player->state != CS_DEAD && player->state != CS_EDITING)
...
auto noedit(bool view, bool msg) -> bool {
	if (!editmode) {
		if (msg)
			conoutf(CON_ERROR, "operation only allowed in edit mode");
		return true;
	}
	if (view || haveselent())
		return false;
	...
	if (!viewable && msg)
		conoutf(CON_ERROR, "selection not in view");
	return !viewable;
}
...
auto blockcube(int x,
				int y,
				int z,
				const block3& b,
				int rgrid) -> cube&	// looks up a world cube, based on coordinates mapped by the block
{
...
#define loopxyz(b, r, f)                                       \
	{                                                          \
		loop(z, (b).s[D[dimension((b).orient)]]) loopxy((b)) { \
			cube& c = blockcube(x, y, z, b, r);                \
			f;                                                 \
		}                                                      \
	}
#define loopselxyz(f)              \
...
auto editmoveplane(const vec& o, const vec& ray, int d, float off, vec& handle, vec& dest, bool first) -> bool {
	plane pl(d, off);
	float dist = 0.0f;
...
namespace hmap {
inline auto isheightmap(int orient, int d, bool empty, cube* c) -> bool;
}
extern void entdrag(const vec& ray);
extern auto hoveringonent(int ent, int orient) -> bool;
extern void renderentselection(const vec& o, const vec& ray, bool entmoving);
extern auto rayent(const vec& o, const vec& ray, float radius, int mode, int size, int& orient, int& ent) -> float;

VAR(gridlookup, 0, 0, 1);
VAR(passthroughcube, 0, 1, 1);

void rendereditcursor() {
	int d = dimension(sel.orient), od = dimension(orient), odc = dimcoord(orient);
	...
	if (moving) {
		...
		if (editmoveplane(
				vec(sel.o), camdir, od, sel.o[D[od]] + odc * sel.grid * sel.s[D[od]], handle, dest, moving == 1)) {
			if (moving == 1) {
				dest.add(handle);
		...
		wdist = rayent(player->o,
					   camdir,
					   1e16f,
					   (editmode && showmat ? RAY_EDITMAT : 0)	// select cubes first
						   | (!dragging && entediting ? RAY_ENTS : 0) | RAY_SKIPFIRST |
						   (passthroughcube == 1 ? RAY_PASS : 0),
					   gridsize,
					   entorient,
					   ent);

		if ((havesel || dragging) && !passthroughsel && !hmapedit)	// now try selecting the selection
			if (rayboxintersect(vec(sel.o),
								vec(sel.s).mul(sel.grid),
								player->o,
								camdir,
								sdist,
								orient)) {	// and choose the nearest of the two
				if (sdist < wdist) {
					wdist = sdist;
					ent = -1;
				}
	...
	loopoctabox(cor, size, bbmin, bbmax) {
		...
		if (c[i].ext) {
			if (c[i].ext->va)  // removes va s so that octarender will recreate
			{
				...
				c[i].ext->va = nullptr;
				if (hasmerges)
					invalidatemerges(c[i], o, size, true);
			}
...
void changed(const block3& sel, bool commit) {
	if (sel.s.iszero())
		return;
	readychanges(
		ivec(sel.o).sub(1), ivec(sel.s).mul(sel.grid).add(sel.o).add(1), worldroot, ivec(0, 0, 0), worldsize / 2);
	haschanged = true;
...
auto blockcopy(const block3& s, int rgrid) -> block3* {
...
static inline auto undosize(undoblock* u) -> int {
	if (u->numents)
...
struct undolist {
	undoblock *first{NULL}, *last{NULL};
...
auto newundocube(const selinfo& s) -> undoblock* {
...
static inline auto countblock(cube* c, int n = 8) -> int {
	int r = n;
	loopi(n) if (c[i].children) r += countblock(c[i].children);
	return r;
}

static auto countblock(block3* b) -> int {
...
void swapundo(undolist& a, undolist& b, int op) {
	if (noedit())
		return;
	if (a.empty()) {
		conoutf(CON_WARN, "nothing more to %s", op == EDIT_REDO ? "redo" : "undo");
		return;
	}
...
template <class B>
static auto packblock(block3& b, B& buf) -> bool {
...
template <class B>
static auto unpackblock(block3*& b, B& buf) -> bool {
	if (b) {
		freeblock(b);
		b = nullptr;
	}
	block3 hdr;
	if (buf.get((uchar*)&hdr, sizeof(hdr)) < int(sizeof(hdr)))
		return false;
	lilswap(hdr.o.v, 3);
	lilswap(hdr.s.v, 3);
	lilswap(&hdr.grid, 1);
...
static void unpackvslots(cube& c, ucharbuf& buf) {
	if (c.children) {
		...
		loopi(6) {
			...
			loopvj(unpackingvslots) if (unpackingvslots[j].index == tex) {
...
static auto compresseditinfo(const uchar* inbuf, int inlen, uchar*& outbuf, int& outlen) -> bool {
...
static auto uncompresseditinfo(const uchar* inbuf, int inlen, uchar*& outbuf, int& outlen) -> bool {
...
auto packeditinfo(editinfo* e, int& inlen, uchar*& outbuf, int& outlen) -> bool {
	vector<uchar> buf;
...
auto unpackeditinfo(editinfo*& e, const uchar* inbuf, int inlen, int outlen) -> bool {
	if (e && e->copy) {
		freeblock(e->copy);
		e->copy = nullptr;
	}
...
auto packundo(undoblock* u, int& inlen, uchar*& outbuf, int& outlen) -> bool {
	vector<uchar> buf;
	buf.reserve(512);
...
auto unpackundo(const uchar* inbuf, int inlen, int outlen) -> bool {
	uchar* outbuf = nullptr;
	if (!uncompresseditinfo(inbuf, inlen, outbuf, outlen))
		return false;
	ucharbuf buf(outbuf, outlen);
...
auto packundo(int op, int& inlen, uchar*& outbuf, int& outlen) -> bool {
	switch (op) {
	case EDIT_UNDO:
...
void saveprefab(char* name) {
	...
	if (!f) {
		conoutf(CON_ERROR, "could not write prefab to %s", filename);
		return;
	}
	prefabheader hdr;
...
void pasteblock(block3& b, selinfo& sel, bool local) {
	...
	sel.orient = b.orient;
	cube* s = b.c();
	loopselxyz(if (!isempty(*s) || s->children || s->material != MAT_AIR) pastecube(*s, c);
			   s++);  // 'transparent'. old opaque by 'delcube; paste'
	sel.orient = o;
}

auto loadprefab(const char* name, bool msg = true) -> prefab* {
	prefab* b = prefabs.access(name);
	if (b)
		return b;
	...
	if (!f) {
		if (msg)
			conoutf(CON_ERROR, "could not read prefab %s", filename);
		return nullptr;
	}
	prefabheader hdr;
...
struct prefabmesh {
	...
	auto addvert(const vertex& v) -> int {
	...
	auto addvert(const vec& pos, const bvec& norm) -> int {
		vertex vtx;
		vtx.pos = pos;
		vtx.norm = norm;
		return addvert(vtx);
...
static void genprefabmesh(prefabmesh& r, cube& c, const ivec& co, int size) {
	if (c.children) {
		...
		loopi(6) if ((vis = visibletris(c, i, co, size))) {
			...
			loopj(numverts - 2) if (index[0] != index[j + 1] && index[j + 1] != index[j + 2] &&
									index[j + 2] != index[0]) {
				r.tris.add(index[0]);
...
static void renderprefab(prefab& p, const vec& o, float yaw, float pitch, float roll, float size, const vec& color) {
	...
	gle::enablevertex();
	gle::enablenormal();
	auto* v = (prefabmesh::vertex*)nullptr;
	...
	glDrawRangeElements_(GL_TRIANGLES, 0, p.numverts - 1, p.numtris * 3, GL_UNSIGNED_SHORT, (ushort*)nullptr);
...
#define editingvslot(...)                 \
	vslotref vslotrefs[] = {__VA_ARGS__}; \
	(void)vslotrefs;
...
ICOMMAND(hmapselect, "", (), int t = lookupcube(cur).texture[orient]; int i = textures.find(t);
		 if (i < 0) textures.add(t);
		 else textures.remove(i););

inline auto isheightmap(int o, int d, bool empty, cube* c) -> bool {
	return havesel || (empty && isempty(*c)) || textures.empty() || textures.find(c->texture[o]) >= 0;
}
...
auto getcube(ivec t, int f) -> cube* {
	t[d] += dcr * f * gridsize;
	if (t[d] > nz || t[d] < mz)
		return nullptr;
	...
	if (t.x < changes.o.x)
		changes.o.x = t.x;
	else if (t.x > changes.s.x)
		changes.s.x = t.x;
	if (t.y < changes.o.y)
		changes.o.y = t.y;
	else if (t.y > changes.s.y)
		changes.s.y = t.y;
	if (t.z < changes.o.z)
		changes.o.z = t.z;
	else if (t.z > changes.s.z)
		changes.s.z = t.z;
	return c;
}

auto getface(cube* c, int d) -> uint {
...
void select(int x, int y, int z) {
	...
	if (face == 0x08080808 && (!c[0] || !isempty(*c[0]))) {
		flags[x][y] |= NOTHMAP;
		return;
	}
...
#define pullhmap(I, LT, GT, M, N, A)                                            \
	...
	if (!changed)
		return;
	if (x > mx)
		ripple(x - 1, y, mapz[x][y], true);
	if (x < nx)
		ripple(x + 1, y, mapz[x][y], true);
	if (y > my)
		ripple(x, y - 1, mapz[x][y], true);
	if (y < ny)
		ripple(x, y + 1, mapz[x][y], true);
...
#define loopbrush(i)                     \
	for (int x = bmx; x <= bnx + i; x++) \
		for (int y = bmy; y <= bny + i; y++)

void paint() {
	loopbrush(1) map[x][y] -= dr * brush[x][y];
}
...
void smooth() {
	...
	loopbrush(-2) {
		sum = 0;
		div = 9;
		loopi(3) loopj(3) if (flags[x + i][y + j] & MAPPED) sum += map[x + i][y + j];
		else div--;
		if (div)
			map[x + 1][y + 1] = sum / div;
	}
...
void rippleandset() {
	loopbrush(0) ripple(x, y, gz, false);
}
...
void run(int dir, int mode) {
	...
	selecting = true;
	select(clamp(MAXBRUSH2 - cx, bmx, bnx), clamp(MAXBRUSH2 - cy, bmy, bny), dc ? gz : hws - gz);
	selecting = false;
	if (paintme)
		paint();
	else
...
static auto getmaterial(cube& c) -> ushort {
	if (c.children) {
...
void mpeditface(int dir, int mode, selinfo& sel, bool local) {
	...
	loopselxyz(
		if (c.children) solidfaces(c); ushort mat = getmaterial(c); discardchildren(c, true); c.material = mat;
		if (mode == 1)	// fill command
		{
			if (dir < 0) {
				solidfaces(c);
				cube& o = blockcube(x, y, 1, sel, -sel.grid);
				loopi(6) c.texture[i] = o.children ? DEFAULT_GEOM : o.texture[i];
			} else
				emptyfaces(c);
		} else {
			uint bak = c.faces[d];
			auto* p = (uchar*)&c.faces[d];
...
static auto remapvslot(int index, bool delta, const VSlot& ds) -> VSlot* {
...
void edittexcube(cube& c, int tex, int orient, bool& findrep) {
	...
	if (c.children)
		loopi(8) edittexcube(c.children[i], tex, orient, findrep);
}
...
auto mpeditvslot(int delta, int allfaces, selinfo& sel, ucharbuf& buf) -> bool {
	VSlot ds;
...
static auto unpacktex(int& tex, ucharbuf& buf, bool insert = true) -> int {
	if (tex < 0x10000)
		return true;
	VSlot ds;
	if (!unpackvslot(buf, ds, false))
...
auto shouldpacktex(int index) -> int {
...
auto mpedittex(int tex, int allfaces, selinfo& sel, ucharbuf& buf) -> bool {
...
void edittex(int i, bool save = true) {
	...
	lasttexmillis = totalmillis;
	if (save) {
		loopvj(texmru) if (texmru[j] == lasttex) {
			curtexindex = j;
			break;
		}
	}
...
void replacetexcube(cube& c, int oldtex, int newtex) {
	...
	if (c.children)
		loopi(8) replacetexcube(c.children[i], oldtex, newtex);
}
...
auto mpreplacetex(int oldtex, int newtex, bool insel, selinfo& sel, ucharbuf& buf) -> bool {
...
void rotatecube(cube& c, int d)	 // rotates cube clockwise. see pics in cvs for help.
{
	...
	if (c.children) {
		...
		for (int i = 0; i <= octadim(d); i += octadim(d))
			rotatequad(c.children[i + row], c.children[i], c.children[i + col], c.children[i + col + row]);
...
void mprotate(int cw, selinfo& sel, bool local) {
	...
	loop(z, sel.s[D[d]]) loopi(cw > 0 ? 1 : 3) {
		...
		loop(y, ss / 2) loop(x, ss - 1 - y * 2) rotatequad(selcube(ss - 1 - y, x + y, z),
														   selcube(x + y, y, z),
...
static const struct {
	const char* name;
	int filter;
} editmatfilters[] = {{"empty", EDITMATF_EMPTY},
...
void mpeditmat(int matid, int filter, selinfo& sel, bool local) {
	...
	if (filter >= 0) {
		...
		filtermask = filtermat & (MATF_VOLUME | MATF_INDEX) ? MATF_VOLUME | MATF_INDEX
															: (filtermat & MATF_CLIP ? MATF_CLIP : filtermat);
		filtergeom = filter & ~0xFF;
	}
	if (matid < 0) {
		matid = 0;
		matmask = filtermask;
		if (isclipped(filtermat & MATF_VOLUME))
			matmask &= ~MATF_CLIP;
		if (isdeadly(filtermat & MATF_VOLUME))
			matmask &= ~MAT_DEATH;
	} else {
		matmask = matid & (MATF_VOLUME | MATF_INDEX) ? 0 : (matid & MATF_CLIP ? ~MATF_CLIP : ~matid);
		if (isclipped(matid & MATF_VOLUME))
			matid |= MAT_CLIP;
		if (isdeadly(matid & MATF_VOLUME))
			matid |= MAT_DEATH;
	}
...
void editmat(char* name, char* filtername) {
	...
	int filter = -1;
	if (filtername[0]) {
		loopi(sizeof(editmatfilters) / sizeof(editmatfilters[0])) if (!strcmp(editmatfilters[i].name, filtername)) {
			...
			if (texmru.inrange(ti)) {
				...
				loopj(glowtex ? 3 : 2) {
					...
					if (j == 1 && layertex) {
						...
						gle::attribf(x + r / 2, y + r / 2);
						gle::attrib(tc[0]);
						gle::attribf(x + r, y + r / 2);
						gle::attrib(tc[1]);
						gle::attribf(x + r / 2, y + r);
						gle::attrib(tc[3]);
						gle::attribf(x + r, y + r);
						gle::attrib(tc[2]);
						xtraverts += gle::end();
