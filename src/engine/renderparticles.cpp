...
struct particleemitter {
	...
	particleemitter(extentity* ent) : ent(ent), bbmin(ent->o), bbmax(ent->o), maxfade(-1), lastemit(0), lastcull(0) {
	}
	...
	void finalize() {
		...
		if (dbgpseed)
			conoutf(CON_DEBUG, "radius: %f, maxfade: %d", radius, maxfade);
	}
...
struct partrenderer {
	...
	partrenderer(int type, int stain = -1) : tex(NULL), texname(NULL), texclamp(0), type(type), stain(stain) {
	}
			...
			if (type & PT_COLLIDE && o.z < p->val && step) {
				if (stain >= 0) {
					...
					if (o.z >= collidez + COLLIDEERROR)
						...
						addstain(stain,
								 vec(o.x, o.y, collidez),
								 vec(p->o).sub(o).normalize(),
								 2 * p->size,
								 p->color,
								 type & PT_RND4 ? (p->flags >> 5) & 3 : 0);
						blend = 0;
					}
				} else
					blend = 0;
...
struct listrenderer : partrenderer {
	...
	listrenderer(int type, int stain = -1) : partrenderer(type, stain), list(NULL) {
	}
...
struct meterrenderer : listrenderer {
	meterrenderer(int type) : listrenderer(type | PT_NOTEX | PT_LERP | PT_NOLAYER) {
	}
...
struct textrenderer : listrenderer {
	textrenderer(int type = 0) : listrenderer(type | PT_TEXT | PT_LERP | PT_SHADER | PT_NOLAYER) {
	}
...
#define ROTCOEFFS(n)                                                                                              \
	{                                                                                                             \
		vec2(-1, 1).rotate_around_z(n * 2 * M_PI / 32.0f), vec2(1, 1).rotate_around_z(n * 2 * M_PI / 32.0f),      \
			vec2(1, -1).rotate_around_z(n * 2 * M_PI / 32.0f), vec2(-1, -1).rotate_around_z(n * 2 * M_PI / 32.0f) \
	}
static const vec2 rotcoeffs[32][4] = {
	ROTCOEFFS(0),  ROTCOEFFS(1),  ROTCOEFFS(2),	 ROTCOEFFS(3),	ROTCOEFFS(4),  ROTCOEFFS(5),  ROTCOEFFS(6),
	ROTCOEFFS(7),  ROTCOEFFS(8),  ROTCOEFFS(9),	 ROTCOEFFS(10), ROTCOEFFS(11), ROTCOEFFS(12), ROTCOEFFS(13),
	ROTCOEFFS(14), ROTCOEFFS(15), ROTCOEFFS(16), ROTCOEFFS(17), ROTCOEFFS(18), ROTCOEFFS(19), ROTCOEFFS(20),
	ROTCOEFFS(21), ROTCOEFFS(22), ROTCOEFFS(7),	 ROTCOEFFS(24), ROTCOEFFS(25), ROTCOEFFS(26), ROTCOEFFS(27),
	ROTCOEFFS(28), ROTCOEFFS(29), ROTCOEFFS(30), ROTCOEFFS(31),
};
...
struct varenderer : partrenderer {
	...
	particle* addpart(const vec& o, const vec& d, int fade, int color, float size, int gravity) {
		particle* p =
			parts + (numparts < maxparts ? numparts++ : rnd(maxparts));	 // next free slot, or kill a random kitten
		p->o = o;
		p->d = d;
		p->gravity = gravity;
		p->fade = fade;
...
struct softquadrenderer : quadrenderer {
	softquadrenderer(const char* texname, int type, int stain = -1) : quadrenderer(texname, type | PT_SOFT, stain) {
	}
};

static partrenderer* parts[] = {
	new quadrenderer("<grey>media/particle/blood.png",
					 PT_PART | PT_FLIP | PT_MOD | PT_RND4 | PT_COLLIDE,
					 STAIN_BLOOD),	// blood spats (note: rgb is inverted)
	...
	new quadrenderer("media/particle/rail_muzzle.png",
					 PT_PART | PT_FEW | PT_FLIP | PT_BRIGHT | PT_TRACK),  // rail muzzle flash
	new quadrenderer("media/particle/pulse_muzzle.png",
					 PT_PART | PT_FEW | PT_FLIP | PT_BRIGHT | PT_TRACK),  // pulse muzzle flash
...
void initparticles() {
	...
	loopi(sizeof(parts) / sizeof(parts[0])) parts[i]->init(parts[i]->type & PT_FEW ? min(fewparticles, maxparticles)
																				   : maxparticles);
...
void renderparticles(int layer) {
	...
	if (dbgparts && (layer == PL_ALL || layer == PL_UNDER))
		loopi(sizeof(parts) / sizeof(parts[0])) parts[i]->debuginfo();
...
newparticle(const vec& o, const vec& d, int fade, int type, int color, float size, int gravity = 0) {
...
static void splash(int type, int color, int radius, int num, int fade, const vec& p, float size, int gravity) {
	...
	loopi(num) {
		...
		int f = (num < 10)
			? (fmin + rnd(fmax))
			: (fmax -
			   (i * (fmax - fmin)) / (num - 1));  // help deallocater by using fade distribution rather than random
...
static void regularsplash(int type,
						  ...
						  int gravity,
						  int delay = 0) {
	if (!canemitparticles() || (delay > 0 && rnd(delay) != 0))
		return;
 ...
 */
static void regularshape(int type,
						 int radius,
						 int color,
						 int dir,
						 int num,
	...
	bool flare = (basetype == PT_TAPE) || (basetype == PT_LIGHTNING), inv = (dir & 0x20) != 0,
	...
	loopi(num) {
		...
		if (taper) {
			...
			if (dist > 0.2f) {
				...
				if (rnd(0x10000) > dist * dist * 0xFFFF)
					continue;
			}
		}
		...
		if (flare)
			...
			if (parts[type]->type & PT_COLLIDE)
				n->val =
					from.z -
					raycube(
						from, vec(0, 0, -1), parts[type]->stain >= 0 ? COLLIDERADIUS : max(from.z, 0.0f), RAY_CLIPMAT) +
...
static void makeparticles(entity& e) {
	...
	case 0:	 // fire and smoke -  <radius> <height> <rgb> - 0 values default to compat for old maps
	{
		...
		regularflame(PART_SMOKE,
					 vec(e.o.x, e.o.y, e.o.z + 4.0f * min(radius, height)),
					 radius,
					 height,
					 0x303020,
					 1,
	...
	case 2:	 // water fountain - <dir>
	{
		...
		if (e.attr3 > 0)
			...
			if (!color)
				color = getwatercolour(mat).tohexcolor();
		}
	...
	case 13:  // snow
	{
		static const int typemap[] = {
			PART_STREAK, -1, -1, PART_LIGHTNING, -1, PART_STEAM, PART_WATER, -1, -1, PART_SNOW};
		...
		if (e.attr2 >= 256)
			...
			newparticle(
				e.o, offsetvec(e.o, e.attr2, max(1 + e.attr3, 0)), 1, type, colorfromattr(e.attr4), size, gravity);
		break;
	}
	case 5:	 // meter, metervs - <percent> <rgb> <rgb2>
	case 6: {
		particle* p =
			newparticle(e.o, vec(0, 0, 1), 1, e.attr1 == 5 ? PART_METER : PART_METER_VS, colorfromattr(e.attr3), 2.0f);
	...
	case 11:  // flame <radius> <height> <rgb> - radius=100, height=100 is the classic size
		regularflame(
			PART_FLAME, e.o, float(e.attr2) / 100.0f, float(e.attr3) / 100.0f, colorfromattr(e.attr4), 3, 2.0f);
		break;
	case 12:  // smoke plume <radius> <height> <rgb>
		regularflame(PART_SMOKE,
					 e.o,
					 float(e.attr2) / 100.0f,
					 float(e.attr3) / 100.0f,
					 colorfromattr(e.attr4),
					 1,
					 4.0f,
					 100.0f,
...
bool printparticles(extentity& e, char* buf, int len) {
	...
	case 13:
		nformatstring(
			buf, len, "%s %d %d %d 0x%.3hX %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3, e.attr4, e.attr5);
		return true;
	case 3:
		nformatstring(
			buf, len, "%s %d %d 0x%.3hX %d %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3, e.attr4, e.attr5);
		return true;
	case 5:
	case 6:
		nformatstring(buf,
					  len,
					  "%s %d %d 0x%.3hX 0x%.3hX %d",
					  entities::entname(e.type),
					  e.attr1,
					  e.attr2,
					  e.attr3,
					  e.attr4,
...
void updateparticles() {
	...
	if (!editmode || showparticles) {
		...
		loopv(emitters) {
			...
			if (cullparticles && pe.maxfade >= 0) {
				...
				if (pvsoccluded(pe.cullmin, pe.cullmax)) {
					pe.lastcull = lastmillis;
					continue;
				}
			}
			...
			if (replayparticles && pe.maxfade > 5 && pe.lastcull > pe.lastemit) {
				for (emitoffset = max(pe.lastemit + emitmillis - lastmillis, -pe.maxfade); emitoffset < 0;
					 emitoffset += emitmillis) {
					makeparticles(e);
					replayed++;
				}
				emitoffset = 0;
	...
	{
		const vector<extentity*>& ents = entities::getents();
		// note: order matters in this case as particles of the same type are drawn in the reverse order that they are
		// added
		loopv(entgroup) {
