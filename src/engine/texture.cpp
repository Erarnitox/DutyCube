...
static void
scaletexture(uchar* RESTRICT src, uint sw, uint sh, uint bpp, uint pitch, uchar* RESTRICT dst, uint dw, uint dh) {
	if (sw == dw * 2 && sh == dh * 2) {
		switch (bpp) {
		case 1:
		...
		switch (bpp) {
		case 1:
			return scaletexture1(src, sw, sh, pitch, dst, dw, dh);
		case 2:
			return scaletexture2(src, sw, sh, pitch, dst, dw, dh);
		case 3:
			return scaletexture3(src, sw, sh, pitch, dst, dw, dh);
		case 4:
			return scaletexture4(src, sw, sh, pitch, dst, dw, dh);
		}
	} else {
		switch (bpp) {
		case 1:
			return shifttexture1(src, sw, sh, pitch, dst, dw, dh);
		case 2:
			return shifttexture2(src, sw, sh, pitch, dst, dw, dh);
		case 3:
			return shifttexture3(src, sw, sh, pitch, dst, dw, dh);
		case 4:
			return shifttexture4(src, sw, sh, pitch, dst, dw, dh);
		}
	}
}

static void reorientnormals(uchar* RESTRICT src,
							int sw,
							int sh,
							int bpp,
							int stride,
...
template <int BPP>
static inline void reorienttexture(uchar* RESTRICT src,
								   int sw,
								   int sh,
								   int stride,
								   uchar* RESTRICT dst,
...
static void reorienttexture(uchar* RESTRICT src,
							int sw,
							int sh,
							int bpp,
							int stride,
	...
	switch (bpp) {
	case 1:
		return reorienttexture<1>(src, sw, sh, stride, dst, flipx, flipy, swapxy);
	case 2:
		return reorienttexture<2>(src, sw, sh, stride, dst, flipx, flipy, swapxy);
	case 3:
		return reorienttexture<3>(src, sw, sh, stride, dst, flipx, flipy, swapxy);
	case 4:
		return reorienttexture<4>(src, sw, sh, stride, dst, flipx, flipy, swapxy);
	}
}
...
static void reorients3tc(GLenum format,
						 ...
						 bool swapxy,
						 bool normals = false) {
	int bx1 = 0, by1 = 0, bx2 = min(w, 4), by2 = min(h, 4), bw = (w + 3) / 4, bh = (h + 3) / 4, stridex = blocksize,
		stridey = blocksize;
	if (swapxy)
		stridex *= bw;
	else
		...
		for (uchar *curdst = dst, *end = &src[bw * blocksize]; src < end; src += blocksize, curdst += stridex) {
			if (format == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT) {
				...
				*(ullong*)curdst = lilswap(dalpha);
				...
				ullong salpha = lilswap(*(const ushort*)&src[2]) + ((ullong)lilswap(*(const uint*)&src[4]) << 16),
					   dalpha = 0;
			...
			if (normals) {
				...
				if (swapxy) {
					ncolor1 = (ncolor1 & 0x1F) | (((((ncolor1 >> 11) & 0x1F) * 0x3F) / 0x1F) << 5) |
						(((((ncolor1 >> 5) & 0x3F) * 0x1F) / 0x3F) << 11);
					ncolor2 = (ncolor2 & 0x1F) | (((((ncolor2 >> 11) & 0x1F) * 0x3F) / 0x1F) << 5) |
						(((((ncolor2 >> 5) & 0x3F) * 0x1F) / 0x3F) << 11);
				}
...
static void
reorientrgtc(GLenum format, int blocksize, int w, int h, uchar* src, uchar* dst, bool flipx, bool flipy, bool swapxy) {
	int bx1 = 0, by1 = 0, bx2 = min(w, 4), by2 = min(h, 4), bw = (w + 3) / 4, bh = (h + 3) / 4, stridex = blocksize,
		stridey = blocksize;
	if (swapxy)
		stridex *= bw;
	...
	loopi(bh) {
		for (uchar *curdst = dst, *end = &src[bw * blocksize]; src < end; curdst += stridex) {
			loopj(blocksize / 8) {
				...
				ullong sval = lilswap(*(const ushort*)&src[2]) + ((ullong)lilswap(*(const uint*)&src[4]) << 16),
					   dval = 0;
				uint xmask = flipx ? 7 : 0, ymask = flipy ? 7 : 0, xshift = 0, yshift = 2;
				if (swapxy)
					swap(xshift, yshift);
...
#define readwritetex(t, s, body)                                                             \
	do {                                                                                     \
		...
		loop(y, t.h) {                                                                       \
			for (uchar* dst = dstrow, *src = srcrow, *end = &srcrow[s.w * s.bpp]; src < end; \
				 dst += t.bpp, src += s.bpp) {                                               \
				body;                                                                        \
			}                                                                                \
			dstrow += t.pitch;                                                               \
...
#define read2writetex(t, s1, src1, s2, src2, body)                                                              \
	do {                                                                                                        \
		...
		loop(y, t.h) {                                                                                          \
			for (uchar* dst = dstrow, *end = &dstrow[t.w * t.bpp], *src1 = src1row, *src2 = src2row; dst < end; \
				 dst += t.bpp, src1 += s1.bpp, src2 += s2.bpp) {                                                \
				body;                                                                                           \
			}                                                                                                   \
			dstrow += t.pitch;                                                                                  \
			src1row += s1.pitch;                                                                                \
...
void texmad(ImageData& s, const vec& mul, const vec& add) {
	...
	writetex(s, loopk(maxk) dst[k] = uchar(clamp(dst[k] * mul[k] + 255 * add[k], 0.0f, 255.0f)););
}
...
void texcolorify(ImageData& s, const vec& color, vec weights) {
	...
	writetex(s, float lum = dst[0] * weights.x + dst[1] * weights.y + dst[2] * weights.z;
...
void texcolormask(ImageData& s, const vec& color1, const vec& color2) {
	if (s.bpp < 4)
		return;
	ImageData d(s.w, s.h, 3);
	readwritetex(d, s, vec color; color.lerp(color2, color1, src[3] / 255.0f);
...
void texgrey(ImageData& s) {
	...
	if (s.bpp >= 4) {
		readwritetex(d, s, dst[0] = src[0]; dst[1] = src[3];);
	} else {
...
void texpremul(ImageData& s) {
	switch (s.bpp) {
	case 2:
		writetex(s, dst[0] = uchar((uint(dst[0]) * uint(dst[1])) / 255););
		break;
	case 4:
		writetex(s, uint alpha = dst[3]; dst[0] = uchar((uint(dst[0]) * alpha) / 255);
...
void texagrad(ImageData& s, float x2, float y2, float x1, float y1) {
	...
	float dx = (maxx - minx) / max(s.w - 1, 1), dy = (maxy - miny) / max(s.h - 1, 1), cury = miny;
...
void texblend(ImageData& d, ImageData& s, ImageData& m) {
	...
	if (&s == &m) {
		if (s.bpp == 2) {
			...
			return;
		if (d.bpp < 3)
			readwritetex(d, s, int srcblend = src[1]; int dstblend = 255 - srcblend;
						 dst[0] = uchar((dst[0] * dstblend + src[0] * srcblend) / 255););
		else
			readwritetex(d, s, int srcblend = src[3]; int dstblend = 255 - srcblend;
						 dst[0] = uchar((dst[0] * dstblend + src[0] * srcblend) / 255);
		...
		if (s.bpp < 3) {
			...
			swizzleimage(d);
		if (d.bpp < 3)
			read2writetex(d, s, src, m, mask, int srcblend = mask[0]; int dstblend = 255 - srcblend;
						  dst[0] = uchar((dst[0] * dstblend + src[0] * srcblend) / 255););
		else
			read2writetex(d, s, src, m, mask, int srcblend = mask[0]; int dstblend = 255 - srcblend;
						  dst[0] = uchar((dst[0] * dstblend + src[0] * srcblend) / 255);
...
auto compressedformat(GLenum format, int w, int h, int force = 0) -> GLenum {
	if (usetexcompress && texcompress && force >= 0 && (force || max(w, h) >= texcompress))
		switch (format) {
		case GL_RGB5:
		case GL_RGB8:
		case GL_RGB:
		...
		case GL_RED:
		case GL_R8:
			return hasRGTC ? (usetexcompress > 1 ? GL_COMPRESSED_RED_RGTC1 : GL_COMPRESSED_RED)
						   : (usetexcompress > 1 ? GL_COMPRESSED_RGB_S3TC_DXT1_EXT : GL_COMPRESSED_RGB);
		case GL_RG:
		case GL_RG8:
			return hasRGTC ? (usetexcompress > 1 ? GL_COMPRESSED_RG_RGTC2 : GL_COMPRESSED_RG)
						   : (usetexcompress > 1 ? GL_COMPRESSED_RGBA_S3TC_DXT5_EXT : GL_COMPRESSED_RGBA);
		case GL_LUMINANCE:
		case GL_LUMINANCE8:
			return hasLATC ? (usetexcompress > 1 ? GL_COMPRESSED_LUMINANCE_LATC1_EXT : GL_COMPRESSED_LUMINANCE)
						   : (usetexcompress > 1 ? GL_COMPRESSED_RGB_S3TC_DXT1_EXT : GL_COMPRESSED_RGB);
		case GL_LUMINANCE_ALPHA:
		case GL_LUMINANCE8_ALPHA8:
			return hasLATC
				? (usetexcompress > 1 ? GL_COMPRESSED_LUMINANCE_ALPHA_LATC2_EXT : GL_COMPRESSED_LUMINANCE_ALPHA)
				: (usetexcompress > 1 ? GL_COMPRESSED_RGBA_S3TC_DXT5_EXT : GL_COMPRESSED_RGBA);
		}
	return format;
}

auto formatsize(GLenum format) -> int {
	switch (format) {
	case GL_RED:
	case GL_LUMINANCE:
	case GL_ALPHA:
...
auto textarget(GLenum subtarget) -> GLenum {
	switch (subtarget) {
...
auto uncompressedformat(GLenum format) -> GLenum {
	switch (format) {
...
auto swizzlemask(GLenum format) -> const GLint* {
...
void setuptexparameters(int tnum,
	...
	glTexParameteri(
		target, GL_TEXTURE_WRAP_S, clamp & 1 ? GL_CLAMP_TO_EDGE : (clamp & 0x100 ? GL_MIRRORED_REPEAT : GL_REPEAT));
	if (target != GL_TEXTURE_1D)
		glTexParameteri(
			target, GL_TEXTURE_WRAP_T, clamp & 2 ? GL_CLAMP_TO_EDGE : (clamp & 0x200 ? GL_MIRRORED_REPEAT : GL_REPEAT));
	if (target == GL_TEXTURE_3D)
		glTexParameteri(
			target, GL_TEXTURE_WRAP_R, clamp & 4 ? GL_CLAMP_TO_EDGE : (clamp & 0x400 ? GL_MIRRORED_REPEAT : GL_REPEAT));
	...
	glTexParameteri(target,
					...
					filter > 1 ? (trilinear ? (bilinear ? GL_LINEAR_MIPMAP_LINEAR : GL_NEAREST_MIPMAP_LINEAR)
...
static auto textype(GLenum& component, GLenum& format) -> GLenum {
...
static auto texformat(int bpp, bool swizzle = false) -> GLenum {
	switch (bpp) {
	case 1:
		return hasTRG && (hasTSW || !glcompat || !swizzle) ? GL_RED : GL_LUMINANCE;
	case 2:
		return hasTRG && (hasTSW || !glcompat || !swizzle) ? GL_RG : GL_LUMINANCE_ALPHA;
	case 3:
		return GL_RGB;
	case 4:
		return GL_RGBA;
...
static auto alphaformat(GLenum format) -> bool {
	switch (format) {
	case GL_ALPHA:
	case GL_LUMINANCE_ALPHA:
	case GL_RG:
...
auto texalign(const void* data, int w, int bpp) -> int {
	int stride = w * bpp;
	if (stride & 1)
		return 1;
	if (stride & 2)
...
auto floatformat(GLenum format) -> bool {
	switch (format) {
	case GL_R16F:
	case GL_R32F:
	case GL_RG16F:
...
static auto newtexture(Texture* t,
						   const char* rname,
						   ImageData& s,
						   int clamp = 0,
						   bool mipit = true,
	...
	if (s.compressed) {
		...
		createcompressedtexture(
			t->id, t->w, t->h, data, s.align, s.bpp, levels, clamp, filter, s.compressed, GL_TEXTURE_2D, swizzle);
	} else {
...
auto wrapsurface(void* data, int width, int height, int bpp) -> SDL_Surface* {
	switch (bpp) {
	case 3:
...
auto creatergbsurface(SDL_Surface* os) -> SDL_Surface* {
...
auto creatergbasurface(SDL_Surface* os) -> SDL_Surface* {
...
auto checkgrayscale(SDL_Surface* s) -> bool {
...
auto fixsurfaceformat(SDL_Surface* s) -> SDL_Surface* {
	if (!s)
		return nullptr;
	...
	case 3:
		...
		break;
	case 4:
		if (s->format->Rmask != rgbamasks[0] || s->format->Gmask != rgbamasks[1] || s->format->Bmask != rgbamasks[2] ||
...
static void blurtexture(int w, int h, uchar* dst, const uchar* src, int margin) {
	static const int weights3x3[9] = {0x10, 0x20, 0x10, 0x20, 0x40, 0x20, 0x10, 0x20, 0x10};
	static const int weights5x5[25] = {0x05, 0x05, 0x09, 0x05, 0x05, 0x05, 0x0A, 0x14, 0x0A, 0x05, 0x09, 0x14, 0x28,
									   0x14, 0x09, 0x05, 0x0A, 0x14, 0x0A, 0x05, 0x05, 0x05, 0x09, 0x05, 0x05};
	const int* mat = n > 1 ? weights5x5 : weights3x3;
	int mstride = 2 * n + 1, mstartoffset = n * (mstride + 1), stride = bpp * w, startoffset = n * bpp,
		nextoffset1 = stride + mstride * bpp, nextoffset2 = stride - mstride * bpp;
...
void blurtexture(int n, int bpp, int w, int h, uchar* dst, const uchar* src, int margin) {
	...
	case 0x13:
		blurtexture<1, 3, false>(w, h, dst, src, margin);
		break;
	case 0x23:
		blurtexture<2, 3, false>(w, h, dst, src, margin);
		break;
	case 0x14:
		blurtexture<1, 4, false>(w, h, dst, src, margin);
		break;
	case 0x24:
		blurtexture<2, 4, false>(w, h, dst, src, margin);
		break;
	}
}
...
void blurnormals(int n, int w, int h, bvec* dst, const bvec* src, int margin) {
	switch (clamp(n, 1, 2)) {
	case 1:
		blurtexture<1, 3, true>(w, h, dst->v, src->v, margin);
		break;
	case 2:
		blurtexture<2, 3, true>(w, h, dst->v, src->v, margin);
		break;
	}
}
...
auto canloadsurface(const char* name) -> bool {
...
auto loadsurface(const char* name) -> SDL_Surface* {
...
static auto parsevec(const char* arg) -> vec {
	vec v(0, 0, 0);
	int i = 0;
...
static auto texturedata(ImageData& d,
						const char* tname,
						bool msg = true,
						int* compress = nullptr,
						int* wrap = nullptr,
						const char* tdir = nullptr,
						int ttype = TEX_DIFFUSE) -> bool {
...
#define COPYTEXARG(dst, src) copystring(dst, stringslice(src, strcspn(src, ":,><")))
	...
	if (flen >= 4 && (!strcasecmp(file + flen - 4, ".dds") || (dds && !raw))) {
		...
		if (!loaddds(dfile, d, raw ? 1 : (dds ? 0 : -1)) && (!dds || raw)) {
			if (msg)
				conoutf(CON_ERROR, "could not load texture %s", dfile);
			return false;
		}
	...
	if (!d.data) {
		...
		if (!s) {
			if (msg)
				conoutf(CON_ERROR, "could not load texture %s", file);
			return false;
		}
		...
		if (max(s->w, s->h) > (1 << 12)) {
			SDL_FreeSurface(s);
			conoutf(CON_ERROR, "texture size exceeded %dx%d pixels: %s", 1 << 12, 1 << 12, file);
			return false;
		}
		d.wrap(s);
	}
	...
	while (cmds) {
		...
		if (matchstring(cmd, len, "mad"))
			...
			if (srcname[0] && texturedata(src, srcname, false, nullptr, nullptr, tdir, ttype) &&
				(!maskname[0] || texturedata(mask, maskname, false, nullptr, nullptr, tdir, ttype)))
...
static inline auto texturedata(ImageData& d,
							   Slot& slot,
							   Slot::Tex& tex,
							   bool msg = true,
							   int* compress = nullptr,
							   int* wrap = nullptr) -> bool {
	return texturedata(d, tex.name, msg, compress, wrap, slot.texturedir(), tex.type);
}

auto loadalphamask(Texture* t) -> uchar* {
	if (t->alphamask)
		return t->alphamask;
...
auto textureload(const char* name, int clamp, bool mipit, bool msg) -> Texture* {
	string tname;
...
auto settexture(const char* name, int clamp) -> bool {
...
void compactvslots(cube* c, int n) {
	...
	loopi(n) {
		if (c[i].children)
			...
			loopj(6) if (vslots.inrange(c[i].texture[j])) {
...
auto compactvslots(bool cull) -> int {
	defslot = nullptr;
	clonedvslots = 0;
	markingvslots = cull;
	compactedvslots = 0;
	...
	loopv(vslots) {
		...
		if (vs.index >= 0) {
			if (vs.layer && vslots.inrange(vs.layer))
				vs.layer = vslots[vs.layer]->index;
			if (vs.detail && vslots.inrange(vs.detail))
				vs.detail = vslots[vs.detail]->index;
		}
	}
	if (cull) {
...
static void propagatevslot(VSlot& dst, const VSlot& src, int diff, bool edit = false) {
	if (diff & (1 << VSLOT_SHPARAM))
		loopv(src.params) dst.params.add(src.params[i]);
...
static auto reassignvslot(Slot& owner, VSlot* vs) -> VSlot* {
	vs->reset();
	owner.variants = vs;
	while (vs) {
		vs->slot = &owner;
...
static auto emptyvslot(Slot& owner) -> VSlot* {
	int offset = 0;
	loopvrev(slots) if (slots[i]->variants) {
...
auto Slot::emptyvslot() -> VSlot& {
	return *::emptyvslot(*this);
}

static auto comparevslot(const VSlot& dst, const VSlot& src, int diff) -> bool {
	if (diff & (1 << VSLOT_SHPARAM)) {
	...
	if (diff & (1 << VSLOT_ROTATION) && dst.rotation != src.rotation)
		return false;
	if (diff & (1 << VSLOT_OFFSET) && dst.offset != src.offset)
		return false;
	if (diff & (1 << VSLOT_SCROLL) && dst.scroll != src.scroll)
		return false;
	if (diff & (1 << VSLOT_LAYER) && dst.layer != src.layer)
		return false;
	if (diff & (1 << VSLOT_ALPHA) && (dst.alphafront != src.alphafront || dst.alphaback != src.alphaback))
		return false;
	if (diff & (1 << VSLOT_COLOR) && dst.colorscale != src.colorscale)
		return false;
	if (diff & (1 << VSLOT_REFRACT) && (dst.refractscale != src.refractscale || dst.refractcolor != src.refractcolor))
		return false;
...
auto unpackvslot(ucharbuf& buf, VSlot& dst, bool delta) -> bool {
	while (buf.remaining()) {
		int changed = buf.get();
		if (changed >= 0x80)
			break;
		...
		case VSLOT_ROTATION:
			...
			if (!delta)
				dst.rotation = clamp(dst.rotation, 0, 5);
			break;
		case VSLOT_OFFSET:
...
auto findvslot(Slot& slot, const VSlot& src, const VSlot& delta) -> VSlot* {
	for (VSlot* dst = slot.variants; dst; dst = dst->next) {
		if ((!dst->changed || dst->changed == (src.changed | delta.changed)) &&
			comparevslot(*dst, src, src.changed & ~delta.changed) && comparevslot(*dst, delta, delta.changed))
			return dst;
	}
	return nullptr;
}

static auto clonevslot(const VSlot& src, const VSlot& delta) -> VSlot* {
...
auto editvslot(const VSlot& src, const VSlot& delta) -> VSlot* {
...
static void fixinsidefaces(cube* c, const ivec& o, int size, int tex) {
	loopi(8) {
		...
		if (c[i].children)
			...
			loopj(6) if (!visibletris(c[i], j, co, size)) c[i].texture[j] = tex;
	}
}
...
auto findslottex(const char* name) -> int {
...
void texture(char* type, char* name, int* rot, int* xoffset, int* yoffset, float* scale) {
	...
	s.texmask |= 1 << tnum;
	if (s.sts.length() >= 8)
		conoutf(CON_WARN, "warning: too many textures in %s", s.name());
...
static void addglow(ImageData& c, ImageData& g, const vec& glowcolor) {
	if (g.bpp < 3) {
		readwritergbtex(c, g, loopk(3) dst[k] = clamp(int(dst[k]) + int(src[0] * glowcolor[k]), 0, 255););
	} else {
		readwritergbtex(c, g, loopk(3) dst[k] = clamp(int(dst[k]) + int(src[k] * glowcolor[k]), 0, 255););
	}
}

static void mergespec(ImageData& c, ImageData& s) {
	if (s.bpp < 3) {
		readwritergbatex(c, s, dst[3] = src[0];);
	} else {
		readwritergbatex(c, s, dst[3] = (int(src[0]) + int(src[1]) + int(src[2])) / 3;);
	}
}
...
static void collapsespec(ImageData& s) {
	...
	if (s.bpp >= 3)
		readwritetex(d, s, { dst[0] = (int(src[0]) + int(src[1]) + int(src[2])) / 3; });
	else
		readwritetex(d, s, { dst[0] = src[0]; });
	s.replace(d);
}

auto Slot::findtextype(int type, int last) const -> int {
...
auto Slot::cancombine(int type) const -> int {
	switch (type) {
	case TEX_DIFFUSE:
		return TEX_SPEC;
	case TEX_NORMAL:
...
auto DecalSlot::cancombine(int type) const -> int {
	switch (type) {
	case TEX_GLOW:
		return TEX_SPEC;
	case TEX_NORMAL:
...
auto lookupmaterialslot(int index, bool load) -> MatSlot& {
...
auto lookupslot(int index, bool load) -> Slot& {
...
auto lookupvslot(int index, bool load) -> VSlot& {
...
auto lookupdecalslot(int index, bool load) -> DecalSlot& {
...
auto Slot::loadthumbnail() -> Texture* {
	if (thumbnail)
		return thumbnail;
	if (!variants) {
		thumbnail = notexture;
	...
	if (vslot.colorscale == vec(1, 1, 1))
		...
		defformatstring(
			prefix, "<thumbnail:%.2f/%.2f/%.2f>", vslot.colorscale.x, vslot.colorscale.y, vslot.colorscale.z);
		addname(name, *this, sts[0], false, prefix);
	}
	int glow = -1;
	if (texmask & (1 << TEX_GLOW)) {
		loopvj(sts) if (sts[j].type == TEX_GLOW) {
			glow = j;
			break;
		}
		if (glow >= 0) {
	...
	if (layer) {
		if (layer->colorscale == vec(1, 1, 1))
			...
			defformatstring(
				prefix, "<layer:%.2f/%.2f/%.2f>", layer->colorscale.x, layer->colorscale.y, layer->colorscale.z);
			addname(name, *layer->slot, layer->slot->sts[0], true, prefix);
		}
	}
	VSlot* detail = vslot.detail ? &lookupvslot(vslot.detail, false) : nullptr;
	if (detail)
		addname(name, *detail->slot, detail->slot->sts[0], true, "<detail>");
	name.add('\0');
	...
	if (t)
		...
		if (layer)
			texturedata(l, *layer->slot, layer->slot->sts[0], false);
		if (detail)
			texturedata(d, *detail->slot, detail->slot->sts[0], false);
		if (!s.data)
...
auto cubemaploadwildcard(Texture* t, const char* name, bool mipit, bool msg, bool transient = false) -> Texture* {
	string tname;
	if (!name)
		copystring(tname, t->name);
	else {
		copystring(tname, name);
	...
	loopi(6) {
		...
		if (s.w != s.h) {
			if (msg)
				conoutf(CON_ERROR, "cubemap texture %s does not have square size", sname);
			return nullptr;
		}
		if (s.compressed ? s.compressed != surface[0].compressed || s.w != surface[0].w || s.h != surface[0].h ||
					s.levels != surface[0].levels
						 : surface[0].compressed || s.bpp != surface[0].bpp) {
			if (msg)
	...
	loopi(6) {
		...
		if (s.compressed) {
			...
			createcompressedtexture(
				!i ? t->id : 0, w, h, data, s.align, s.bpp, levels, 3, mipit ? 2 : 1, s.compressed, side.target, true);
		} else {
			createtexture(!i ? t->id : 0,
						  t->w,
						  t->h,
						  s.data,
						  3,
...
auto cubemapload(const char* name, bool mipit, bool msg, bool transient) -> Texture* {
	string pname;
	...
	if (!strchr(pname, '*')) {
		...
		if (!t) {
			...
			if (!t && msg)
				conoutf(CON_ERROR, "could not load envmap %s", name);
		}
	} else
		t = cubemaploadwildcard(nullptr, pname, mipit, msg, transient);
	return t;
}
...
auto genenvmap(const vec& o, int envmapsize, int blur, bool onlysky) -> GLuint {
	int rendersize = 1 << (envmapsize + aaenvmap), sizelimit = min(hwcubetexsize, min(gw, gh));
	if (maxtexsize)
		sizelimit = min(sizelimit, maxtexsize);
	while (rendersize > sizelimit)
		rendersize /= 2;
...
auto closestenvmap(const vec& o) -> ushort {
...
auto closestenvmap(int orient, const ivec& co, int size) -> ushort {
	vec loc(co);
...
static inline auto lookupskyenvmap() -> GLuint {
	return envmaps.length() && envmaps[0].radius < 0 ? envmaps[0].tex : 0;
}

auto lookupenvmap(Slot& slot) -> GLuint {
	loopv(slot.sts) if (slot.sts[i].type == TEX_ENVMAP && slot.sts[i].t) return slot.sts[i].t->id;
	return lookupskyenvmap();
}

auto lookupenvmap(ushort emid) -> GLuint {
	if (emid == EMID_SKY || emid == EMID_CUSTOM || drawtex)
		return lookupskyenvmap();
	if (emid == EMID_NONE || !envmaps.inrange(emid - EMID_RESERVED))
		return 0;
...
auto reloadtexture(const char* name) -> bool {
...
auto reloadtexture(Texture& tex) -> bool {
	if (tex.id)
		return true;
	...
	case Texture::IMAGE: {
		int compress = 0;
		ImageData s;
		if (!texturedata(s, tex.name, true, &compress) ||
			!newtexture(&tex, nullptr, s, tex.clamp, tex.mipmap, false, false, compress))
			return false;
		break;
	}

	case Texture::CUBEMAP:
		if (!cubemaploadwildcard(&tex, nullptr, tex.mipmap, true))
			return false;
		break;
	}
	return true;
...
struct DDPIXELFORMAT {
	...
	union {
		uint dwRGBBitCount, dwYUVBitCount, dwZBufferBitDepth, dwAlphaBitDepth, dwLuminanceBitCount, dwBumpBitCount,
			dwPrivateFormatBitCount;
	};
	union {
		uint dwRBitMask, dwYBitMask, dwStencilBitDepth, dwLuminanceBitMask, dwBumpDuBitMask, dwOperations;
	};
	union {
		uint dwGBitMask, dwUBitMask, dwZBitMask, dwBumpDvBitMask;
		struct {
			ushort wFlipMSTypes, wBltMSTypes;
		} MultiSampleCaps;
	};
	union {
		uint dwBBitMask, dwVBitMask, dwStencilBitMask, dwBumpLuminanceBitMask;
	};
	union {
		uint dwRGBAlphaBitMask, dwYUVAlphaBitMask, dwLuminanceAlphaBitMask, dwRGBZBitMask, dwYUVZBitMask;
	};
};
struct DDSCAPS2 {
	uint dwCaps, dwCaps2, dwCaps3, dwCaps4;
};
struct DDSURFACEDESC2 {
	...
	union {
		uint dwMipMapCount, dwRefreshRate, dwSrcVBHandle;
	};
...
DECODEDDS(
	decodedxt1, s.compressed == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ? 4 : 3, ushort color0 = lilswap(*(const ushort*)src);
...
DECODEDDS(decodedxt3, 4, ullong alpha = lilswap(*(const ullong*)src); ushort color0 = lilswap(*(const ushort*)&src[8]);
...
DECODEDDS(decodedxt5, 4, uchar alpha[8]; decodealpha(src[0], src[1], alpha);
...
DECODEDDS(decodergtc1, 1, uchar red[8]; decodealpha(src[0], src[1], red);
		  ullong redbits = lilswap(*(const ushort*)&src[2]) + ((ullong)lilswap(*(const uint*)&src[4]) << 16);
		  , dst[0] = red[redbits & 7];
		  , redbits >>= 3;);

DECODEDDS(decodergtc2, 2, uchar red[8]; decodealpha(src[0], src[1], red);
		  ullong redbits = lilswap(*(const ushort*)&src[2]) + ((ullong)lilswap(*(const uint*)&src[4]) << 16);
		  uchar green[8];
...
auto loaddds(const char* filename, ImageData& image, int force) -> bool {
	...
	if (d.ddpfPixelFormat.dwFlags & DDPF_FOURCC) {
		...
		case FOURCC_DXT1:
			if ((supported = hasS3TC) || force)
				format = d.ddpfPixelFormat.dwFlags & DDPF_ALPHAPIXELS ? GL_COMPRESSED_RGBA_S3TC_DXT1_EXT
		...
		case FOURCC_DXT5:
			...
			break;
		case FOURCC_ATI1:
			if ((supported = hasRGTC) || force)
				format = GL_COMPRESSED_RED_RGTC1;
			else if ((supported = hasLATC))
...
void gendds(char* infile, char* outfile) {
	if (!hasS3TC || usetexcompress <= 1) {
		conoutf(CON_ERROR, "OpenGL driver does not support S3TC texture compression");
		return;
	}
	...
	case GL_COMPRESSED_RG_RGTC2:
		...
		break;
	default:
		conoutf(CON_ERROR, "failed compressing %s: unknown format: 0x%X", infile, format);
		break;
		return;
	}
	...
	if (!f) {
		conoutf(CON_ERROR, "failed writing to %s", outfile);
		return;
	}
...
void savetga(const char* filename, ImageData& image, bool flip) {
	...
	case 4:
		break;
	default:
		conoutf(CON_ERROR, "failed saving tga to %s", filename);
		return;
	}
		...
		for (int remaining = image.w; remaining > 0;) {
			...
			if (compresstga) {
				...
				for (uchar* scan = src; run < min(remaining, 128); run++) {
					scan += image.bpp;
					if (src[0] != scan[0] || src[1] != scan[1] || src[2] != scan[2] ||
						(image.bpp == 4 && src[3] != scan[3]))
						break;
				}
				if (run > 1) {
...
enum { IMG_BMP = 0, IMG_TGA = 1, IMG_PNG = 2, NUMIMG };

VARP(screenshotformat, 0, IMG_PNG, NUMIMG - 1);

const char* imageexts[NUMIMG] = {".bmp", ".tga", ".png"};

auto guessimageformat(const char* filename, int format = IMG_BMP) -> int {
...
void saveimage(const char* filename, int format, ImageData& image, bool flip = false) {
	switch (format) {
	case IMG_PNG:
		savepng(filename, image, flip);
		break;
	case IMG_TGA:
		savetga(filename, image, flip);
		break;
	default: {
...
auto loadimage(const char* filename, ImageData& image) -> bool {
...
void screenshot(char* filename) {
	...
	if (screenshotdir[0]) {
		dirlen = strlen(buf);
		if (buf[dirlen] != '/' && buf[dirlen] != '\\' && dirlen + 1 < (int)sizeof(buf)) {
			buf[dirlen++] = '/';
			buf[dirlen] = '\0';
		}
...
{
	ImageData ns;
	if (!loadimage(normalfile, ns))
		return;
	ImageData d(ns.w, ns.h, 3);
	readwritetex(d, ns, dst[0] = src[0]; dst[1] = 255 - src[1]; dst[2] = src[2];);
...
{
	ImageData hs, ns;
	if (!loadimage(heightfile, hs) || !loadimage(normalfile, ns) || hs.w != ns.w || hs.h != ns.h)
		return;
	ImageData d(ns.w, ns.h, 3);
	read2writetex(d,
				  hs,
...
void normalizenormalmap(char* destfile, char* normalfile)  // jpg/png/tga-> tga
{
	...
	if (!loadimage(normalfile, ns))
		return;
	ImageData d(ns.w, ns.h, 3);
	readwritetex(d, ns, *(bvec*)dst = bvec(src[0], src[1], src[2]).normalize(););
	saveimage(destfile, guessimageformat(destfile, IMG_TGA), d);
}

void removealphachannel(char* destfile, char* rgbafile) {
	ImageData ns;
	if (!loadimage(rgbafile, ns))
		return;
	ImageData d(ns.w, ns.h, 3);
	readwritetex(d, ns, dst[0] = src[0]; dst[1] = src[1]; dst[2] = src[2];);
	saveimage(destfile, guessimageformat(destfile, IMG_TGA), d);
}
