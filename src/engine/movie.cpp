...
struct avisegmentinfo {
	...
	avisegmentinfo() = default;
	avisegmentinfo(stream::offset offset, int firstindex)
		: offset(offset),
		  videoindexoffset(0),
		  soundindexoffset(0),
...
struct aviwriter {
	...
	void close() {
		...
		if (dbgmovie)
			conoutf(CON_DEBUG,
					"fileframes: sound=%d, video=%d+%d(dups)\n",
					soundframes,
					videoframes,
					indexframes - videoframes);
	...
	aviwriter(const char* name, uint w, uint h, uint fps, bool sound) : videow(w & ~1), videoh(h & ~1), videofps(fps) {
		copystring(filename, moviedir);
		if (moviedir[0]) {
			int dirlen = strlen(filename);
			if (filename[dirlen] != '/' && filename[dirlen] != '\\' && dirlen + 1 < (int)sizeof(filename)) {
				filename[dirlen++] = '/';
				filename[dirlen] = '\0';
			}
		...
		if (sound && !nosound) {
			...
			if (dbgmovie)
				conoutf(CON_DEBUG, "soundspec: %dhz %s x %d", soundfrequency, desc, soundchannels);
		}
	}
	...
	auto open() -> bool {
		...
		listchunk("LIST", "INFO");
		const char* software = "DropEngine";
	...
	static inline void boxsample(const uchar* src,
								 const uint stride,
								 const uint area,
								 const uint w,
								 uint h,
	...
	void scaleyuv(const uchar* pixels, uint srcw, uint srch) {
		...
		const uint wfrac = (srcw << 12) / videow, hfrac = (srch << 12) / videoh,
				   area = ((ullong)planesize << 12) / (srcw * srch + 1), dw = videow * wfrac, dh = videoh * hfrac;

		for (uint y = 0; y < dh;) {
			uint yn = y + hfrac - 1, yi = y >> 12, h = (yn >> 12) - yi,
				 ylow = ((yn | (-int(h) >> 24)) & 0xFFFU) + 1 - (y & 0xFFFU), yhigh = (yn & 0xFFFU) + 1;
			y += hfrac;
			uint y2n = y + hfrac - 1, y2i = y >> 12, h2 = (y2n >> 12) - y2i,
				 y2low = ((y2n | (-int(h2) >> 24)) & 0xFFFU) + 1 - (y & 0xFFFU), y2high = (y2n & 0xFFFU) + 1;
			y += hfrac;

			const uchar *src = &pixels[yi * stride], *src2 = &pixels[y2i * stride];
			uchar *ydst = yplane, *ydst2 = yplane + ystride, *udst = uplane, *vdst = vplane;
			for (uint x = 0; x < dw;) {
				uint xn = x + wfrac - 1, xi = x >> 12, w = (xn >> 12) - xi,
					 xlow = ((w + 0xFFFU) & 0x1000U) - (x & 0xFFFU), xhigh = (xn & 0xFFFU) + 1;
				x += wfrac;
				uint x2n = x + wfrac - 1, x2i = x >> 12, w2 = (x2n >> 12) - x2i,
					 x2low = ((w2 + 0xFFFU) & 0x1000U) - (x & 0xFFFU), x2high = (x2n & 0xFFFU) + 1;
				x += wfrac;
				...
				*ydst2++ = ((16 << 12) + 1052 * r4 + 2065 * g4 + 401 * b4) >> 12;
				...
				const uint b = b1 + b2 + b3 + b4, g = g1 + g2 + g3 + g4, r = r1 + r2 + r3 + r4;
	...
	void encodeyuv(const uchar* pixels) {
		...
		while (src < yend) {
			...
			uchar *ydst = yplane, *ydst2 = yplane + ystride, *udst = uplane, *vdst = vplane;
			while (src < xend) {
				const uint b1 = src[0], g1 = src[1], r1 = src[2], b2 = src[4], g2 = src[5], r2 = src[6], b3 = src2[0],
						   g3 = src2[1], r3 = src2[2], b4 = src2[4], g4 = src2[5], r4 = src2[6];
	...
	auto writesound(uchar* data, uint framesize, uint frame) -> bool {
		// do conversion in-place to little endian format
		// note that xoring by half the range yields the same bit pattern as subtracting the range regardless of
		// signedness
		// ... so can toggle signedness just by xoring the high byte with 0x80
		switch (soundformat) {
		case AUDIO_U8:
			for (uchar *dst = data, *end = &data[framesize]; dst < end; dst++)
				*dst ^= 0x80;
			break;
		case AUDIO_S8:
			break;
		case AUDIO_U16LSB:
			for (uchar *dst = &data[1], *end = &data[framesize]; dst < end; dst += 2)
				*dst ^= 0x80;
			break;
		case AUDIO_U16MSB:
			for (auto *dst = (ushort*)data, *end = (ushort*)&data[framesize]; dst < end; dst++)
		...
		if (totalsize - segments.last().offset + framesize > 1000 * 1000 * 1000 && !nextsegment())
			return false;
	...
	auto nextsegment() -> bool {
		if (segments.length()) {
			if (segments.length() >= MAX_SUPER_INDEX)
				return false;
			flushsegment();
			listchunk("RIFF", "AVIX");
		}
	...
	auto writevideoframe(const uchar* pixels, uint srcw, uint srch, int format, uint frame) -> bool {
		if (frame < videoframes)
			return true;

		switch (format) {
		case VID_RGB:
			if (srcw != videow || srch != videoh)
				scaleyuv(pixels, srcw, srch);
			else
				encodeyuv(pixels);
			break;
		case VID_YUV:
		...
		while (videoframes <= frame)
			addindex(videoframes++, 0, framesize);
...
auto calcquality() -> float {
	return 1.0f - float(dps) / float(dps + file->videofps);	 // strictly speaking should lock to read dps - 1.0=perfect,
															 // 0.5=half of frames are beingdropped
}
...
auto videoencoder(void* data) -> int  // runs on a separate thread
{
	for (int numvid = 0, numsound = 0;;) {
		...
		while (videobuffers.empty() && state == REC_OK)
			SDL_CondWait(shouldencode, videolock);
		if (state != REC_OK) {
		...
		if (calcquality() < movieminquality)
			state = REC_TOOSLOW;
		else if (!file->writevideoframe(m.video, m.w, m.h, m.format, m.frame))
			state = REC_FILERROR;
...
void start(const char* filename, int videofps, int videow, int videoh, bool sound) {
	...
	if (videofps > fps)
		conoutf(CON_WARN, "frame rate may be too low to capture at %d fps", videofps);
	...
	conoutf("movie recording to: %s %dx%d @ %dfps%s",
			file->filename,
			file->videow,
			file->videoh,
			file->videofps,
	...
	if (file->soundfrequency > 0)
		Mix_SetPostMix(soundencoder, nullptr);
}
...
void readbuffer(videobuffer& m, uint nextframe) {
	bool accelyuv = movieaccelyuv && !(m.w % 8),
		 usefbo = movieaccel && file->videow <= (uint)screenw && file->videoh <= (uint)screenh &&
		(accelyuv || file->videow < (uint)screenw || file->videoh < (uint)screenh);
	...
	if (usefbo) {
		...
		if (accelyuv && (!encodefb || !encoderb)) {
			if (!encodefb)
				glGenFramebuffers_(1, &encodefb);
			glBindFramebuffer_(GL_FRAMEBUFFER, encodefb);
			if (!encoderb)
				glGenRenderbuffers_(1, &encoderb);
		...
		if (accelyuv) {
			glBindFramebuffer_(GL_FRAMEBUFFER, encodefb);
			glBindTexture(GL_TEXTURE_RECTANGLE, scaletex[0]);
			glViewport(0, 0, m.w / 4, m.h);
			SETSHADER(moviey);
			screenquadflipped(m.w, m.h);
			glViewport(m.w / 4, 0, m.w / 8, m.h / 2);
			SETSHADER(movieu);
			screenquadflipped(m.w, m.h);
			glViewport(m.w / 4, m.h / 2, m.w / 8, m.h / 2);
			SETSHADER(moviev);
			screenquadflipped(m.w, m.h);
			...
			glReadPixels(
				m.w / 4, m.h / 2, m.w / 8, m.h / 2, GL_BGRA, GL_UNSIGNED_BYTE, &m.video[planesize + planesize / 4]);
...
auto readbuffer() -> bool {
	...
	SDL_LockMutex(videolock);
	if (moviesync && videobuffers.full())
		SDL_CondWait(shouldread, videolock);
	uint nextframe = (max(gettime() - starttime, 0) * file->videofps) / 1000;
