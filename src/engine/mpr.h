// This code is based off the Minkowski Portal Refinement algorithm by Gary Snethen in XenoCollide & Game Programming
// Gems 7.
...
struct EntOBB : Ent {
	...
	vec contactface(const vec& wn, const vec& wdir) const {
		vec n = orient.transform(wn).div(vec(ent->xradius, ent->yradius, (ent->aboveeye + ent->eyeheight) / 2)),
			dir = orient.transform(wdir), an(fabs(n.x), fabs(n.y), dir.z ? fabs(n.z) : 0), fn(0, 0, 0);
		if (an.x > an.y) {
			if (an.x > an.z)
	...
	float left() const {
		return supportcoordneg(orient.a) + ent->o.x;
	}
	float right() const {
		return supportcoord(orient.a) + ent->o.x;
	}
	float back() const {
		return supportcoordneg(orient.b) + ent->o.y;
	}
	float front() const {
		return supportcoord(orient.b) + ent->o.y;
	}
	float bottom() const {
		return ent->o.z - ent->eyeheight;
	}
	float top() const {
		return ent->o.z + ent->aboveeye;
	}
};
...
struct EntFuzzy : Ent {
	...
	float left() const {
		return ent->o.x - ent->radius;
	}
	float right() const {
		return ent->o.x + ent->radius;
	}
	float back() const {
		return ent->o.y - ent->radius;
	}
	float front() const {
		return ent->o.y + ent->radius;
	}
		...
		for (int j = 0;; j++) {
			...
			if (n.dot(v1) >= 0 && !hit) {
				...
				if (contactpoint1 || contactpoint2) {
					float b0 = v3.scalartriple(v1, v2), b1 = v0.scalartriple(v3, v2), b2 = v3.scalartriple(v0, v1),
						  b3 = v0.scalartriple(v2, v1), sum = b0 + b1 + b2 + b3;
					if (sum <= 0) {
						b0 = 0;
					...
					if (contactpoint1)
						*contactpoint1 =
							(vec(v01).mul(b0).add(vec(v11).mul(b1)).add(vec(v21).mul(b2)).add(vec(v31).mul(b3)))
								.mul(1.0f / sum);
					if (contactpoint2)
						*contactpoint2 =
							(vec(v02).mul(b0).add(vec(v12).mul(b1)).add(vec(v22).mul(b2)).add(vec(v32).mul(b3)))
								.mul(1.0f / sum);
				}
