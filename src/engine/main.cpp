...
Texture* s_backgroundmapshot = nullptr;
string s_backgroundmapname = "";
char* s_backgroundmapinfo = nullptr;
...
static auto filterevent(const SDL_Event& event) -> bool {
	switch (event.type) {
...
static inline auto pollevent(SDL_Event& event) -> bool {
...
static auto findarg(int argc, char** argv, const char* str) -> bool {
...
static void setbackgroundinfo(const char* caption = nullptr,
...
static void restorebackground(int w, int h, bool force = false) {
	...
	renderbackgroundview(w,
						 h,
						 s_backgroundcaption[0] ? s_backgroundcaption : nullptr,
						 s_backgroundmapshot,
						 s_backgroundmapname[0] ? s_backgroundmapname : nullptr,
						 s_backgroundmapinfo);
}
...
static void stackdumper(unsigned int type, EXCEPTION_POINTERS* ep) {
	...
	char out[512];
	formatstring(out,
				 "DropEngine Win32 Exception: 0x%x [0x%x]\n\n",
				 er->ExceptionCode,
...
void fatal(const char* s, ...)	// failure exit
{
	...
	if (errors <= 2)  // print up to one extra recursive error
	{
		...
		if (errors <= 1)  // avoid recursion
		{
			...
			SDL_Quit();
			SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "DropEngine fatal error", msg, nullptr);
		}
	}
...
auto initwarning(const char* desc, int level, int type) -> bool {
	if (initing < level) {
		addchange(desc, type);
		return true;
	}
...
void resetgl() {
	...
	if (!reloadtexture(*notexture) || !reloadtexture("<premul>media/interface/logo_512.png") ||
		!reloadtexture("<premul>media/interface/logo_1024.png") ||
		!reloadtexture("<premul>media/interface/logo_1500.png") || !reloadtexture("media/interface/background.png") ||
...
auto interceptkey(int sym) -> bool {
