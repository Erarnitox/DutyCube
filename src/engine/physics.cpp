...
static inline bool pointinbox(const vec& v, const vec& bo, const vec& br) {
	return v.x <= bo.x + br.x && v.x >= bo.x - br.x && v.y <= bo.y + br.y && v.y >= bo.y - br.y && v.z <= bo.z + br.z &&
		v.z >= bo.z - br.z;
}

bool pointincube(const clipplanes& p, const vec& v) {
	if (!pointinbox(v, p.o, p.r))
		return false;
...
#define INTERSECTBOX(setentry, exit)                                                                         \
	loop(i, 3) {                                                                                             \
		if (ray[i]) {                                                                                        \
			float prad = fabs(p.r[i] * invray[i]), pdist = (p.o[i] - v[i]) * invray[i], pmin = pdist - prad, \
				  pmax = pdist + prad;                                                                       \
			if (pmin > enterdist) {                                                                          \
				if (pmin > exitdist)                                                                         \
					exit;                                                                                    \
...
static inline bool raycubeintersect(const clipplanes& p,
									const cube& c,
									const vec& v,
									const vec& ray,
									const vec& invray,
...
#define entintersect(type, func)                                                         \
	do {                                                                                 \
		loopv(oc->type) {                                                                \
			extentity& e = *ents[oc->type[i]];                                           \
			if (!(e.flags & EF_OCTA) || &e == t)                                         \
				continue;                                                                \
			func;                                                                        \
	...
	if ((mode & RAY_POLY) == RAY_POLY)
		entintersect(mapmodels, {
			if (!mmintersect(e, o, ray, radius, mode, f))
				continue;
		});

#define entselintersect(type)                            \
	entintersect(type, {                                 \
	...
	for (;;) {
		...
		if ((dist > 0 || !(mode & RAY_SKIPFIRST)) &&
			(((mode & RAY_CLIPMAT) && isclipped(c.material & MATF_VOLUME)) ||
			 ((mode & RAY_EDITMAT) && c.material != MAT_AIR) || (!(mode & RAY_PASS) && lsize == size && !isempty(c)) ||
			 isentirelysolid(c) || dent < dist) &&
		...
		if (!isempty(c)) {
			...
			float f = 0;
			if (raycubeintersect(p, c, v, ray, invray, dent - dist, f) && (dist + f > 0 || !(mode & RAY_SKIPFIRST)) &&
				(!(mode & RAY_CLIPMAT) || (c.material & MATF_CLIP) != MAT_NOCLIP))
...
float shadowray(const vec& o, const vec& ray, float radius, int mode, extentity* t) {
	...
	for (;;) {
		...
		if (!isempty(c) && !(c.material & MAT_ALPHA)) {
			...
			if (exitdist >= 0)
				return c.texture[side] == DEFAULT_SKY && mode & RAY_SKIPSKY ? radius
																			: dist + max(enterdist + 0.1f, 0.0f);
		}
...
bool ellipseboxcollide(physent* d,
	...
	if (dist < 0) {
		int sx = yo.x <= -xr ? -1 : (yo.x >= xr ? 1 : 0), sy = yo.y <= -yr ? -1 : (yo.y >= yr ? 1 : 0);
		...
		if (yo.z < 0) {
			if (dir.iszero() ||
				(dir.z > 0 && (d->type != ENT_PLAYER || below >= d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f))) {
				collidewall = vec(0, 0, -1);
				return true;
			}
		} else if (dir.iszero() ||
				   (dir.z < 0 &&
					(d->type != ENT_PLAYER || above >= d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f))) {
			collidewall = vec(0, 0, 1);
			return true;
		}
		collideinside++;
...
void cleardynentcache() {
	...
	if (!dynentframe || dynentframe == 1)
		loopi(DYNENTCACHESIZE) dynentcache[i].frame = 0;
	if (!dynentframe)
		dynentframe = 1;
}
...
const vector<physent*>& checkdynentcache(int x, int y) {
	...
	loopi(numdyns) {
		...
		if (d->state != CS_ALIVE || d->o.x + d->radius <= dx || d->o.x - d->radius >= dx + dsize ||
...
#define loopdynentcache(curx, cury, o, radius)                               \
	for (int curx = max(int(o.x - radius), 0) >> dynentsize,                 \
			 endx = min(int(o.x + radius), worldsize - 1) >> dynentsize;     \
		 curx <= endx;                                                       \
		 curx++)                                                             \
		for (int cury = max(int(o.y - radius), 0) >> dynentsize,             \
				 endy = min(int(o.y + radius), worldsize - 1) >> dynentsize; \
			 cury <= endy;                                                   \
			 cury++)
...
static inline bool plcollide(physent* d, const vec& dir, physent* o) {
	...
	case COLLIDE_ELLIPSE:
		if (o->collidetype == COLLIDE_ELLIPSE)
			return ellipsecollide(
				d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye, o->eyeheight);
		else
			return ellipseboxcollide(
				d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye, o->eyeheight);
	case COLLIDE_OBB:
		if (o->collidetype == COLLIDE_ELLIPSE)
...
{
	if (d->type == ENT_CAMERA || d->state != CS_ALIVE)
		return false;
...
static inline bool mmcollide(physent* d,
							 const vec& dir,
							 const extentity& e,
							 const vec& center,
							 const vec& radius,
...
template <class E>
static bool fuzzycollidebox(physent* d,
							const vec& dir,
							float cutoff,
							const vec& o,
							const vec& center,
	...
	loopi(6) {
		...
		if (!dir.iszero()) {
			...
			if (d->type == ENT_PLAYER &&
				dist < (dir.z * w.z < 0 ? d->zmargin - (d->eyeheight + d->aboveeye) / (dir.z < 0 ? 3.0f : 4.0f)
...
template <class E>
static bool fuzzycollideellipse(physent* d,
								const vec& dir,
								float cutoff,
								const vec& o,
								const vec& center,
...
bool mmcollide(physent* d, const vec& dir, float cutoff, octaentities& oc)	// collide with a mapmodel
{
	...
	loopv(oc.mapmodels) {
		extentity& e = *ents[oc.mapmodels[i]];
		if (e.flags & EF_NOCOLLIDE || !mapmodels.inrange(e.attr1))
			continue;
...
static bool fuzzycollidesolid(physent* d,
							  ...
							  const cube& c,
							  const ivec& co,
							  int size)	 // collide with solid cube geometry
{
	int crad = size / 2;
...
#define CHECKSIDE(side, distval, dotval, margin, normal)                  \
		...
		do {                                                              \
			...
			if (!dir.iszero()) {                                          \
				if (dotval >= -cutoff * dir.magnitude())                  \
					continue;                                             \
				if (d->type == ENT_PLAYER && dotval < 0 && dist < margin) \
					continue;                                             \
			}                                                             \
			collidewall = normal;                                         \
			bestdist = dist;                                              \
	...
	CHECKSIDE(O_BOTTOM,
			  co.z - (d->o.z + d->aboveeye),
			  -dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f,
			  vec(0, 0, -1));
	CHECKSIDE(O_TOP,
			  d->o.z - d->eyeheight - (co.z + size),
			  dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
...
static bool fuzzycollideplanes(physent* d,
							   ...
							   const cube& c,
							   const ivec& co,
							   int size)  // collide with deformed cube geometry
{
	...
	CHECKSIDE(O_BOTTOM,
			  p.o.z - p.r.z - (d->o.z + d->aboveeye),
			  -dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f,
			  vec(0, 0, -1));
	CHECKSIDE(O_TOP,
			  d->o.z - d->eyeheight - (p.o.z + p.r.z),
			  dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
							 ...
							 int size)	// collide with solid cube geometry
{
	...
	CHECKSIDE(
		O_TOP, entvol.bottom() - (co.z + size), dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f, vec(0, 0, 1));
							  ...
							  int size)	 // collide with deformed cube geometry
{
	...
	CHECKSIDE(O_BOTTOM,
			  p.o.z - p.r.z - entvol.top(),
			  -dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f,
			  vec(0, 0, -1));
	CHECKSIDE(O_TOP,
			  entvol.bottom() - (p.o.z + p.r.z),
			  dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
...
static inline bool
cubecollide(physent* d, const vec& dir, float cutoff, const cube& c, const ivec& co, int size, bool solid) {
	switch (d->collidetype) {
	case COLLIDE_OBB:
...
static inline bool octacollide(physent* d,
							   const vec& dir,
							   float cutoff,
							   const ivec& bo,
							   const ivec& bs,
	...
	loopoctabox(cor, size, bo, bs) {
		...
		if (c[i].children) {
			...
			case MAT_NOCLIP:
				continue;
			case MAT_CLIP:
				if (isclipped(c[i].material & MATF_VOLUME) || d->type == ENT_PLAYER)
					solid = true;
				break;
			}
			if (!solid && isempty(c[i]))
				continue;
...
static inline bool octacollide(physent* d, const vec& dir, float cutoff, const ivec& bo, const ivec& bs) {
	int diff = (bo.x ^ bs.x) | (bo.y ^ bs.y) | (bo.z ^ bs.z), scale = worldscale - 1;
	...
	case MAT_NOCLIP:
		return false;
	case MAT_CLIP:
		if (isclipped(c->material & MATF_VOLUME) || d->type == ENT_PLAYER)
			solid = true;
		break;
	}
	if (!solid && isempty(*c))
...
void switchfloor(physent* d, vec& dir, const vec& floor) {
	...
	oldvel.add(d->falling);
	if (dir.dot(floor) >= 0) {
		if (d->physstate < PHYS_SLIDE || fabs(dir.dot(d->floor)) > 0.01f * dir.magnitude())
			return;
...
bool trystepup(physent* d, vec& dir, const vec& obstacle, float maxstep, const vec& floor) {
	vec old(d->o),
		stairdir = (obstacle.z >= 0 && obstacle.z < SLOPEZ ? vec(-obstacle.x, -obstacle.y, 0) : vec(dir.x, dir.y, 0))
					   .rescale(1);
	bool cansmooth = true;
...
bool trystepdown(physent* d, vec& dir, bool init = false) {
	...
	if (trystepdown(d, dir, step, 4, 1, init))
		return true;
#else
	if (trystepdown(d, dir, step, 2, 1, init))
		return true;
	if (trystepdown(d, dir, step, 1, 1, init))
		return true;
	if (trystepdown(d, dir, step, 1, 2, init))
		return true;
#endif
	return false;
}
...
bool findfloor(physent* d, const vec& dir, bool collided, const vec& obstacle, bool& slide, vec& floor) {
	...
	if (collide(d, vec(0, 0, -1), d->physstate == PHYS_SLOPE || d->physstate == PHYS_STEP_DOWN ? SLOPEZ : FLOORZ)) {
		if (d->physstate == PHYS_STEP_UP && d->floor != collidewall) {
			vec old(d->o), checkfloor(collidewall),
				checkdir = vec(dir).projectxydir(checkfloor).rescale(dir.magnitude());
			d->o.add(checkdir);
			if (!collide(d, checkdir)) {
				floor = checkfloor;
				found = true;
...
bool move(physent* d, vec& dir) {
	...
	if (collide(d, dir)) {
		 ...
		 */
		if (d->type == ENT_PLAYER &&
			((collidewall.z >= SLOPEZ && dir.z < 0) || (collidewall.z <= -SLOPEZ && dir.z > 0)) && (dir.x || dir.y) &&
			collide(d, vec(dir.x, dir.y, 0))) {
		...
		if (d->physstate == PHYS_SLOPE || d->physstate == PHYS_FLOOR ||
			(collide(d, vec(0, 0, -1), SLOPEZ) &&
			 (d->physstate == PHYS_STEP_UP || d->physstate == PHYS_STEP_DOWN || collidewall.z >= FLOORZ))) {
			d->o = old;
			d->zmargin = 0;
			if (trystepup(d,
						  dir,
						  obstacle,
						  STAIRHEIGHT,
						  d->physstate == PHYS_SLOPE || d->physstate == PHYS_FLOOR ? d->floor : vec(collidewall)))
				return true;
		} else {
			d->o = old;
			d->zmargin = 0;
	...
	vec floor(0, 0, 0);
	bool slide = collided, found = findfloor(d, dir, collided, obstacle, slide, floor);
...
void crouchplayer(physent* pl, int moveres, bool local) {
	...
	if (pl->crouching < 0) {
		...
		pl->crouching = 0;
		loopi(moveres) {
			if (!collide(pl, vec(0, 0, pl->physstate <= PHYS_FALL ? -1 : 1), 0, true))
				break;
			pl->crouching = 1;
			pl->eyeheight -= step;
			if (pl->physstate > PHYS_FALL) {
...
bool moveplayer(physent* pl, int moveres, bool local, int curtime) {
	...
	if (pl->strafe && maxroll)
		pl->roll = clamp(pl->roll -
							 pow(clamp(1.0f + pl->strafe * pl->roll / maxroll, 0.0f, 1.0f), 0.33f) * pl->strafe *
								 curtime * straferoll,
						 -maxroll,
						 maxroll);
	else
	...
	if (pl->state == CS_ALIVE && (pl->o.z < 0 || material & MAT_DEATH))
		game::suicide(pl);
...
void updatephysstate(physent* d) {
	...
	if (d->physstate > PHYS_FALL && d->floor.z <= 0)
		d->floor = vec(0, 0, 1);
	d->o = old;
}

#define dir(name, v, d, s, os)                               \
	ICOMMAND(name, "D", (int* down), {                       \
		player->s = *down != 0;                              \
