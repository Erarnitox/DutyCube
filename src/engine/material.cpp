...
struct QuadNode {
	...
	QuadNode(int x, int y, int size) : x(x), y(y), size(size) {
		loopi(4) child[i] = nullptr;
	}
...
const struct material {
	...
	{"air", MAT_AIR},		   {"water", MAT_WATER},	  {"water1", MAT_WATER},	  {"water2", MAT_WATER + 1},
	{"water3", MAT_WATER + 2}, {"water4", MAT_WATER + 3}, {"glass", MAT_GLASS},		  {"glass1", MAT_GLASS},
	{"glass2", MAT_GLASS + 1}, {"glass3", MAT_GLASS + 2}, {"glass4", MAT_GLASS + 3},  {"lava", MAT_LAVA},
	{"lava1", MAT_LAVA},	   {"lava2", MAT_LAVA + 1},	  {"lava3", MAT_LAVA + 2},	  {"lava4", MAT_LAVA + 3},
	{"clip", MAT_CLIP},		   {"noclip", MAT_NOCLIP},	  {"gameclip", MAT_GAMECLIP}, {"death", MAT_DEATH},
	{"nogi", MAT_NOGI},		   {"alpha", MAT_ALPHA}};

auto findmaterial(const char* name) -> int {
...
auto findmaterialname(int mat) -> const char* {
	loopi(sizeof(materials) / sizeof(materials[0])) if (materials[i].id == mat) return materials[i].name;
	return nullptr;
}

auto getmaterialdesc(int mat, const char* prefix) -> const char* {
...
auto visiblematerial(const cube& c, int orient, const ivec& co, int size, ushort matmask) -> int {
...
static inline auto mergematcmp(const materialsurface& x, const materialsurface& y) -> bool {
...
static auto mergematr(materialsurface* m, int sz, materialsurface& n) -> int {
...
static auto mergematc(materialsurface& m, materialsurface& n) -> int {
	int dim = dimension(n.orient), c = C[dim], r = R[dim];
...
static auto mergemat(materialsurface* m, int sz, materialsurface& n) -> int {
...
static auto mergemats(materialsurface* m, int sz) -> int {
...
static inline auto optmatcmp(const materialsurface& x, const materialsurface& y) -> bool {
...
auto optimizematsurfs(materialsurface* matbuf, int matsurfs) -> int {
	...
	while (cur < end) {
		...
		while (cur < end && cur->material == start->material && cur->orient == start->orient &&
			   cur->visible == start->visible && cur->o[dim] == start->o[dim])
			++cur;
		...
		loopj(va->matsurfs) {
			...
			if (isliquid(matvol) && m.orient != O_BOTTOM && m.orient != O_TOP) {
				...
				while (o[dim ^ 1] < maxc) {
					...
					if (isliquid(c.material & MATF_VOLUME)) {
						m.ends |= 1;
						break;
					}
					o[dim ^ 1] += csize;
				...
				while (o[dim ^ 1] < maxc) {
					cube& c = lookupcube(o, 0, co, csize);
					if (visiblematerial(c, O_TOP, co, csize)) {
						m.ends |= 2;
						break;
					}
					o[dim ^ 1] += csize;
...
static inline auto editmatcmp(const materialsurface& x, const materialsurface& y) -> bool {
...
auto findmaterials() -> int {
	...
	for (vtxarray* va = visibleva; va; va = va->next) {
		if (!va->matsurfs || va->occluded >= OCCLUDE_BB || va->curvfc >= VFC_FOGGED)
			continue;
		...
		if (va->lavamin.x <= va->lavamax.x && calcbbscissor(va->lavamin, va->lavamax, sx1, sy1, sx2, sy2)) {
			...
			loopi(va->matsurfs) {
				...
				if ((m.material & MATF_VOLUME) != MAT_LAVA || m.visible == MATSURF_EDIT_ONLY) {
					i += m.skip;
					continue;
				}
				hasmats |= 1;
				if (m.orient == O_TOP)
					lavasurfs[m.material & MATF_INDEX].put(&m, 1 + int(m.skip));
				else
					lavafallsurfs[m.material & MATF_INDEX].put(&m, 1 + int(m.skip));
				i += m.skip;
			}
		}
		if (va->watermin.x <= va->watermax.x && calcbbscissor(va->watermin, va->watermax, sx1, sy1, sx2, sy2)) {
			...
			loopi(va->matsurfs) {
				materialsurface& m = va->matbuf[i];
				if ((m.material & MATF_VOLUME) != MAT_WATER || m.visible == MATSURF_EDIT_ONLY) {
					i += m.skip;
					continue;
				}
				hasmats |= 4 | 1;
				if (m.orient == O_TOP)
					watersurfs[m.material & MATF_INDEX].put(&m, 1 + int(m.skip));
				else
					waterfallsurfs[m.material & MATF_INDEX].put(&m, 1 + int(m.skip));
				i += m.skip;
			}
		}
		if (drawtex != DRAWTEX_ENVMAP && va->glassmin.x <= va->glassmax.x &&
			calcbbscissor(va->glassmin, va->glassmax, sx1, sy1, sx2, sy2)) {
			...
			loopi(va->matsurfs) {
				materialsurface& m = va->matbuf[i];
				if ((m.material & MATF_VOLUME) != MAT_GLASS) {
					i += m.skip;
					continue;
				}
				hasmats |= 4 | 2;
...
void rendermaterialmask() {
	...
	loopk(4) {
		...
		loopv(surfs) drawmaterial(surfs[i], 0.1f);
	}
	loopk(4) {
		vector<materialsurface>& surfs = watersurfs[k];
		loopv(surfs) drawmaterial(surfs[i], WATER_OFFSET);
	}
	loopk(4) {
