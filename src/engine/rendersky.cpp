...
Texture *sky[6] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr}, *clouds[6] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
...
void loadsky(const char* basename, Texture* texs[6]) {
	...
	loopi(6) {
		...
		if (texs[i] == notexture)
			conoutf(CON_ERROR, "could not load side %s of sky texture %s", side, basename);
	}
}

Texture* cloudoverlay = nullptr;

auto loadskyoverlay(const char* basename) -> Texture* {
	...
	if (t == notexture)
		conoutf(CON_ERROR, "could not load sky overlay texture %s", basename);
	return t;
}
...
void drawenvbox(Texture** sky = nullptr, float z1clip = 0.0f, float z2clip = 1.0f, int faces = 0x3F) {
	...
	if (faces & 0x01)
		drawenvboxface(1.0f, v2, -w, -w, z2, 0.0f, v2, -w, w, z2, 0.0f, v1, -w, w, z1, 1.0f, v1, -w, -w, z1, sky[0]);

	if (faces & 0x02)
		drawenvboxface(0.0f, v1, w, -w, z1, 1.0f, v1, w, w, z1, 1.0f, v2, w, w, z2, 0.0f, v2, w, -w, z2, sky[1]);

	if (faces & 0x04)
		drawenvboxface(0.0f, v1, -w, -w, z1, 1.0f, v1, w, -w, z1, 1.0f, v2, w, -w, z2, 0.0f, v2, -w, -w, z2, sky[2]);

	if (faces & 0x08)
		drawenvboxface(0.0f, v1, w, w, z1, 1.0f, v1, -w, w, z1, 1.0f, v2, -w, w, z2, 0.0f, v2, w, w, z2, sky[3]);

	if (z1clip <= 0 && faces & 0x10)
		drawenvboxface(
			1.0f, 1.0f, -w, w, -w, 1.0f, 0.0f, w, w, -w, 0.0f, 0.0f, w, -w, -w, 0.0f, 1.0f, -w, -w, -w, sky[4]);

	if (z2clip >= 1 && faces & 0x20)
		drawenvboxface(1.0f, 1.0f, w, w, w, 1.0f, 0.0f, -w, w, w, 0.0f, 0.0f, -w, -w, w, 0.0f, 1.0f, w, -w, w, sky[5]);
}
...
static void init(const bvec& color,
	...
	if (clipz >= 1) {
		...
		loopi(hres << depth) verts[numverts++] =
			vert(vec(sincos360[(360 * i) / (hres << depth)], 0.0f), color, maxalpha);
	} else {
		...
		loopi(hres) {
			...
			verts[numverts++] =
				vert(vec(sc.x * scm.x - sc.y * scm.y, sc.y * scm.x + sc.x * scm.y, 0.0f), color, maxalpha);
		}
		loopi(hres) {
...
void draw() {
	...
	if (!numverts || lastcolor != color || lastminalpha != fogdomemin || lastmaxalpha != fogdomemax ||
		lastcapsize != capsize || lastclipz != fogdomeclip) {
	...
	glDrawRangeElements_(
		GL_TRIANGLES, 0, numverts - 1, numindices + fogdomecap * capindices, GL_UNSIGNED_SHORT, indices);
	xtraverts += numverts;
	glde++;
...
static void drawatmosphere() {
	...
	LOCALPARAM(
		sunlight,
		(!atmosunlight.iszero() ? atmosunlight.tocolor().mul(atmosunlightscale) : sunlight.tocolor().mul(sunlightscale))
			.mul(atmobright * ldrscale));
	...
	LOCALPARAMF(
		atmoradius, planetradius, atmoradius * atmoradius, atmoradius * atmoradius - planetradius * planetradius);

	float gm = (1 - atmohaze) * 0.2f + 0.75f;
	LOCALPARAMF(gm, gm);

	vec lambda(680e-9f, 550e-9f, 450e-9f), betar = vec(lambda).square().square().recip().mul(1.86e-31f / atmodensity),
										   betam = vec(lambda)
													   .recip()
													   .mul(2 * M_PI)
													   .square()
													   .mul(atmohazefade.tocolor().mul(atmohazefadescale))
													   .mul(1.36e-19f * max(atmohaze, 1e-3f)),
...
void drawskybox(bool clear) {
	...
	if (skybox[0]) {
		if (ldrscale < 1 && (skyboxoverbrightmin != 1 || (skyboxoverbright > 1 && skyboxoverbrightthreshold < 1))) {
			...
			LOCALPARAMF(overbrightparams,
						skyboxoverbrightmin,
						max(skyboxoverbright, skyboxoverbrightmin),
	...
	if (cloudlayer[0] && cloudheight) {
		...
		drawenvoverlay(cloudoverlay,
					   cloudoffsetx + cloudscrollx * lastmillis / 1000.0f,
					   cloudoffsety + cloudscrolly * lastmillis / 1000.0f);
