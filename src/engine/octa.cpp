...
auto growcubeext(cubeext* old, int maxverts) -> cubeext* {
	auto* ext = (cubeext*)new uchar[sizeof(cubeext) + maxverts * sizeof(vertinfo)];
	if (old) {
		ext->va = old->va;
		ext->ents = old->ents;
...
auto newcubeext(cube& c, int maxverts, bool init) -> cubeext* {
...
auto newcubes(uint face, int mat) -> cube* {
	cube* c = new cube[8];
	loopi(8) {
		c->children = nullptr;
		c->ext = nullptr;
...
auto familysize(const cube& c) -> int {
	int size = 1;
	if (c.children)
		loopi(8) size += familysize(c.children[i]);
	return size;
}
...
void getcubevector(cube& c, int d, int x, int y, int z, ivec& p) {
	...
	loopi(3) p[i] = edgeget(cubeedge(c, i, v[R[i]], v[C[i]]), v[D[i]]);
}

void setcubevector(cube& c, int d, int x, int y, int z, const ivec& p) {
	ivec v(d, x, y, z);

	loopi(3) edgeset(cubeedge(c, i, v[R[i]], v[C[i]]), v[D[i]], p[i]);
}
...
auto isvalidcube(const cube& c) -> bool {
	clipplanes p;
...
ivec lu;
int lusize;
auto lookupcube(const ivec& to, int tsize, ivec& ro, int& rsize) -> cube& {
	int tx = clamp(to.x, 0, worldsize - 1), ty = clamp(to.y, 0, worldsize - 1), tz = clamp(to.z, 0, worldsize - 1);
...
auto lookupmaterial(const vec& v) -> int {
	ivec o(v);
	if (!insideworld(o))
		return MAT_AIR;
...
auto neighbourcube(const cube& c, int orient, const ivec& co, int size, ivec& ro, int& rsize) -> const cube& {
	ivec n = co;
...
auto getmippedtexture(const cube& p, int orient) -> int {
	cube* c = p.children;
...
void forcemip(cube& c, bool fixtex) {
	...
	if (fixtex)
		loopj(6) c.texture[j] = getmippedtexture(c, j);
}

static auto midedge(const ivec& a, const ivec& b, int xd, int yd, bool& perfect) -> int {
...
static inline auto crosscenter(const ivec& a, const ivec& b, int xd, int yd) -> bool {
	int ax = a[xd], ay = a[yd], bx = b[xd], by = b[yd];
	return (((ax <= 8 && bx <= 8) || (ax >= 8 && bx >= 8)) && ((ay <= 8 && by <= 8) || (ay >= 8 && by >= 8))) ||
		(ax + bx == 16 && ay + by == 16);
}

auto subdividecube(cube& c, bool fullcheck, bool brighten) -> bool {
	if (c.children)
		return true;
	if (c.ext)
	...
	loopj(6) {
		...
		const ivec &v00 = v[octaindex(d, 0, 0, z)], &v10 = v[octaindex(d, 1, 0, z)], &v01 = v[octaindex(d, 0, 1, z)],
	...
	validatec(ch);
	if (fullcheck)
		loopi(8) if (!isvalidcube(ch[i]))  // not so good...
		{
			emptyfaces(ch[i]);
			perfect = false;
		}
...
auto crushededge(uchar e, int dc) -> bool {
	return dc ? e == 0 : e == 0x88;
}

auto visibleorient(const cube& c, int orient) -> int {
	loopi(2) {
		int a = faceedgesidx[orient][i * 2 + 0];
		int b = faceedgesidx[orient][i * 2 + 1];
		loopj(2) {
			if (crushededge(c.edges[a], j) && crushededge(c.edges[b], j) && touchingface(c, orient))
				return ((a >> 2) << 1) + j;
		}
	}
	return orient;
...
auto remip(cube& c, const ivec& co, int size) -> bool {
	cube* ch = c.children;
	if (!ch) {
		if (size << 1 <= 0x1000)
			return true;
		...
		renderprogress(float(remipprogress) / remiptotal, "remipping...");
	...
	loopj(6) c.texture[j] = getmippedtexture(c, j);	 // parents get child texs regardless
	...
	loopi(8) {
		...
		if ((mat & MATF_CLIP) == MAT_NOCLIP || mat & MAT_ALPHA) {
			...
			while (++i < 8) {
				...
				if (isentirelysolid(ch[i]) ? (omat & MATF_CLIP) == MAT_NOCLIP || omat & MAT_ALPHA : mat != omat)
					return false;
			}
			break;
		}
	...
	loopi(8) {
		if (ch[i].faces[0] != nh[i].faces[0] || ch[i].faces[1] != nh[i].faces[1] || ch[i].faces[2] != nh[i].faces[2]) {
			freeocta(nh);
			return false;
		}

		if (isempty(ch[i]) && isempty(nh[i]))
			continue;

		ivec o(i, co, size);
		loop(orient, 6) if (visibleface(ch[i], orient, o, size, MAT_AIR, (mat & MAT_ALPHA) ^ MAT_ALPHA, MAT_ALPHA)) {
...
#define GENCUBEVERT(n, x, y, z)                                                                                      \
	case n:                                                                                                          \
		v = T(edgeget(cubeedge(c, 0, y, z), x), edgeget(cubeedge(c, 1, z, x), y), edgeget(cubeedge(c, 2, x, y), z)); \
		break;
...
#define GENFACEVERT(o, n, x, y, z, xv, yv, zv) \
	v[n] = ivec(edgeget(cubeedge(c, 0, y, z), x), edgeget(cubeedge(c, 1, z, x), y), edgeget(cubeedge(c, 2, x, y), z));
...
const uchar fvmasks[64] =  // mask of verts used given a mask of visible face orientations
	{
		0x00, 0x66, 0x99, 0xFF, 0xF0, 0xF6, 0xF9, 0xFF, 0x0F, 0x6F, 0x9F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xC3, 0xE7, 0xDB, 0xFF, 0xF3, 0xF7, 0xFB, 0xFF, 0xCF, 0xEF, 0xDF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0x3C, 0x7E, 0xBD, 0xFF, 0xFC, 0xFE, 0xFD, 0xFF, 0x3F, 0x7F, 0xBF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
auto flataxisface(const cube& c, int orient) -> bool {
...
auto collideface(const cube& c, int orient) -> bool {
...
auto faceconvexity(const vertinfo* verts, int numverts, int size) -> int {
	if (numverts < 4)
		return 0;
	ivec v0 = verts[0].getxyz(), e1 = verts[1].getxyz().sub(v0), e2 = verts[2].getxyz().sub(v0), n;
	if (size >= (8 << 5)) {
		if (size >= (8 << 10))
...
auto faceconvexity(const ivec v[4], int& vis) -> int {
	ivec e1, e2, e3, n;
...
auto faceconvexity(const cube& c, int orient) -> int {
	if (flataxisface(c, orient))
		return 0;
	ivec v[4];
...
auto faceorder(const cube& c, int orient) -> int  // gets above 'fv' so that each face is convex
{
...
auto faceedges(const cube& c, int orient) -> uint {
	union {
		uchar edges[4];
		uint face;
	} u;
...
static inline auto
genfacevecs(const cube& cu, int orient, const ivec& pos, int size, bool solid, ivec2* fvecs, const ivec* v = nullptr) -> int {
	int i = 0;
	if (solid) {
		switch (orient) {
...
static inline auto clipfacevecy(const ivec2& o, const ivec2& dir, int cx, int cy, int size, ivec2& r) -> int {
	if (dir.x >= 0) {
...
static inline auto clipfacevecx(const ivec2& o, const ivec2& dir, int cx, int cy, int size, ivec2& r) -> int {
	if (dir.y >= 0) {
...
static inline auto clipfacevec(const ivec2& o, const ivec2& dir, int cx, int cy, int size, ivec2* rvecs) -> int {
	int r = 0;

	if (o.x >= cx && o.x <= cx + size && o.y >= cy && o.y <= cy + size &&
...
static inline auto insideface(const ivec2* p, int nump, const ivec2* o, int numo) -> bool {
	int bounds = 0;
	ivec2 prev = o[numo - 1];
	loopi(numo) {
		const ivec2& cur = o[i];
...
static inline auto clipfacevecs(const ivec2* o, int numo, int cx, int cy, int size, ivec2* rvecs) -> int {
	cx <<= 3;
	cy <<= 3;
	size <<= 3;
...
auto collapsedface(const cube& c, int orient) -> bool {
	int e0 = c.edges[faceedgesidx[orient][0]], e1 = c.edges[faceedgesidx[orient][1]],
		e2 = c.edges[faceedgesidx[orient][2]], e3 = c.edges[faceedgesidx[orient][3]], face = dimension(orient) * 4,
		f0 = c.edges[face + 0], f1 = c.edges[face + 1], f2 = c.edges[face + 2], f3 = c.edges[face + 3];
	if (dimcoord(orient)) {
		f0 >>= 4;
		f1 >>= 4;
		f2 >>= 4;
	...
	ivec v0(e0 & 0xF, e2 & 0xF, f0), v1(e0 >> 4, e3 & 0xF, f1), v2(e1 >> 4, e3 >> 4, f3), v3(e1 & 0xF, e2 >> 4, f2);
	return ivec().cross(v1.sub(v0), v2.sub(v0)).iszero() && ivec().cross(v2, v3.sub(v0)).iszero();
}

static inline auto occludesface(const cube& c,
								int orient,
								const ivec& o,
								int size,
								const ivec& vo,
	...
	if (!c.children) {
		...
		if (isentirelysolid(c))
			return true;
		if (vmat != MAT_AIR &&
			((c.material & matmask) == vmat || (isliquid(vmat) && isclipped(c.material & MATF_VOLUME))))
			return true;
		...
		if (numc < 3)
			return true;
		if (isempty(c) || notouchingface(c, orient))
			return false;
		ivec2 of[4];
...
auto visibleface(const cube& c, int orient, const ivec& co, int size, ushort mat, ushort nmat, ushort matmask) -> bool {
	if (mat != MAT_AIR) {
	...
	if (nsize > size || (nsize == size && !o.children)) {
		if (nmat != MAT_AIR && (o.material & matmask) == nmat)
			return true;
		if (isentirelysolid(o))
			return false;
		if (mat != MAT_AIR &&
			((o.material & matmask) == mat || (isliquid(mat) && (o.material & MATF_VOLUME) == MAT_GLASS)))
			return false;
		if (isempty(o) || notouchingface(o, opp))
			return true;
...
auto classifyface(const cube& c, int orient, const ivec& co, int size) -> int {
	if (collapsedface(c, orient))
		return 0;
	...
	if (nsize > size || (nsize == size && !o.children)) {
		if ((~c.material & o.material) & MAT_ALPHA)
			vis |= 1;
		if ((o.material & MATF_CLIP) == MAT_NOCLIP)
			vis |= vismask & 2;
		if (vis == vismask || isentirelysolid(o))
			return vis;
		if (isempty(o) || notouchingface(o, opp))
			return vismask;
		if (touchingface(o, opp) && faceedges(o, opp) == F_SOLID)
		...
		no.mask(0xFFF);
		ivec2 cf[4], of[4];
		int numc = genfacevecs(c, orient, vo, size, false, cf), numo = genfacevecs(o, opp, no, nsize, false, of);
		if (numo < 3 || !insideface(cf, numc, of, numo))
			return vismask;
		return vis;
	}
	...
	if (!occludesface(o, opp, no, nsize, vo, size, MAT_AIR, (c.material & MAT_ALPHA) ^ MAT_ALPHA, MAT_ALPHA, cf, numc))
		vis |= 1;
	if (vismask & 2 && !occludesface(o, opp, no, nsize, vo, size, MAT_AIR, MAT_NOCLIP, MATF_CLIP, cf, numc))
		vis |= 2;
	return vis;
}

// more expensive version that checks both triangles of a face independently
auto visibletris(const cube& c, int orient, const ivec& co, int size, ushort nmat, ushort matmask) -> int {
	...
	if (nsize > size || (nsize == size && !o.children)) {
		...
		if (nmat != MAT_AIR && (o.material & matmask) == nmat)
			return vis;
		if (isentirelysolid(o) || (touchingface(o, opp) && faceedges(o, opp) == F_SOLID))
			return vis & notouch;
		...
		numc = genfacevecs(c, orient, vo, size, false, cf, v);
		if (occludesface(o, opp, no, nsize, vo, size, MAT_AIR, nmat, matmask, cf, numc))
			return vis & notouch;
	}
	if (vis != 3 || notouch)
		return vis;

	static const int triverts[2][2][2][3] = {// order
											 {// coord
											  {{1, 2, 3}, {0, 1, 3}},  // verts
											  {{0, 1, 2}, {0, 2, 3}}},
											 {// coord
		...
		loopi(2) {
			...
			if (numo > 0) {
				if (!insideface(tf, 3, of, numo))
					continue;
			} else if (!occludesface(o, opp, no, nsize, vo, size, MAT_AIR, nmat, matmask, tf, 3))
				continue;
			return vis & ~(1 << i);
		}
		vis |= 4;
...
auto genclipplane(const cube& c, int orient, vec* v, plane* clip) -> int {
	int planes = 0, convex = faceconvexity(c, orient), order = convex < 0 ? 1 : 0;
	const vec &v0 = v[fv[orient][order]], &v1 = v[fv[orient][order + 1]], &v2 = v[fv[orient][order + 2]],
	...
	if (collide || (c.visible & 0xC0) == 0x40) {
		loopi(6) if (c.visible & (1 << i)) {
			...
			if (flataxisface(c, i))
				...
				const vec &v0 = p.v[fv[i][order]], &v1 = p.v[fv[i][order + 1]], &v2 = p.v[fv[i][order + 2]],
...
static inline auto mergefacecmp(const facebounds& x, const facebounds& y) -> bool {
	if (x.v2 < y.v2)
		return true;
	if (x.v2 > y.v2)
		return false;
...
static auto mergefacev(int orient, facebounds* m, int sz, facebounds& n) -> int {
...
static auto mergefaceu(int orient, facebounds& m, facebounds& n) -> int {
...
static auto mergeface(int orient, facebounds* m, int sz, facebounds& n) -> int {
...
auto mergefaces(int orient, facebounds* m, int sz) -> int {
...
static inline auto htcmp(const cfkey& x, const cfkey& y) -> bool {
	return x.orient == y.orient && x.tex == y.tex && x.n == y.n && x.offset == y.offset && x.material == y.material;
}

static inline auto hthash(const cfkey& k) -> uint {
...
auto mincubeface(const cube& cu, int orient, const ivec& co, int size, facebounds& orig) -> bool {
	ivec no;
	int nsize;
...
struct pvert {
	...
	auto operator==(const pvert& o) const -> bool {
		return x == o.x && y == o.y;
	}
	auto operator!=(const pvert& o) const -> bool {
		return x != o.x || y != o.y;
...
struct pedge {
	...
	auto operator==(const pedge& o) const -> bool {
...
static inline auto hthash(const pedge& x) -> uint {
	return uint(x.from.x) ^ (uint(x.from.y) << 8);
}
static inline auto htcmp(const pedge& x, const pedge& y) -> bool {
	return x == y;
}
...
auto clippoly(poly& p, const facebounds& b) -> bool {
...
auto genpoly(cube& cu, int orient, const ivec& o, int size, int vis, ivec& n, int& offset, poly& p) -> bool {
	int dim = dimension(orient), coord = dimcoord(orient);
	ivec v[4];
	...
	if (faceedges(cu, orient) != F_SOLID) {
		int px = int(p.verts[p.numverts - 2].x) - int(p.verts[p.numverts - 3].x),
			py = int(p.verts[p.numverts - 2].y) - int(p.verts[p.numverts - 3].y),
			cx = int(p.verts[p.numverts - 1].x) - int(p.verts[p.numverts - 2].x),
			cy = int(p.verts[p.numverts - 1].y) - int(p.verts[p.numverts - 2].y), dir = px * cy - py * cx;
		if (dir > 0)
			return false;
		if (!dir) {
			if (p.numverts < 4)
		...
		px = cx;
		py = cy;
		cx = int(p.verts[0].x) - int(p.verts[p.numverts - 1].x);
		cy = int(p.verts[0].y) - int(p.verts[p.numverts - 1].y);
		dir = px * cy - py * cx;
		if (dir > 0)
			return false;
		if (!dir) {
		...
		px = cx;
		py = cy;
		cx = int(p.verts[1].x) - int(p.verts[0].x);
		cy = int(p.verts[1].y) - int(p.verts[0].y);
		dir = px * cy - py * cx;
		if (dir > 0)
			return false;
		if (!dir) {
		...
		px = cx;
		py = cy;
		cx = int(p.verts[2].x) - int(p.verts[1].x);
		cy = int(p.verts[2].y) - int(p.verts[1].y);
		dir = px * cy - py * cx;
		if (dir > 0)
			return false;
		if (!dir) {
...
auto mergepolys(int orient, hashset<plink>& links, vector<plink*>& queue, int owner, poly& p, poly& q, const pedge& e) -> bool {
	int pe = -1, qe = -1;
	loopi(p.numverts) if (p.verts[i] == e.from) {
		pe = i;
		break;
	}
	loopi(q.numverts) if (q.verts[i] == e.to) {
		qe = i;
		break;
	}
	if (pe < 0 || qe < 0)
	...
	int px = int(verts[numverts - 1].x) - int(verts[numverts - 2].x),
		py = int(verts[numverts - 1].y) - int(verts[numverts - 2].y);
	loopi(q.numverts - 1) {
		if (index >= q.numverts)
			index -= q.numverts;
...
void addmerge(cube& cu, int orient, const ivec& co, const ivec& n, int offset, poly& p) {
	...
	if (!p.numverts) {
		if (cu.ext)
			cu.ext->surfaces[orient] = ambientsurface;
		return;
	}
...
static inline void clearmerge(cube& c, int orient) {
	if (c.merged & (1 << orient)) {
		c.merged &= ~(1 << orient);
		if (c.ext)
			c.ext->surfaces[orient] = brightsurface;
	}
}
...
void genmerges(cube* c = worldroot, const ivec& o = ivec(0, 0, 0), int size = worldsize >> 1) {
	if ((genmergeprogress++ & 0xFFF) == 0)
		renderprogress(float(genmergeprogress) / allocnodes, "merging faces...");
	...
	loopi(8) {
		...
		if (c[i].children)
			...
			loopj(6) if ((vis = visibletris(c[i], j, co, size))) {
				cfkey k;
				poly p;
		...
		if ((size == 1 << maxmerge || c == worldroot) && cpolys.numelems) {
			enumeratekt(
				cpolys, cfkey, key, cfpolys, val, { mergepolys(key.orient, co, key.n, key.offset, val.polys); });
			cpolys.clear();
		}
	}
	--neighbourdepth;
...
auto calcmergedsize(int orient, const ivec& co, int size, const vertinfo* verts, int numverts) -> int {
	ushort x1 = verts[0].x, y1 = verts[0].y, z1 = verts[0].z, x2 = x1, y2 = y1, z2 = z1;
	...
	while (bits < 15) {
		mo.mask(~((1 << bits) - 1));
		if (mo.x <= x1 && mo.x + (1 << bits) >= x2 && mo.y <= y1 && mo.y + (1 << bits) >= y2 && mo.z <= z1 &&
			mo.z + (1 << bits) >= z2)
			break;
		bits++;
	}
...
static void invalidatemerges(cube& c) {
	...
	if (c.children)
		loopi(8) invalidatemerges(c.children[i]);
}
