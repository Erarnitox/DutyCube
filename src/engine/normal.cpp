...
static inline auto hthash(const normalkey& k) -> uint {
	return hthash(k.pos);
}
...
static inline auto htcmp(const normalkey& k, const normalgroup& n) -> bool {
...
static auto addnormal(const vec& pos, int smooth, const vec& surface) -> int {
...
static void
addtnormal(const vec& pos, int smooth, float offset, int normal1, int normal2, const vec& pos1, const vec& pos2) {
	normalkey key = {pos, smooth};
	normalgroup& g = normalgroups.access(key, key);
...
static auto addnormal(const vec& pos, int smooth, int axis) -> int {
	normalkey key = {pos, smooth};
	normalgroup& g = normalgroups.access(key, key);
	g.flat += 1 << (4 * axis);
	return axis - 6;
...
static inline auto findtnormal(const normalgroup& g, float lerpthreshold, const vec& surface, vec& v) -> bool {
	float bestangle = lerpthreshold;
	tnormal* bestnorm = nullptr;
	for (int cur = g.tnormals; cur >= 0;) {
		tnormal& o = tnormals[cur];
		static const vec flats[6] = {
		...
		vec n1 = o.normals[0] < 0 ? flats[o.normals[0] + 6] : normals[o.normals[0]].surface,
			n2 = o.normals[1] < 0 ? flats[o.normals[1] + 6] : normals[o.normals[1]].surface, nt;
...
void addnormals(cube& c, const ivec& o, int size) {
	...
	loopi(6) if ((vis = visibletris(c, i, o, size))) {
		...
		if (!numplanes)
			loopk(numverts) norms[k] = addnormal(pos[k], smooth, i);
		else if (numplanes == 1)
			loopk(numverts) norms[k] = addnormal(pos[k], smooth, planes[0]);
		else {
		...
		while (tj >= 0 && tjoints[tj].edge < (i + 1) * (MAXFACEVERTS + 1)) {
			...
			if (d[axis] < 0)
				d.neg();
			reduceslope(d);
			int origin = int(min(v1[axis], v2[axis]) * 8) & ~0x7FFF, offset1 = (int(v1[axis] * 8) - origin) / d[axis],
...
auto smoothangle(int id, int angle) -> int {
	if (id < 0)
