...
struct BIH {
	...
	struct tribb {
		...
		bool outside(const ivec& bo, const ivec& br) const {
			return abs(bo.x - center.x) > br.x + radius.x || abs(bo.y - center.y) > br.y + radius.y ||
	...
	bool
	boxcollide(physent* d, const vec& dir, float cutoff, const vec& o, int yaw, int pitch, int roll, float scale = 1);
	...
	void genstaintris(stainrenderer* s,
