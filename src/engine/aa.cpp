...
void cleanuptqaa() {
	tqaatype = -1;
	loopi(2) if (tqaatex[i]) {
		glDeleteTextures(1, &tqaatex[i]);
		tqaatex[i] = 0;
	}
	loopi(2) if (tqaafbo[i]) {
...
void setupfxaa(int w, int h) {
	...
	createtexture(fxaatex,
				  w,
				  h,
				  nullptr,
				  3,
				  1,
				  tqaa || (!fxaagreenluma && !intel_texalpha_bug) ? GL_RGBA8 : GL_RGB,
...
int smaatype = -1;
static Shader *smaalumaedgeshader = nullptr, *smaacoloredgeshader = nullptr, *smaablendweightshader = nullptr,
			  *smaaneighborhoodshader = nullptr;

void loadsmaashaders(bool split = false) {
	smaatype =
		tqaatype >= 0 ? tqaatype : (!smaagreenluma && !intel_texalpha_bug && !smaacoloredge ? AA_LUMA : AA_UNUSED);
	if (split)
		smaatype += AA_SPLIT;
...
enum {
SMAA_SEARCHTEX_WIDTH = 66,
SMAA_SEARCHTEX_HEIGHT = 33
};
...
void gensmaasearchdata() {
	...
	loop(a, 2) loop(b, 2) loop(c, 2) loop(d, 2) edges[(a * 1 + b * 3) + (c * 7 + d * 21)] =
		a + (b << 1) + (c << 2) + (d << 3);
...
auto areaunderortho(const vec2& p1, const vec2& p2, float x) -> vec2 {
	vec2 d(p2.x - p1.x, p2.y - p1.y);
	float y1 = p1.y + (x - p1.x) * d.y / d.x, y2 = p1.y + (x + 1 - p1.x) * d.y / d.x;
	...
	x = -p1.y * d.x / d.y + p1.x;
	float a1 = x > p1.x ? y1 * fmod(x, 1.0f) / 2 : 0, a2 = x < p2.x ? y2 * (1 - fmod(x, 1.0f)) / 2 : 0;
...
static const int edgesortho[][2] = {{0, 0},
									{3, 0},
									{0, 3},
									{3, 3},
									{1, 0},
...
static inline auto areaortho(float p1x, float p1y, float p2x, float p2y, float left) -> vec2 {
...
auto areaortho(int pattern, float left, float right, float offset) -> vec2 {
...
auto areaunderdiag(const vec2& p1, const vec2& p2, const vec2& p) -> float {
...
static inline auto areadiag(const vec2& p1, const vec2& p2, float left) -> vec2 {
	return {1 - areaunderdiag(p1, p2, vec2(1, 0).add(left)), areaunderdiag(p1, p2, vec2(1, 1).add(left))};
}

static const int edgesdiag[][2] = {{0, 0},
								   {1, 0},
								   {0, 2},
								   {1, 2},
								   {2, 0},
...
static inline auto
areadiag(float p1x, float p1y, float p2x, float p2y, float d, float left, const vec2& offset, int pattern) -> vec2 {
...
static inline auto areadiag2(float p1x,
							 float p1y,
							 float p2x,
							 float p2y,
							 float p3x,
...
auto areadiag(int pattern, float left, float right, const vec2& offset) -> vec2 {
	float d = left + right + 1;
	switch (pattern) {
	case 0:
		return areadiag2(1, 1, 1, 1, 1, 0, 1, 0, d, left, offset, pattern);
	case 1:
		return areadiag2(1, 0, 0, 0, 1, 0, 1, 0, d, left, offset, pattern);
	case 2:
		return areadiag2(0, 0, 1, 0, 1, 0, 1, 0, d, left, offset, pattern);
	case 3:
		return areadiag(1, 0, 1, 0, d, left, offset, pattern);
	case 4:
		return areadiag2(1, 1, 0, 0, 1, 1, 1, 0, d, left, offset, pattern);
	case 5:
		return areadiag2(1, 1, 0, 0, 1, 0, 1, 0, d, left, offset, pattern);
	case 6:
		return areadiag(1, 1, 1, 0, d, left, offset, pattern);
	case 7:
		return areadiag2(1, 1, 1, 0, 1, 0, 1, 0, d, left, offset, pattern);
	case 8:
		return areadiag2(0, 0, 1, 1, 1, 0, 1, 1, d, left, offset, pattern);
	case 9:
		return areadiag(1, 0, 1, 1, d, left, offset, pattern);
	case 10:
		return areadiag2(0, 0, 1, 1, 1, 0, 1, 0, d, left, offset, pattern);
	case 11:
		return areadiag2(1, 0, 1, 1, 1, 0, 1, 0, d, left, offset, pattern);
	case 12:
		return areadiag(1, 1, 1, 1, d, left, offset, pattern);
	case 13:
		return areadiag2(1, 1, 1, 1, 1, 0, 1, 1, d, left, offset, pattern);
	case 14:
		return areadiag2(1, 1, 1, 1, 1, 1, 1, 0, d, left, offset, pattern);
	case 15:
		return areadiag2(1, 1, 1, 1, 1, 0, 1, 0, d, left, offset, pattern);
	}
	return {0, 0};
...
static const float offsetsdiag[][2] = {{
										   0.0f,
										   0.0f,
									   },
									   {0.25f, -0.25f},
...
enum {
SMAA_AREATEX_WIDTH = 160,
SMAA_AREATEX_HEIGHT = 560
};
...
void setupsmaa(int w, int h) {
	...
	gensmaasearchdata();
	gensmaaareadata();
	createtexture(smaaareatex,
				  SMAA_AREATEX_WIDTH,
				  SMAA_AREATEX_HEIGHT,
				  smaaareadata,
				  3,
				  1,
				  hasTRG ? GL_RG8 : GL_LUMINANCE8_ALPHA8,
				  GL_TEXTURE_RECTANGLE,
				  0,
				  0,
				  0,
				  false);
	createtexture(smaasearchtex,
...
void cleanupsmaa() {
	...
	loopi(4) if (smaafbo[i]) {
		glDeleteFramebuffers_(1, &smaafbo[i]);
		smaafbo[i] = 0;
	}
	loopi(5) if (smaatex[i]) {
...
void dosmaa(GLuint outfbo = 0, bool split = false) {
	...
	loop(pass, split ? 2 : 1) {
		...
		if (tqaa && split)
			subsamples = tqaaframe & 1 ? (pass != smaasubsampleorder ? vec4(6, 4, 2, 4) : vec4(3, 5, 1, 4))
