struct vertmodel : animmodel {
	...
	struct vertmesh : mesh {
		...
		void interpverts(const animstate& as, T* RESTRICT vdata, skin& s) {
			vdata += voffset;
			const vert *RESTRICT vert1 = &verts[as.cur.fr1 * numverts], *RESTRICT vert2 = &verts[as.cur.fr2 * numverts],
...
#define ipvertp(attrib, type)                                                 \
	v.attrib.lerp(type().lerp(pvert1[i].attrib, pvert2[i].attrib, as.prev.t), \
				  type().lerp(vert1[i].attrib, vert2[i].attrib, as.cur.t),    \
				  as.interp)
			if (as.interp < 1)
				loopi(numverts) {
					T& v = vdata[i];
					ipvertp(pos, vec);
					ipvertp(tangent, vec4);
				}
			else
				loopi(numverts) {
					T& v = vdata[i];
...
#undef ipvertp
		...
		void render(const animstate* as, skin& s, vbocacheentry& vc) {
			glDrawRangeElements_(
				GL_TRIANGLES, minvert, maxvert, elen, GL_UNSIGNED_SHORT, &((vertmeshgroup*)group)->edata[eoffset]);
			glde++;
	...
	struct vertmeshgroup : meshgroup {
		...
		vertmeshgroup()
			: numframes(0), tags(NULL), numtags(0), edata(NULL), ebuf(0), vlen(0), vertsize(0), vdata(NULL) {
		}
		...
		void genvbo(vbocacheentry& vc) {
			...
			if (numframes > 1) {
				...
				looprendermeshes(vertmesh, m, { m.fillverts((vvert*)vdata); });
			} else {
		...
		void render(const animstate* as, float pitch, const vec& axis, const vec& forward, dynent* d, part* p) {
			...
			if (numframes > 1) {
				if (vc->as != *as) {
					...
					looprendermeshes(vertmesh, m, { m.interpverts(*as, (vvert*)vdata, p->skins[i]); });
					gle::bindvbo(vc->vbuf);
...
template <class MDL>
struct vertloader : modelloader<MDL> {};
...
struct vertcommands : modelcommands<MDL, struct MDL::vertmesh> {
	...
	static void loadpart(char* model, float* smooth) {
		...
		if (mdl.index)
			mdl.disablepitch();
		mdl.meshes =
			MDL::loading->sharemeshes(path(filename), *smooth > 0 ? cosf(clamp(*smooth, 0.0f, 180.0f) * RAD) : 2);
		if (!mdl.meshes)
	...
	static void settag(char* tagname, float* tx, float* ty, float* tz, float* rx, float* ry, float* rz) {
		...
		matrix4x3 m(matrix3(quat(sx * cy * cz - cx * sy * sz,
								 cx * sy * cz + sx * cy * sz,
								 cx * cy * sz - sx * sy * cz,
								 cx * cy * cz + sx * sy * sz)),
					vec(*tx, *ty, *tz));
