...
struct editor {
	int mode;  // editor mode - 1= keep while focused, 2= keep while used in gui, 3= keep forever (i.e. until mode
			   // changes)
	...
	void key(int code) {
		...
		case SDLK_DELETE:
			if (!del()) {
				...
				if (cx < current.len)
					current.del(cx, 1);
				else if (cy < lines.length() - 1) {	 // combine with next line
					current.append(lines[cy + 1].text);
					removelines(cy + 1, 1);
				}
			}
		...
		case SDLK_BACKSPACE:
			if (!del()) {
				...
				if (cx > 0)
					current.del(--cx, 1);
				else if (cy > 0) {	// combine with previous line
					cx = lines[cy - 1].len;
...
#define TEXTCOMMAND(f, s, d, body) ICOMMAND(f, s, d, if (!textfocus || identflags & IDF_OVERRIDDEN) return; body)
...
ICOMMAND(textfocus,
		 "si",
		 (char* name, int* mode),  // focus on a (or create a persistent) specific editor, else returns current name
...
TEXTCOMMAND(textmode,
			"i",
			(int* m),  // (1= keep while focused, 2= keep while used in gui, 3= keep forever (i.e. until mode changes))
					   // topmost editor, return current setting if no args
			if (*m) textfocus->mode = *m;
			else intret(textfocus->mode););
TEXTCOMMAND(textsave,
			"s",
			(char* file),  // saves the topmost (filename is optional)
			if (*file) textfocus->setfile(path(file, true));
			textfocus->save(););
TEXTCOMMAND(
	textload,
	"s",
	(char* file),  // loads into the textfocusmost editor, returns filename if no args
	if (*file) {
...
ICOMMAND(textinit,
		 ...
		 (char* name,
		  char* file,
		  char* initval),  // loads into named editor if no file assigned and editor has been rendered
		 {
			 ...
			 if (e && e->rendered && !e->filename && *file &&
				 (e->lines.empty() || (e->lines.length() == 1 && !strcmp(e->lines[0].text, initval)))) {
...
TEXTCOMMAND(textmark,
			"i",
			(int* m),  // (1=mark, 2=unmark), return current mark setting if no args
...
TEXTCOMMAND(textexec,
			"i",
			(int* selected),  // execute script commands from the buffer (0=all, 1=selected region only)
