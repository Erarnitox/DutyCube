...
Shader *nullshader = nullptr, *hudshader = nullptr, *hudtextshader = nullptr, *hudnotextureshader = nullptr, *nocolorshader = nullptr,
	   *foggedshader = nullptr, *foggednotextureshader = nullptr, *ldrshader = nullptr, *ldrnotextureshader = nullptr,
...
auto lookupshaderbyname(const char* name) -> Shader* {
	Shader* s = shaders.access(name);
	return s && s->loaded() ? s : nullptr;
}

auto generateshader(const char* name, const char* fmt, ...) -> Shader* {
	if (!loadedshaders)
		return nullptr;
...
static void showglslinfo(GLenum type, GLuint obj, const char* name, const char** parts = nullptr, int numparts = 0) {
	...
	if (length > 1) {
		conoutf(CON_ERROR,
				"GLSL ERROR (%s:%s)",
				type == GL_VERTEX_SHADER ? "VS" : (type == GL_FRAGMENT_SHADER ? "FS" : "PROG"),
				name);
		FILE* l = getlogfile();
		if (l) {
...
static auto finddecls(const char* line) -> const char* {
	for (;;) {
...
static void bindglsluniform(Shader& s, UniformLoc& u) {
	...
	if (bidx != GL_INVALID_INDEX && uidx != GL_INVALID_INDEX) {
		...
		if (dbgubo)
			conoutf(CON_DEBUG,
					"UBO: %s:%s:%d, offset: %d, size: %d, stride: %d",
					u.name,
					u.blockname,
					u.binding,
					offsetval,
...
static void linkglslprogram(Shader& s, bool msg = true) {
	...
	if (success) {
		...
		loopi(16) {
			static const char* const texnames[16] = {"tex0",
													 "tex1",
													 "tex2",
													 "tex3",
													 "tex4",
...
auto getlocalparam(const char* name) -> int {
	return localparams.access(name, int(localparams.numelems));
}

static auto addlocalparam(Shader& s, const char* name, int loc, int size, GLenum format) -> int {
...
auto getglobalparam(const char* name) -> GlobalShaderParamState* {
...
static auto addglobalparam(Shader& s,
											GlobalShaderParamState* param,
											int loc,
											int size,
											GLenum format) -> GlobalShaderParamUse* {
...
enumerate(globalparams, GlobalShaderParamState, g, { g.version = ++nextversion; });
	...
	static auto findslotparam(Slot& s, const char* name, float* noval = nullptr) -> float* {
		loopv(s.params) {
	...
	static auto findslotparam(VSlot& s, const char* name, float* noval = nullptr) -> float* {
		loopv(s.params) {
			SlotShaderParam& param = s.params[i];
			if (name == param.name)
				return param.val;
	...
	static inline void setslotparam(SlotShaderParamState& l, const float* val) {
		...
		case GL_FLOAT_VEC4:
			...
			break;
		case GL_INT:
			glUniform1i_(l.loc, int(val[0]));
			break;
		case GL_INT_VEC2:
			glUniform2i_(l.loc, int(val[0]), int(val[1]));
			break;
		case GL_INT_VEC3:
			glUniform3i_(l.loc, int(val[0]), int(val[1]), int(val[2]));
			break;
		case GL_INT_VEC4:
			glUniform4i_(l.loc, int(val[0]), int(val[1]), int(val[2]), int(val[3]));
			break;
		case GL_UNSIGNED_INT:
			glUniform1ui_(l.loc, uint(val[0]));
			break;
		case GL_UNSIGNED_INT_VEC2:
			glUniform2ui_(l.loc, uint(val[0]), uint(val[1]));
			break;
		case GL_UNSIGNED_INT_VEC3:
			glUniform3ui_(l.loc, uint(val[0]), uint(val[1]), uint(val[2]));
			break;
		case GL_UNSIGNED_INT_VEC4:
			glUniform4ui_(l.loc, uint(val[0]), uint(val[1]), uint(val[2]), uint(val[3]));
			break;
		}
	}
...
#define SETDEFAULTPARAMS                            \
	...
	void Shader::setslotparams(Slot& slot, VSlot& vslot) {
		...
		if (vslot.slot == &slot) {
			...
			loopv(defaultparams) {
				SlotShaderParamState& l = defaultparams[i];
				SETSLOTPARAM(
					l, unimask, i, l.flags & SlotShaderParam::REUSE ? findslotparam(vslot, l.name, l.val) : l.val);
			}
		}
	}
	...
	void Shader::cleanup(bool full) {
		...
		if (standard || full) {
			...
			loopv(defaultparams) defaultparams[i].loc = -1;
	}
	...
	static void genuniformlocs(Shader& s, const char* vs, const char* ps, Shader* reusevs, Shader* reuseps) {
		...
		if (reusevs)
			...
			while ((vs = strstr(vs, "//:uniform"))) {
				...
				if (numargs >= 3)
					s.uniformlocs.add(UniformLoc(
						getshaderparamname(name), getshaderparamname(blockname), binding, numargs >= 4 ? stride : 0));
				else if (numargs >= 1)
	...
	auto newshader(int type, const char* name, const char* vs, const char* ps, Shader* variant = nullptr, int row = 0) -> Shader* {
		...
		if (variant)
			loopv(variant->defaultparams) s.defaultparams.add(variant->defaultparams[i]);
		else
	...
	static auto findglslmain(const char* s) -> const char* {
	...
	static void genfogshader(vector<char>& vsbuf, vector<char>& psbuf, const char* vs, const char* ps) {
		...
		if (psmain && psend) {
			...
			const char* fogparams =
				...
				"#define fogcoord lineardepth*length(vec3(gl_FragCoord.xy*radialfogscale.xy + radialfogscale.zw, "
				"1.0))\n";
			psbuf.put(fogparams, strlen(fogparams));
			psbuf.put(psmain, psend - psmain);
			const char* psdef = "\n#define FOG_COLOR ";
			const char* psfog = pspragma && !strncmp(pspragma + pragmalen, "rgba", 4)
				? "\nfragcolor = mix((FOG_COLOR), fragcolor, clamp(exp2(fogcoord*-fogdensity.x)*fogdensity.y, 0.0, "
				  "1.0));\n"
				: "\nfragcolor.rgb = mix((FOG_COLOR).rgb, fragcolor.rgb, "
				  "clamp(exp2(fogcoord*-fogdensity.x)*fogdensity.y, 0.0, 1.0));\n";
			int clen = 0;
			if (pspragma) {
				pspragma += pragmalen;
	...
	void setupshaders() {
		...
		hudtextshader = newshader(0,
								  ...
								  "}\n");
		hudnotextureshader = newshader(0,
									   "<init>hudnotexture",
									   "attribute vec4 vvertex, vcolor;\n"
									   "uniform mat4 hudmatrix;"
									   "varying vec4 color;\n"
	...
	auto Shader::uniformlocversion() -> int {
	...
	auto useshaderbyname(const char* name) -> Shader* {
		Shader* s = shaders.access(name);
		if (!s)
			return nullptr;
		if (s->deferred())
...
#define GENSHADER(cond, body)              \
	...
	auto shouldreuseparams(Slot& s, VSlot& p) -> bool {
		if (!s.shader)
			return false;
	...
	auto getshaderparamname(const char* name, bool insert) -> const char* {
...
NUMPOSTFXBINDS = 10
	...
	struct postfxpass {
		...
		vec4 params;
		uint inputs{1}, freeinputs{1};
		int outputbind{0}, outputscale{0};

		postfxpass()  = default;
	};
	vector<postfxpass> postfxpasses;

	static auto allocatepostfxtex(int scale) -> int {
		loopv(postfxtexs) {
		...
		createtexture(
			t.id, max(postfxw >> scale, 1), max(postfxh >> scale, 1), nullptr, 3, 1, GL_RGB, GL_TEXTURE_RECTANGLE);
	...
	auto setuppostfx(int w, int h, GLuint outfbo) -> GLuint {
		...
		loopv(postfxpasses) {
			...
			if (!postfxpasses.inrange(i + 1)) {
				...
				glFramebufferTexture2D_(
					GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_RECTANGLE, postfxtexs[tex].id, 0);
			}
	...
	static auto addpostfx(const char* name,
						  int outputbind,
						  int outputscale,
						  uint inputs,
						  uint freeinputs,
						  const vec4& params) -> bool {
		if (!*name)
			return false;
	...
	ICOMMAND(
		...
		(char* name, int* bind, int* scale, char* inputs, float* x, float* y, float* z, float* w),
		{
			...
			addpostfx(
				name, clamp(*bind, 0, NUMPOSTFXBINDS - 1), max(*scale, 0), inputmask, freemask, vec4(*x, *y, *z, *w));
		});
		...
		enumerate(shaders, Shader, s, {
			if (!s.standard && s.loaded() && !s.variantshader) {
				...
				loopv(s.variants) {
					...
					if ((v->reusevs && v->reusevs->invalid()) || (v->reuseps && v->reuseps->invalid()) || !v->compile())
						v->cleanup(true);
				}
			}
	...
	void setupblurkernel(int radius, float* weights, float* offsets) {
		...
		loopi(radius) {
			float weight1 = exp(-((2 * i) * (2 * i)) / (2 * sigma * sigma)) / sigma,
				  weight2 = exp(-((2 * i + 1) * (2 * i + 1)) / (2 * sigma * sigma)) / sigma, scale = weight1 + weight2,
	...
	void setblurshader(int pass, int size, int radius, float* weights, float* offsets, GLenum target) {
		if (radius < 1 || radius > MAXBLURRADIUS)
			return;
		static Shader *blurshader[7][2] =
			{{nullptr, nullptr}, {nullptr, nullptr}, {nullptr, nullptr}, {nullptr, nullptr}, {nullptr, nullptr}, {nullptr, nullptr}, {nullptr, nullptr}},
					  *blurrectshader[7][2] = {{nullptr, nullptr},
											   {nullptr, nullptr},
											   {nullptr, nullptr},
											   {nullptr, nullptr},
											   {nullptr, nullptr},
