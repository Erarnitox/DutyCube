...
#define OP(c, n) \
	dst[n] = (uint(xsrc[n]) + uint(xsrc[n + BPP]) + uint(xsrc[stride + n]) + uint(xsrc[stride + n + BPP])) >> 2
			PIXELOP
#undef OP
		}
		src += 2 * stride;
...
static void FUNCNAME(
	shifttexture)(uchar* RESTRICT src, uint sw, uint sh, uint stride, uchar* RESTRICT dst, uint dw, uint dh) {
...
#define OP(c, n) c##t = 0
			DEFPIXEL
#undef OP
			for (uchar *ycur = xsrc, *xend = &ycur[wfrac * BPP], *yend = &src[hfrac * stride]; ycur < yend;
...
static void FUNCNAME(
	scaletexture)(uchar* RESTRICT src, uint sw, uint sh, uint stride, uchar* RESTRICT dst, uint dw, uint dh) {
	...
	for (under = 0; (darea << under) < sarea; under++)
		;
	uint cscale = clamp(under, over - 12, 12), ascale = clamp(12 + under - over, 0, 24), dscale = ascale + 12 - cscale,
		 area = ((ullong)darea << ascale) / sarea;
	dw *= wfrac;
	dh *= hfrac;
	for (uint y = 0; y < dh; y += hfrac) {
		const uint yn = y + hfrac - 1, yi = y >> 12, h = (yn >> 12) - yi,
				   ylow = ((yn | (-int(h) >> 24)) & 0xFFFU) + 1 - (y & 0xFFFU), yhigh = (yn & 0xFFFU) + 1;
		const uchar* ysrc = &src[yi * stride];
		for (uint x = 0; x < dw; x += wfrac, dst += BPP) {
			const uint xn = x + wfrac - 1, xi = x >> 12, w = (xn >> 12) - xi,
