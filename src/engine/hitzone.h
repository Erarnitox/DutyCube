...
#define SKELTRIINTERSECT(a, b, c)                                                                                  \
	...
	if ((skelmodel::intersectmode & RAY_ALPHAPOLY) == RAY_ALPHAPOLY) {                                             \
		...
		if (tex->type & Texture::ALPHA && (tex->alphamask || loadalphamask(tex))) {                                \
			int si = clamp(                                                                                        \
					int(tex->xs * (va.tc.x + u * (vb.tc.x - va.tc.x) + v * (vc.tc.x - va.tc.x))), 0, tex->xs - 1), \
				ti = clamp(                                                                                        \
					int(tex->ys * (va.tc.y + u * (vb.tc.y - va.tc.y) + v * (vc.tc.y - va.tc.y))), 0, tex->ys - 1); \
...
inline void skelbih::intersect(skelmodel::skelmeshgroup* m,
							   skelmodel::skin* s,
							   const vec& o,
							   const vec& ray,
							   const vec& invray,
...
void skelbih::intersect(skelmodel::skelmeshgroup* m, skelmodel::skin* s, const vec& o, const vec& ray) {
	...
	float tmin, tmax;
	float t1 = (bbmin.x - o.x) * invray.x, t2 = (bbmax.x - o.x) * invray.x;
	if (invray.x > 0) {
		tmin = t1;
		tmax = t2;
	} else {
...
void skelbih::build(skelmodel::skelmeshgroup* m, ushort* indices, int numindices, const vec& vmin, const vec& vmax) {
	...
	loopk(3) {
		...
		for (left = 0, right = numindices, splitleft = SHRT_MIN, splitright = SHRT_MAX; left < right;) {
			...
			const vec &ta = tm->verts[tri.vert[0]].pos, &tb = tm->verts[tri.vert[1]].pos,
...
struct skelhitzone {
	...
	static bool triintersect(skelmodel::skelmeshgroup* m,
							 skelmodel::skin* s,
	...
	void intersect(skelmodel::skelmeshgroup* m,
		...
		if (!numchildren) {
			if (bih) {
				const dualquat& b =
					blend < numblends ? bdata2[blend] : bdata1[m->skel->bones[blend - numblends].interpindex];
...
bool skelhitzone::triintersect(skelmodel::skelmeshgroup* m,
							   skelmodel::skin* s,
							   const dualquat* bdata1,
							   const dualquat* bdata2,
							   int numblends,
	...
	skelmodel::skelmesh* tm = (skelmodel::skelmesh*)m->meshes[t.mesh];
	const skelmodel::vert &va = tm->verts[t.vert[0]], &vb = tm->verts[t.vert[1]], &vc = tm->verts[t.vert[2]];
	vec a = (va.blend < numblends ? bdata2[va.blend] : bdata1[m->blendcombos[va.blend].interpbones[0]])
				.transform(va.pos),
		b = (vb.blend < numblends ? bdata2[vb.blend] : bdata1[m->blendcombos[vb.blend].interpbones[0]])
				.transform(vb.pos),
		c = (vc.blend < numblends ? bdata2[vc.blend] : bdata1[m->blendcombos[vc.blend].interpbones[0]])
				.transform(vc.pos);
...
struct skelzonekey {
	...
	void subtract(const skelzonekey& o) {
		...
		loopi(sizeof(bones)) {
		retry:
			if (j >= (int)sizeof(o.bones) || bones[i] < o.bones[j]) {
				bones[len++] = bones[i];
				continue;
			}
			if (bones[i] == o.bones[j]) {
			...
			do
				j++;
			while (j < (int)sizeof(o.bones) && bones[i] > o.bones[j]);
			goto retry;
		}
...
void skelmodel::skelmeshgroup::intersect(skelhitdata* z,
										 part* p,
										 const skelmodel::skelcacheentry& sc,
										 const vec& o,
										 const vec& ray) {
...
uchar skelhitdata::chooseid(skelmodel::skelmeshgroup* g,
							skelmodel::skelmesh* m,
							const skelmodel::tri& t,
							const uchar* ids) {
	int numused = 0;
	uchar used[12];
	float weights[12];
...
void skelhitdata::build(skelmodel::skelmeshgroup* g, const uchar* ids) {
	...
	loopv(g->blendcombos) if (!g->blendcombos[i].weights[1]) {
		numblends = i;
		break;
	}
