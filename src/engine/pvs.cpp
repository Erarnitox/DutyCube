...
static auto mergepvsnodes(pvsnode& p, pvsnode* children) -> bool {
	loopi(7) if (children[i].flags != children[7].flags) return false;
	bvec bbs[4];
	loop(x, 2) loop(y, 2) {
		const bvec &lo = children[octaindex(2, x, y, 0)].edges, &hi = children[octaindex(2, x, y, 1)].edges;
		if (lo.x != 0xFF && (lo.x & 0x11 || lo.y & 0x11 || lo.z & 0x11))
			return false;
		if (hi.x != 0xFF && (hi.x & 0x11 || hi.y & 0x11 || hi.z & 0x11))
			return false;
...
#define MERGEBBS(res, coord, row, col)                                                                     \
	if (lo.coord == 0xFF) {                                                                                \
		...
		res.row = lo.row;                                                                                  \
		res.col = lo.col;                                                                                  \
	} else if (lo.row != hi.row || lo.col != hi.col || (lo.coord & 0xF0) != 0x80 || (hi.coord & 0xF) != 0) \
		return false;                                                                                      \
	else {                                                                                                 \
...
static void genpvsnodes(cube* c, int parent = 0, const ivec& co = ivec(0, 0, 0), int size = worldsize / 2) {
	...
	loopi(8) {
		...
		n.flags = 0;
		n.children = 0;
		if (c[i].children || isempty(c[i]) || c[i].material & MAT_ALPHA)
			memset(n.edges.v, 0xFF, 3);
		else
			loopk(3) {
				...
				if (face == F_SOLID)
					...
					uchar low = max(max(face & 0xF, (face >> 8) & 0xF), max((face >> 16) & 0xF, (face >> 24) & 0xF)),
						  high = min(min((face >> 4) & 0xF, (face >> 12) & 0xF),
									 min((face >> 20) & 0xF, (face >> 28) & 0xF));
					if (size < 8) {
						if (low & ((8 / size) - 1)) {
							low += 8 / size - (low & ((8 / size) - 1));
						}
						if (high & ((8 / size) - 1))
							high &= ~(8 / size - 1);
					}
	...
	loopi(8) if (c[i].children) {
		...
		if (origpvsnodes[index + i].children)
			branches++;
	}
	if (!branches && mergepvsnodes(origpvsnodes[parent], &origpvsnodes[index]))
		origpvsnodes.setsize(index);
	else
		origpvsnodes[parent].children = index;
}
...
struct shaftbb {
	...
	auto operator[](int i) -> ushort& {
		return v[i];
	}
	auto operator[](int i) const -> ushort {
		return v[i];
	}

	[[nodiscard]] auto contains(const shaftbb& o) const -> bool {
		return min.x <= o.min.x && min.y <= o.min.y && min.z <= o.min.z && max.x >= o.max.x && max.y >= o.max.y &&
			max.z >= o.max.z;
	}

	[[nodiscard]] auto outside(const ivec& o, int size) const -> bool {
		return o.x >= max.x || o.y >= max.y || o.z >= max.z || o.x + size <= min.x || o.y + size <= min.y ||
			o.z + size <= min.z;
	}

	[[nodiscard]] auto outside(const shaftbb& o) const -> bool {
		return o.min.x > max.x || o.min.y > max.y || o.min.z > max.z || o.max.x < min.x || o.max.y < min.y ||
			o.max.z < min.z;
	}

	[[nodiscard]] auto notinside(const shaftbb& o) const -> bool {
		return o.min.x < min.x || o.min.y < min.y || o.min.z < min.z || o.max.x > max.x || o.max.y > max.y ||
			o.max.z > max.z;
	}
};
...
struct shaft {
	...
	void calcshaft(const shaftbb& from, const shaftbb& to) {
		...
		loopi(3) {
			if (to.min[i] < from.min[i]) {
				color |= 1 << i;
				bounds.min[i] = 0;
			} else if (to.min[i] > from.min[i])
				bounds.min[i] = to.min[i] + 1;
			else {
				match |= 1 << i;
				bounds.min[i] = to.min[i];
			}

			if (to.max[i] > from.max[i]) {
				color |= 8 << i;
				bounds.max[i] = USHRT_MAX;
			} else if (to.max[i] < from.max[i])
				bounds.max[i] = to.max[i] - 1;
			else {
				match |= 8 << i;
				bounds.max[i] = to.max[i];
			}
		}
		numplanes = 0;
		loopi(5) if (!(match & (1 << i))) for (int j = i + 1; j < 6; j++) if (!(match & (1 << j)) && i + 3 != j &&
																			  ((color >> i) ^ (color >> j)) & 1) {
	...
	[[nodiscard]] auto outside(const shaftbb& o) const -> bool {
		...
		for (const shaftplane* p = planes; p < &planes[numplanes]; p++) {
			if (o[p->rnear] * p->r + o[p->cnear] * p->c + p->offset > 0)
				return true;
		}
		return false;
	}

	[[nodiscard]] auto inside(const shaftbb& o) const -> bool {
		if (bounds.notinside(o))
			return false;

		for (const shaftplane* p = planes; p < &planes[numplanes]; p++) {
			if (o[p->rfar] * p->r + o[p->cfar] * p->c + p->offset > 0)
				return false;
		}
		return true;
	}
...
static inline auto hthash(const pvsdata& k) -> uint {
	uint h = 5381;
	loopi(k.len) h = ((h << 5) + h) ^ pvsbuf[k.offset + i];
	return h;
}

static inline auto htcmp(const pvsdata& x, const pvsdata& y) -> bool {
...
struct pvsworker {
	pvsworker() :  pvsnodes(new pvsnode[origpvsnodes.length()]) {
	}
	~pvsworker() {
		delete[] pvsnodes;
	}
	...
	auto hasvoxel(const ivec& p, int coord, int dir, int ocoord = 0, int odir = 0, int* omin = nullptr) -> int {
	...
	void cullpvs(pvsnode& p, const ivec& co = ivec(0, 0, 0), int size = worldsize) {
		if (p.flags & (PVS_HIDE_BB | PVS_HIDE_GEOM) || genpvs_canceled)
			return;
		if (p.children && !(p.flags & PVS_HIDE_BB)) {
			pvsnode* children = &pvsnodes[p.children];
			int csize = size >> 1;
			ivec dmin = ivec(co).add(csize >> 1).sub(ivec(viewcellbb.min).add(ivec(viewcellbb.max)).shr(1)),
				 dmax = ivec(dmin).add(csize);
			dmin.mul(dmin);
			dmax.mul(dmax);
			...
			if (order[2].dist < order[1].dist)
				swap(order[1], order[2]);
			if (order[3].dist < order[2].dist)
			...
			int j;
			for (j = 4; j > 0 && dxy.dist < order[j - 1].dist; --j)
				order[j] = order[j - 1];
			order[j] = dxy;
			for (j = 5; j > 0 && dxz.dist < order[j - 1].dist; --j)
				order[j] = order[j - 1];
			order[j] = dxz;
			for (j = 6; j > 0 && dyz.dist < order[j - 1].dist; --j)
				order[j] = order[j - 1];
			order[j] = dyz;
			loopi(8) {
		...
		if (order[1].dist > order[0].dist)
			swap(order[0], order[1]);
		loopi(6) {
			int dim = order[i >= 3 ? i - 3 : i].index, dc = (i >= 3) != (geom.max[dim] <= viewcellbb.min[dim]) ? 1 : 0,
				r = R[dim], c = C[dim];
			int ccenter = geom.min[c];
			if (geom.min[r] == geom.max[r] || geom.min[c] == geom.max[c])
				continue;
			while (ccenter < geom.max[c]) {
				...
				for (int minstep = -1, maxstep = 1; (minstep || maxstep) && rmax[r] - rmin[r] < maxpvsblocker;) {
					if (minstep)
						minstep = hasvoxel(rmin, r, 0);
					if (maxstep)
						maxstep = hasvoxel(rmax, r, 1);
					rmin[r] += minstep;
					rmax[r] += maxstep;
				}
				...
				if (cmin[c] >= geom.min[c] && cmax[c] < geom.max[c]) {
					if (emin[r] > geom.min[r])
						emin[r] = geom.min[r];
					if (emax[r] < geom.max[r] - 1)
						emax[r] = geom.max[r] - 1;
				}
				...
				if (bb.min[dim] >= viewcellbb.max[dim] || bb.max[dim] <= viewcellbb.min[dim]) {
					int ddir = bb.min[dim] >= viewcellbb.max[dim] ? 1 : -1, dval = ddir > 0 ? USHRT_MAX - 1 : 0,
						dlimit = maxpvsblocker, numsides = 0;
					loopj(4) {
						ivec dmax;
						int odim = j < 2 ? c : r;
						if (j & 1) {
							if (bb.max[odim] >= viewcellbb.max[odim])
								continue;
							dmax[odim] = bb.max[odim] - 1;
						} else {
							if (bb.min[odim] <= viewcellbb.min[odim])
								continue;
						...
						if (!dstep)
							dmax[dim] -= ddir;
						if (ddir > 0)
							dval = min(dval, dmax[dim]);
						else
							dval = max(dval, dmax[dim]);
					}
					if (numsides > 0) {
						if (ddir > 0)
							bb.max[dim] = dval + 1;
					...
					// bb.min.x, bb.min.y, bb.min.z, bb.max.x-bb.min.x, bb.max.y-bb.min.y, bb.max.z-bb.min.z, i,
					// ccenter, co.x, co.y, co.z, size);
				}
				bool dup = false;
				loopvj(prevblockers) {
					if (prevblockers[j].contains(bb)) {
						dup = true;
						break;
					}
				}
	...
	auto compresspvs(pvsnode& p, int size, int threshold) -> bool {
		if (!p.children)
			return true;
		if (p.flags & PVS_HIDE_BB) {
			p.children = 0;
		...
		bool canreduce = true;
		loopi(8) {
			if (!compresspvs(children[i], size / 2, threshold))
				canreduce = false;
		}
		if (canreduce) {
	...
	auto serializepvs(pvsnode& p, int storage = -1) -> bool {
		if (!p.children) {
			outbuf.add(0xFF);
		...
		if (storage >= 0) {
			for (; i < 8; i++) {
				...
				if (child.flags & PVS_HIDE_BB)
					leafvalues |= 1 << i;
				else if (child.children)
					break;
			}
			if (i == 8) {
				outbuf[storage] = leafvalues;
				return false;
			}
		...
		for (; i < 8; i++) {
			pvsnode& child = children[i];
			if (child.children) {
				if (!serializepvs(child, index + 1 + i))
					leafmask |= 1 << i;
			} else {
				leafmask |= 1 << i;
				outbuf[index + 1 + i] = child.flags & PVS_HIDE_BB ? 0xFF : 0;
			}
		}
	...
	auto materialoccluded(pvsnode& p, const ivec& co, int size, const ivec& bbmin, const ivec& bbmax) -> bool {
		pvsnode* children = &pvsnodes[p.children];
		loopoctabox(co, size, bbmin, bbmax) {
			ivec o(i, co, size);
			if (children[i].flags & PVS_HIDE_BB)
				continue;
			if (!children[i].children || !materialoccluded(children[i], o, size / 2, bbmin, bbmax))
				return false;
		}
		return true;
	}

	auto materialoccluded(vector<materialsurface*>& matsurfs) -> bool {
		if (pvsnodes[0].flags & PVS_HIDE_BB)
			return true;
		if (!pvsnodes[0].children)
			return false;
		loopv(matsurfs) {
			...
			bbmax[C[dim]] += m.csize;
			bbmax[R[dim]] += m.rsize;
			if (!materialoccluded(pvsnodes[0], ivec(0, 0, 0), worldsize / 2, bbmin, bbmax))
				return false;
		}
		return true;
	}
	...
	void calcpvs(const ivec& co, int size) {
		...
		loopi(numwaterplanes) {
			if (waterplanes[i].height < 0) {
				if (waterfalls.length() && materialoccluded(waterfalls))
					wateroccluded |= 1 << i;
			} else if (waterplanes[i].matsurfs.length() && materialoccluded(waterplanes[i].matsurfs))
				wateroccluded |= 1 << i;
		}
		waterbytes = 0;
	...
	auto testviewcell(const ivec& co, int size, int* waterpvs = nullptr, int* len = nullptr) -> uchar* {
		calcpvs(co, size);

		auto* buf = new uchar[outbuf.length()];
		memcpy(buf, outbuf.getbuf(), outbuf.length());
		if (waterpvs)
			*waterpvs = wateroccluded;
		if (len)
			*len = outbuf.length();
		return buf;
	}

	auto genviewcell(const ivec& co, int size) -> int {
		calcpvs(co, size);
	...
	static auto run(void* data) -> int {
		auto* w = (pvsworker*)data;
...
static auto genpvs_timer(Uint32 interval, void* param) -> Uint32 {
...
static void show_genpvs_progress(int unique = pvs.length(), int processed = numviewcells) {
	...
	defformatstring(
		text1, "%d%% - %d of %d view cells (%d unique)", int(bar1 * 100), processed, totalviewcells, unique);
...
static inline auto isallclip(cube* c) -> bool {
	loopi(8) {
		cube& h = c[i];
...
static auto countviewcells(cube* c, const ivec& co, int size, int threshold) -> int {
	int count = 0;
	loopi(8) {
		ivec o(i, co, size);
		if (pvsbounds.outside(o, size))
			continue;
		cube& h = c[i];
		if (h.children) {
			if (size > threshold) {
			...
			if (isallclip(h.children))
				continue;
		} else if (isentirelysolid(h) || (h.material & MATF_CLIP) == MAT_CLIP)
			continue;
		count++;
	}
	return count;
...
static inline auto lookupviewcell(const vec& p) -> pvsdata* {
	uint x = uint(floor(p.x)), y = uint(floor(p.y)), z = uint(floor(p.z));
	if (!viewcells || (x | y | z) >= uint(worldsize))
		return nullptr;
...
static void findwaterplanes() {
	...
	loopv(valist) {
		...
		loopj(va->matsurfs) {
			...
			if ((m.material & MATF_VOLUME) != MAT_WATER || m.orient == O_BOTTOM) {
				j += m.skip;
				continue;
			}
			if (m.orient != O_TOP) {
	...
	if (waterfalls.length() > 0 && numwaterplanes < MAXWATERPVS)
		numwaterplanes++;
}
...
void testpvs(int* vcsize) {
	...
	loopi(MAXWATERPVS) lockedwaterplanes[i] = waterplanes[i].height;
	lockpvs = 1;
	conoutf("generated test view cell of size %d at %.1f, %.1f, %.1f (%d B)",
			size,
			camera1->o.x,
			camera1->o.y,
			camera1->o.z,
...
void genpvs(int* viewcellsize) {
	...
	if (numthreads <= 1) {
		...
		if (!pvsmutex)
			pvsmutex = SDL_CreateMutex();
		if (!viewcellmutex)
			viewcellmutex = SDL_CreateMutex();
		loopi(numthreads) {
	...
	if (genpvs_canceled) {
		...
		conoutf("generated %d unique view cells totaling %.1f kB and averaging %d B (%.1f seconds)",
				pvs.length(),
				pvsbuf.length() / 1024.0f,
				pvsbuf.length() / max(pvs.length(), 1),
				(end - start) / 1000.0f);
}
...
static inline auto pvsoccluded(uchar* buf, const ivec& co, int size, const ivec& bbmin, const ivec& bbmax) -> bool {
	...
	loopoctabox(co, size, bbmin, bbmax) {
		...
		if (leafmask & (1 << i)) {
			...
			if (!leafvalues || (leafvalues != 0xFF && octaboxoverlap(o, size >> 1, bbmin, bbmax) & ~leafvalues))
				return false;
		} else if (!pvsoccluded(buf + 9 * buf[1 + i], o, size >> 1, bbmin, bbmax))
			return false;
	}
	return true;
}

static inline auto pvsoccluded(uchar* buf, const ivec& bbmin, const ivec& bbmax) -> bool {
	...
	while (!(diff & (1 << scale))) {
		...
		if (leafmask & (1 << i)) {
			...
			return leafvalues &&
				(leafvalues == 0xFF ||
				 !(octaboxoverlap(ivec(bbmin).mask(~((2 << scale) - 1)), 1 << scale, bbmin, bbmax) & ~leafvalues));
		}
		buf += 9 * buf[1 + i];
	}
...
auto pvsoccluded(const ivec& bbmin, const ivec& bbmax) -> bool {
	return curpvs != nullptr && pvsoccluded(curpvs, bbmin, bbmax);
}

auto pvsoccludedsphere(const vec& center, float radius) -> bool {
	if (curpvs == nullptr)
		return false;
...
auto waterpvsoccluded(int height) -> bool {
	if (!curwaterpvs)
		return false;
	if (lockedpvs) {
...
void saveviewcells(stream* f, viewcellnode& p) {
	...
	loopi(8) {
		if (p.leafmask & (1 << i))
			f->putlil<int>(p.children[i].pvs);
		else
			saveviewcells(f, *p.children[i].node);
	}
}
...
auto loadviewcells(stream* f) -> viewcellnode* {
	auto* p = new viewcellnode;
	p->leafmask = f->getchar();
	loopi(8) {
		if (p->leafmask & (1 << i))
			p->children[i].pvs = f->getlil<int>();
		else
			p->children[i].node = loadviewcells(f);
	}
	return p;
}
