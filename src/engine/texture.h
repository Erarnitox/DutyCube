...
struct UniformLoc {
	...
	void* data;
	UniformLoc(const char* name = NULL, const char* blockname = NULL, int binding = -1, int stride = -1)
		: name(name),
		  blockname(blockname),
		  loc(-1),
		  version(-1),
...
struct FragDataLoc {
	...
	GLenum format;
	int index;
	FragDataLoc(const char* name = NULL, int loc = -1, GLenum format = GL_FALSE, int index = 0)
		: name(name), loc(loc), format(format), index(index) {
	}
};
...
#define GLOBALPARAM(name, vals)                \
	do {                                       \
		static GlobalShaderParam param(#name); \
		param.set(vals);                       \
	} while (0)
...
#define GLOBALPARAMV(name, vals, num)          \
	do {                                       \
		static GlobalShaderParam param(#name); \
		param.setv(vals, num);                 \
	} while (0)
...
struct ImageData {
	...
	ImageData() : data(NULL), owner(NULL), freefunc(NULL) {
	}
	...
	ImageData(int nw, int nh, int nbpp, uchar* data) : owner(NULL), freefunc(NULL) {
	...
	void
	setdata(uchar* ndata, int nw, int nh, int nbpp, int nlevels = 1, int nalign = 0, GLenum ncompressed = GL_FALSE) {
		w = nw;
		h = nh;
		bpp = nbpp;
		levels = nlevels;
...
struct Slot {
	...
	Slot(int index = -1) : index(index), variants(NULL), grass(NULL) {
		reset();
	}
	virtual ~Slot() {
	}
...
extern Shader *nullshader, *hudshader, *hudtextshader, *hudnotextureshader, *nocolorshader, *foggedshader,
	*foggednotextureshader, *ldrshader, *ldrnotextureshader, *stdworldshader;
...
extern void setblurshader(int pass,
						  int size,
						  int radius,
						  float* weights,
						  float* offsets,
