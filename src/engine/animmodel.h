...
struct animmodel : model {
	...
	struct skin : shaderparams {
		...
		void setshaderparams(mesh& m, const animstate* as, bool skinned = true) {
			...
			if (envmapped())
				LOCALPARAMF(envmapscale, envmapmin - envmapmax, envmapmax);
		}
	...
	struct mesh {
		...
		void calctangents(V* verts, TC* tcverts, int numverts, T* tris, int numtris, bool areaweight) {
			...
			loopi(numtris) {
				...
				const vec2 &tc0 = tcverts[t.vert[0]].tc, &tc1 = tcverts[t.vert[1]].tc, &tc2 = tcverts[t.vert[2]].tc;
				float u1 = tc1.x - tc0.x, v1 = tc1.y - tc0.y, u2 = tc2.x - tc0.x, v2 = tc2.y - tc0.y;
				vec u(e2), v(e2);
			...
			loopi(numverts) {
				V& v = verts[i];
				const vec &t = tangent[i], &bt = bitangent[i];
				matrix3 m;
				m.c = v.norm;
...
#define looprendermeshes(type, name, body)      \
	do {                                        \
		loopv(meshes) {                         \
		...
		virtual void intersect(const animstate* as,
							   float pitch,
							   const vec& axis,
							   const vec& forward,
							   dynent* d,
	...
	struct part {
		...
		bool calcanim(int animpart,
			...
			if (d && interp >= 0) {
				...
				if ((info.anim & (ANIM_LOOP | ANIM_CLAMP)) == ANIM_CLAMP)
					aitime = min(aitime, int(info.range * info.speed * 0.5e-3f));
		...
		void intersect(int anim,
			...
			if (as->cur.anim & ANIM_NOPITCH || (as->interp < 1 && as->prev.anim & ANIM_NOPITCH))
				pitchamount *= (as->cur.anim & ANIM_NOPITCH ? 0 : as->interp) +
					(as->interp < 1 && as->prev.anim & ANIM_NOPITCH ? 0 : 1 - as->interp);
			if (pitchamount) {
				++matrixpos;
		...
		void
		render(int anim, int basetime, int basetime2, float pitch, const vec& axis, const vec& forward, dynent* d) {
			animstate as[MAXANIMPARTS];
	...
	void render(int anim,
		...
		if (a) {
			...
			for (int i = 0; a[i].tag; i++) {
				...
				if (!m) {
					if (a[i].pos)
						link(NULL, a[i].tag, vec(0, 0, 0), 0, 0, a[i].pos);
					continue;
				}
				part* p = m->parts[0];
				switch (linktype(m, p)) {
	...
	void preloadBIH() {
		model::preloadBIH();
		if (bih)
			loopv(parts) parts[i]->preloadBIH();
	}
...
GLuint animmodel::lastvbuf = 0, animmodel::lasttcbuf = 0, animmodel::lastxbuf = 0, animmodel::lastbbuf = 0,
	   animmodel::lastebuf = 0, animmodel::lastenvmaptex = 0, animmodel::closestenvmaptex = 0;
Texture *animmodel::lasttex = NULL, *animmodel::lastdecal = NULL, *animmodel::lastmasks = NULL,
...
template <class MDL>
MDL* modelloader<MDL>::loading = NULL;
template <class MDL>
string modelloader<MDL>::dir = {'\0'};	// crashes clang if "" is used here
...
#define loopskins(meshname, s, body) \
	...
	static void setskin(char* meshname, char* tex, char* masks, float* envmapmax, float* envmapmin) {
		loopskins(
			meshname, s, s.tex = textureload(makerelpath(MDL::dir, tex), 0, true, false); if (*masks) {
	...
	static void setglow(char* meshname, float* percent, float* delta, float* pulse) {
		float glow = *percent > 0 ? *percent / 100.0f : 0.0f, glowdelta = *delta / 100.0f,
	...
	static void setdecal(char* meshname, char* decal) {
		loopskins(meshname, s, s.decal = textureload(makerelpath(MDL::dir, decal), 0, true, false););
	}
	...
	static void settricollide(char* meshname) {
		...
		if (init)
			loopmeshes("*", m, m.cancollide = false);
		loopmeshes(meshname, m, {
			m.cancollide = true;
			m.canrender = false;
		});
