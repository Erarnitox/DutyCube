...
static auto loadmapheader(stream* f, const char* ogzname, mapheader& hdr, octaheader& ohdr) -> bool {
	...
	if (!memcmp(hdr.magic, "TMAP", 4)) {
		if (hdr.version > MAPVERSION) {
			conoutf(CON_ERROR, "map %s requires a newer version of DropEngine", ogzname);
			return false;
		}
		...
		if (hdr.version != OCTAVERSION) {
			conoutf(CON_ERROR, "map %s uses an unsupported map format version", ogzname);
			return false;
		}
		...
		conoutf(CON_ERROR, "map %s uses an unsupported map type", ogzname);
		return false;
	}
...
auto supportedgametype(const string& gametype) -> bool {
	return (strcmp(gametype, game::gameident()) == 0) || (strcmp(gametype, "Tesseract") == 0);
}

auto loadents(const char* fname, vector<entity>& ents, uint* crc) -> bool {
...
void setmapfilenames(const char* fname, const char* cname = nullptr) {
	...
	if (savebak == 1)
		formatstring(bakname, "media/map/%s.BAK", fname);
	else {
		string baktime;
...
void savec(cube* c, const ivec& o, int size, stream* f, bool nolms) {
	if ((savemapprogress++ & 0xFFF) == 0)
		renderprogress(float(savemapprogress) / allocnodes, "saving octree...");
		...
		if (c[i].children) {
			...
			if (oflags & 0x20) {
				...
				loopj(6) if (surfmask & (1 << j)) {
					...
					int layerverts = surf.numverts & MAXFACEVERTS, numverts = surf.totalverts(), vertmask = 0,
						vertorder = 0, dim = dimension(j), vc = C[dim], vr = R[dim];
					if (numverts) {
						if (c[i].merged & (1 << j)) {
							vertmask |= 0x04;
							if (layerverts == 4) {
								ivec v[4] = {
									verts[0].getxyz(), verts[1].getxyz(), verts[2].getxyz(), verts[3].getxyz()};
								loopk(4) {
									const ivec &v0 = v[k], &v1 = v[(k + 1) & 3], &v2 = v[(k + 2) & 3],
											   &v3 = v[(k + 3) & 3];
					...
					if (layerverts == 4) {
						if (hasxyz && vertmask & 0x01) {
							...
							f->putlil<ushort>(v0[vc]);
							f->putlil<ushort>(v0[vr]);
							f->putlil<ushort>(v2[vc]);
							f->putlil<ushort>(v2[vr]);
							hasxyz = false;
						}
					}
					...
					if (hasxyz || hasnorm)
						loopk(layerverts) {
							...
							if (hasxyz) {
								ivec xyz = v.getxyz();
								f->putlil<ushort>(xyz[vc]);
								f->putlil<ushort>(xyz[vr]);
							}
							if (hasnorm)
...
auto loadchildren(stream* f, const ivec& co, int size, bool& failed) -> cube*;
...
void loadc(stream* f, cube& c, const ivec& co, int size, bool& failed) {
	...
	if (octsav & 0x20) {
		...
		loopi(6) if (surfmask & (1 << i)) {
			...
			genfaceverts(c, i, v);
			bool hasxyz = (vertmask & 0x04) != 0, hasuv = mapversion <= 0 && (vertmask & 0x40) != 0,
			...
			if (layerverts == 4) {
				if (hasxyz && vertmask & 0x01) {
					auto c1 = f->getlil<ushort>(), r1 = f->getlil<ushort>(), c2 = f->getlil<ushort>(),
						   r2 = f->getlil<ushort>();
					ivec xyz;
					xyz[vc] = c1;
					xyz[vr] = r1;
			...
			if (hasxyz || hasuv || hasnorm)
				loopk(layerverts) {
					...
					if (hasxyz) {
						ivec xyz;
						xyz[vc] = f->getlil<ushort>();
						xyz[vr] = f->getlil<ushort>();
						xyz[dim] = n[dim] ? -(bias + n[vc] * xyz[vc] + n[vr] * xyz[vr]) / n[dim] : vo[dim];
						v.setxyz(xyz);
					}
					if (hasuv) {
...
auto save_world(const char* mname, bool nolms) -> bool {
	if (!*mname)
	...
	enumerate(idents, ident, id, {
		if ((id.type == ID_VAR || id.type == ID_FVAR || id.type == ID_SVAR) && id.flags & IDF_OVERRIDE &&
			!(id.flags & IDF_READONLY) && id.flags & IDF_OVERRIDDEN)
			hdr.numvars++;
	});
	...
	enumerate(idents, ident, id, {
		...
		case ID_VAR:
			if (dbgvars)
				conoutf(CON_DEBUG, "wrote var %s: %d", id.name, *id.storage.i);
			f->putlil<int>(*id.storage.i);
			break;

		case ID_FVAR:
			if (dbgvars)
				conoutf(CON_DEBUG, "wrote fvar %s: %f", id.name, *id.storage.f);
			f->putlil<float>(*id.storage.f);
			break;

		case ID_SVAR:
			if (dbgvars)
				conoutf(CON_DEBUG, "wrote svar %s: %s", id.name, *id.storage.s);
	...
	if (dbgvars)
		conoutf(CON_DEBUG, "wrote %d vars", hdr.numvars);
	...
	if (!nolms) {
		if (getnumviewcells() > 0) {
			renderprogress(0, "saving pvs...");
			savepvs(f);
		}
	}
	if (shouldsaveblendmap()) {
		renderprogress(0, "saving blendmap...");
		saveblendmap(f);
	}
...
auto load_world(const char* mname,
				const char* cname) -> bool	// still supports all map formats that have existed since the earliest cube betas!
{
		...
		if (id && id->flags & IDF_OVERRIDE)
			...
			case ID_VAR: {
				...
				if (id->minval <= id->maxval && i >= id->minval && i <= id->maxval) {
					setvar(name, i);
					if (dbgvars)
						conoutf(CON_DEBUG, "read var %s: %d", name, i);
				}
				break;
			}
			case ID_FVAR: {
				...
				if (id->minvalf <= id->maxvalf && f >= id->minvalf && f <= id->maxvalf) {
					setfvar(name, f);
					if (dbgvars)
						conoutf(CON_DEBUG, "read fvar %s: %f", name, f);
				}
				break;
			}
			case ID_SVAR:
				...
				if (dbgvars)
					conoutf(CON_DEBUG, "read svar %s: %s", name, val.getstr());
				break;
			}
	}
	if (dbgvars)
		conoutf(CON_DEBUG, "read %d vars", hdr.numvars);
	...
	loopi(min(hdr.numents, MAXENTS)) {
		...
		if (!insideworld(e.o)) {
			if (e.type != ET_LIGHT && e.type != ET_SPOTLIGHT) {
				conoutf(CON_WARN,
						"warning: ent outside of world: enttype[%s] index %d (%f, %f, %f)",
	...
	if (failed)
		conoutf(CON_ERROR, "garbage in map");
	...
	if (!failed) {
		...
		if (hdr.numpvs > 0)
			loadpvs(f, hdr.numpvs);
		if (hdr.blendmap)
			loadblendmap(f, hdr.blendmap);
	}
...
void writecollideobj(char* name) {
	...
	loopv(entgroup) {
		...
		if (e.type != ET_MAPMODEL || !pointinsel(sel, e.o))
			continue;
		mm = &e;
		break;
	}
	...
	if (!m) {
		...
		if (mmi)
			conoutf(CON_ERROR, "could not load map model: %s", mmi->name);
		else
			conoutf(CON_ERROR, "could not find map model: %d", mm->attr1);
		return;
	}
	...
	loopv(valist) {
		vtxarray& va = *valist[i];
		if (va.geommin.x > selmax.x || va.geommin.y > selmax.y || va.geommin.z > selmax.z || va.geommax.x < selmin.x ||
			va.geommax.y < selmin.y || va.geommax.z < selmin.z)
			continue;
		if (!va.edata || !va.vdata)
			continue;
		ushort* edata = va.edata + va.eoffset;
