...
} flaretypes[] = {{2, 1.30f, 0.04f, 153},  // flares
				  {3, 1.00f, 0.10f, 102},
				  {1, 0.50f, 0.20f, 77},
				  {3, 0.20f, 0.05f, 77},
				  {0, 0.00f, 0.04f, 77},
...
struct flarerenderer : partrenderer {
	...
	void newflare(vec& o, const vec& center, uchar r, uchar g, uchar b, float mod, float size, bool sun, bool sparkle) {
		...
		dir.mul(1 / dist);
		if (raycube(o, dir, dist, RAY_CLIPMAT | RAY_POLY) < dist)
			return;
	...
	void addflare(vec& o, uchar r, uchar g, uchar b, bool sun, bool sparkle) {
		// frustrum + fog check
		if (isvisiblesphere(0.0f, o) > (sun ? VFC_FOGGED : VFC_FULL_VISIBLE))
			return;
	...
	void render() {
		...
		loopi(numflares) {
			...
			loopj(f.sparkle ? 12 : 9) {
				...
				gle::attribf(o.x + (-camright.x + camup.x) * sz,
							 o.y + (-camright.y + camup.y) * sz,
							 o.z + (-camright.z + camup.z) * sz);
	...
	particle* addpart(const vec& o, const vec& d, int fade, int color, float size, int gravity = 0) {
		return NULL;
	}
};
