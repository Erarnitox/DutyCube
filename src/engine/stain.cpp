...
struct stainbuffer {
	...
	stainbuffer()
		: verts(NULL), maxverts(0), startvert(0), endvert(0), lastvert(0), availverts(0), vbo(0), dirty(false) {
	}
	...
	void fadestain(const staininfo& d, const bvec4& color) {
		stainvert *vert = &verts[d.startvert], *end = &verts[d.endvert < d.startvert ? maxverts : d.endvert];
		while (vert < end) {
			vert->color = color;
			vert++;
		}
...
struct stainrenderer {
	...
	bool usegbuffer() const {
		return !(flags & (SF_INVMOD | SF_GLOW));
	}
	...
	void clearfadedstains() {
		...
		staininfo *d = &stains[startstain], *end = &stains[endstain < startstain ? maxstains : endstain],
		...
		if (startstain == endstain)
			...
			loopi(NUMSTAINBUFS) if (cleared[i]) verts[i].clearstains(*cleared[i]);
	}

	void fadeinstains() {
		if (!fadeintime)
			return;
		staininfo *d = &stains[endstain], *end = &stains[endstain < startstain ? 0 : startstain];
		while (d > end) {
			d--;
	...
	static void setuprenderstate(int sbuf, bool gbuf, int layer) {
		if (gbuf)
			maskgbuffer(sbuf == STAINBUF_TRANSPARENT ? "cg" : "c");
		else
			zerofogcolor();
	...
	static void cleanuprenderstate(int sbuf, bool gbuf, int layer) {
		...
		if (gbuf)
			maskgbuffer(sbuf == STAINBUF_TRANSPARENT ? "cndg" : "cnd");
		else
			resetfogcolor();
	}
...
#endif
		if (flags & SF_ROTATE)
			staintangent.rotate(sincos360[rnd(360)], dir);
...
#undef GENFACEVERT
			...
			if (numverts >= 4 && !(cu.merged & (1 << orient)) && !flataxisface(cu, orient) &&
				faceconvexity(verts, numverts, size)) {
			...
			return;
				 ...
				 (vismask & 0x40 &&
				  visibleface(cu, orient, o, size, MAT_AIR, (cu.material & MAT_ALPHA) ^ MAT_ALPHA, MAT_ALPHA))) {
			ivec v[4];
	...
	void findmaterials(vtxarray* va) {
		...
		loopi(matsurfs) {
			...
			if (!isclipped(m.material & MATF_VOLUME)) {
				i += m.skip;
				continue;
			}
			...
			for (;;) {
				materialsurface& m = matbuf[i];
				if (m.o[dim] >= bbmin[dim] && m.o[dim] <= bbmax[dim] && m.o[c] + m.csize >= bbmin[c] &&
					m.o[c] <= bbmax[c] && m.o[r] + m.rsize >= bbmin[r] && m.o[r] <= bbmax[r]) {
					static cube dummy;
		...
		loopi(8) {
			...
			if (escaped & (1 << i)) {
				...
				if (cu.children)
					...
					if (vismask)
						loopj(6) if (vismask & (1 << j)) gentris(cu, j, co, size);
				}
			}
		}
	}
		...
		loopi(8) {
			...
			if (overlap & (1 << i)) {
				...
				if (cu.children)
					...
					if (vismask & 0xC0) {
						if (vismask & 0x80)
							loopj(6) gentris(cu, j, co, size, NULL, vismask);
						else
							loopj(6) if (vismask & (1 << j)) gentris(cu, j, co, size);
					}
				}
...
void addstain(int type, const vec& center, const vec& surface, float radius, const bvec& color, int info) {
	if (!showstains || type < 0 || (size_t)type >= sizeof(stains) / sizeof(stains[0]) ||
		center.dist(camera1->o) - radius > maxstaindistance)
		return;
