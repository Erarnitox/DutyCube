...
auto resolverloop(void* data) -> int {
...
auto resolvercheck(const char** name, ENetAddress* address) -> bool {
	bool resolved = false;
	SDL_LockMutex(resolvermutex);
...
auto connectwithtimeout(ENetSocket sock, const char* hostname, const ENetAddress& address) -> int {
	defformatstring(text, "connecting to %s... (esc to abort)", hostname);
	renderprogress(0, text);

	ENetSocketSet readset, writeset;
	if (!enet_socket_connect(sock, &address))
		for (int starttime = SDL_GetTicks(), timeout = 0; timeout <= CONNLIMIT;) {
...
struct pingattempts {
	...
	int offset{0}, attempts[MAXATTEMPTS];
	...
	auto encodeping(int millis) -> int {
		millis += offset;
		return millis ? millis : 1;
	}

	auto decodeping(int val) -> int {
		return val - offset;
	}

	auto addattempt(int millis) -> int {
	...
	auto checkattempt(int val, bool del = true) -> bool {
		if (val)
...
struct serverinfo : servinfo, pingattempts {
	...
	int resolved{UNRESOLVED}, lastping, nextping;
	int pings[MAXPINGS];
	ENetAddress address;
	bool keep{false};
	...
	void calcping() {
		...
		loopk(MAXPINGS) if (pings[k] != WAITING) {
			totalpings += pings[k];
			numpings++;
		}
	...
	[[nodiscard]] auto status() const -> const char* {
	...
	static auto compare(serverinfo* a, serverinfo* b) -> bool {
...
static auto newserver(const char* name, int port, uint ip = ENET_HOST_ANY) -> serverinfo* {
...
void refreshservers() {
	...
	checkresolver();
	checkpings();
	if (totalmillis - lastinfo >=
		servpingrate / (maxservpings ? max(1, (servers.length() + maxservpings - 1) / maxservpings) : 1))
		pingservers();
	if (autosortservers)
		sortservers();
}
...
#define GETSERVERINFO(idx, si, body) \
	GETSERVERINFO_(                  \
		idx, si, if (si.valid()) { body; })

ICOMMAND(servinfovalid, "i", (int* i), GETSERVERINFO_(*i, si, intret(si.valid() ? 1 : 0)));
ICOMMAND(servinfodesc, "i", (int* i), GETSERVERINFO_(*i, si, {
...
ICOMMAND(servinfoplayers, "i", (int* i), GETSERVERINFO(*i, si, {
			 if (si.maxplayers <= 0)
				 intret(si.numplayers);
			 else
				 result(tempformatstring(
					 si.numplayers >= si.maxplayers ? "\f3%d/%d" : "%d/%d", si.numplayers, si.maxplayers));
		 }));
ICOMMAND(servinfoattr, "ii", (int* i, int* n), GETSERVERINFO(*i, si, {
			 if (si.attr.inrange(*n))
				 intret(si.attr[*n]);
		 }));

ICOMMAND(connectservinfo,
		 "is",
		 (int* i, char* pw),
		 GETSERVERINFO_(*i, si, connectserv(si.name, si.address.port, pw[0] ? pw : si.password)));

auto getservinfo(int i) -> servinfo* {
...
void clearservers(bool full = false) {
	...
	if (full)
		...
		loopvrev(servers) if (!servers[i]->keep) delete servers.remove(i);
}
...
ICOMMAND(addserver,
		 "sis",
		 (const char* name, int* port, const char* password),
