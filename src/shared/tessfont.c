...
const uchar cubectype[256] = {CUBECTYPE(CT_SPACE,
										CT_PRINT,
										CT_PRINT | CT_DIGIT,
...
struct fontchar {
	int code, uni, tex, x, y, sdfradius, sdfpitch, sdfx, sdfy, sdfw, sdfh;
...
void gensdf(struct fontchar* c) {
	...
	int dx, dy, dw = (w + 2 * radius + supersample - 1) / supersample,
		...
		for (dx = 0; dx < dw; dx++) {
			...
			for (y = dy * supersample - radius; y < (dy + 1) * supersample - radius; y++)
				for (x = dx * supersample - radius; x < (dx + 1) * supersample - radius; x++) {
					int sx = imax(x - radius, 0), sy = imax(y - radius, 0), ex = imin(x + radius, w),
						ey = imin(y + radius, h), cy, val = 0;
					if (y >= 0 && y < h && x >= 0 && x < w) {
					...
					if (val) {
						for (cy = imin(ey, y) - 1,
							sx1 = sx,
							ex1 = ex,
							src = (uchar*)c->glyph->bitmap.buffer + cy * c->glyph->bitmap.pitch;
...
void writecfg(const char* name,
	...
	for (i = 0; i < numchars; i++) {
		...
		if (c->code != c->uni)
			fprintf(f,
					"fontchar %d %d %d %d %g %g %g // %s (%d -> 0x%X)\n",
					c->x,
					c->y,
					c->sdfw,
					c->sdfh,
...
int main(int argc, char** argv) {
	...
	if (argc < 13)
		fatal(
			"Usage: tessfont infile outfile supersample border[:border2[:outline:outline2]] radius pad "
			"offsetx[:offsety] advance charwidth charheight texwidth texheight [spacewidth spaceheight scale texdir]");
	...
	if (FT_New_Face(l, argv[1], 0, &f) || FT_Set_Charmap(f, f->charmaps[0]) ||
