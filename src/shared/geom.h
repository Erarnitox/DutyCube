...
struct vec {
	...
	vec& rotate_around_x(float angle) {
		return rotate_around_x(cosf(angle), sinf(angle));
	}
	vec& rotate_around_y(float angle) {
		return rotate_around_y(cosf(angle), sinf(angle));
	}
	...
	vec& rotate(float c, float s, const vec& d) {
		*this = vec(
			x * (d.x * d.x * (1 - c) + c) + y * (d.x * d.y * (1 - c) - d.z * s) + z * (d.x * d.z * (1 - c) + d.y * s),
	...
	vec& rotate(float angle, const vec& d) {
		return rotate(cosf(angle), sinf(angle), d);
	}
	...
	bool insidebb(const T& bbmin, const T& bbmax, U margin) const {
		return x >= bbmin.x - margin && x <= bbmax.x + margin && y >= bbmin.y - margin && y <= bbmax.y + margin &&
			z >= bbmin.z - margin && z <= bbmax.z + margin;
	}
	...
	bool insidebb(const T& o, U size, U margin) const {
		size += margin;
		return x >= o.x - margin && x <= o.x + size && y >= o.y - margin && y <= o.y + size && z >= o.z - margin &&
			z <= o.z + size;
	}
	...
	static vec hexcolor(int color) {
		return vec(((color >> 16) & 0xFF) * (1.0f / 255.0f),
				   ((color >> 8) & 0xFF) * (1.0f / 255.0f),
				   (color & 0xFF) * (1.0f / 255.0f));
	}

	int tohexcolor() const {
		return (int(::clamp(r, 0.0f, 1.0f) * 255) << 16) | (int(::clamp(g, 0.0f, 1.0f) * 255) << 8) |
			int(::clamp(b, 0.0f, 1.0f) * 255);
	}
};
...
struct vec4 {
	...
	vec4& madd(const vec4& a, const B& b) {
		return add(vec4(a).mul(b));
	}
	template <class B>
	vec4& msub(const vec4& a, const B& b) {
		return sub(vec4(a).mul(b));
	}
...
struct quat : vec4 {
	...
	explicit quat(const matrix4x3& m) {
		convertmatrix(m);
	}
	...
	template <class B>
	quat& madd(const vec4& a, const B& b) {
		return add(vec4(a).mul(b));
	}
	template <class B>
	quat& msub(const vec4& a, const B& b) {
		return sub(vec4(a).mul(b));
	}
	...
	vec calcangles() const {
		...
		float rr = qq.x + qq.y + qq.z + qq.w, t = x * y + z * w;
...
struct dualquat {
	...
	vec transform(const vec& v) const {
		return vec()
			.cross(real, vec().cross(real, v).madd(v, real.w).add(vec(dual)))
			.madd(vec(dual), real.w)
			.msub(vec(real), dual.w)
			.mul(2)
			.add(v);
	}
...
struct matrix3 {
	...
	explicit matrix3(const quat& q) {
		float x = q.x, y = q.y, z = q.z, w = q.w, tx = 2 * x, ty = 2 * y, tz = 2 * z, txx = tx * x, tyy = ty * y,
			  tzz = tz * z, txy = tx * y, txz = tx * z, tyz = ty * z, twx = w * tx, twy = w * ty, twz = w * tz;
	...
	void rotate(float ck, float sk, const vec& axis) {
		a = vec(axis.x * axis.x * (1 - ck) + ck,
				axis.x * axis.y * (1 - ck) + axis.z * sk,
				axis.x * axis.z * (1 - ck) - axis.y * sk);
		b = vec(axis.x * axis.y * (1 - ck) - axis.z * sk,
				axis.y * axis.y * (1 - ck) + ck,
				axis.y * axis.z * (1 - ck) + axis.x * sk);
		c = vec(axis.x * axis.z * (1 - ck) + axis.y * sk,
				axis.y * axis.z * (1 - ck) - axis.x * sk,
	...
	void rotate_around_x(float ck, float sk) {
		vec rb = vec(b).mul(ck).madd(c, sk), rc = vec(c).mul(ck).msub(b, sk);
		b = rb;
		c = rc;
	}
	...
	void rotate_around_y(float ck, float sk) {
		vec rc = vec(c).mul(ck).madd(a, sk), ra = vec(a).mul(ck).msub(c, sk);
		c = rc;
		a = ra;
	}
	...
	void rotate_around_z(float ck, float sk) {
		vec ra = vec(a).mul(ck).madd(b, sk), rb = vec(b).mul(ck).msub(a, sk);
		a = ra;
		b = rb;
	}
	void rotate_around_z(float angle) {
		rotate_around_z(cosf(angle), sinf(angle));
	}
...
struct matrix4x3 {
	...
	matrix4x3(const dualquat& dq) {
		...
		r.mul(2);
		float xy = r.x * dq.real.y, xz = r.x * dq.real.z, yz = r.y * dq.real.z, wx = r.w * dq.real.x,
			  wy = r.w * dq.real.y, wz = r.w * dq.real.z;
...
struct ivec {
	...
	ivec(int i, const ivec& co, int size)
		: x(co.x + ((i & 1) >> 0) * size), y(co.y + ((i & 2) >> 1) * size), z(co.z + ((i & 4) >> 2) * size) {
	}
	...
	static inline ivec floor(const vec& o) {
		return ivec(int(::floor(o.x)), int(::floor(o.y)), int(::floor(o.z)));
	}
	static inline ivec ceil(const vec& o) {
		return ivec(int(::ceil(o.x)), int(::ceil(o.y)), int(::ceil(o.z)));
	}
};
...
struct bvec {
	...
	explicit bvec(const vec& v)
		: x(uchar((v.x + 1) * (255.0f / 2.0f))),
		  y(uchar((v.y + 1) * (255.0f / 2.0f))),
		  z(uchar((v.z + 1) * (255.0f / 2.0f))) {
	}
	...
	static bvec fromcolor(const vec& v) {
		return bvec(uchar(v.x * 255.0f), uchar(v.y * 255.0f), uchar(v.z * 255.0f));
	}
	vec tocolor() const {
	...
	static bvec from565(ushort c) {
		return bvec(
			(((c >> 11) & 0x1F) * 527 + 15) >> 6, (((c >> 5) & 0x3F) * 259 + 35) >> 6, ((c & 0x1F) * 527 + 15) >> 6);
	}
...
struct matrix4 {
	...
	matrix4(const vec4& a, const vec4& b, const vec4& c, const vec4& d = vec4(0, 0, 0, 1)) : a(a), b(b), c(c), d(d) {
	}
	matrix4(const matrix4x3& m) : a(m.a, 0), b(m.b, 0), c(m.c, 0), d(m.d, 1) {
	}
	matrix4(const matrix3& rot, const vec& trans) : a(rot.a, 0), b(rot.b, 0), c(rot.c, 0), d(trans, 1) {
	}
	...
	void muld(const matrix4& y) {
		mult<dvec4>(matrix4(*this), y);
	}

	void rotate_around_x(float ck, float sk) {
		vec4 rb = vec4(b).mul(ck).madd(c, sk), rc = vec4(c).mul(ck).msub(b, sk);
		b = rb;
		c = rc;
	}
	void rotate_around_x(float angle) {
	...
	void rotate_around_y(float ck, float sk) {
		vec4 rc = vec4(c).mul(ck).madd(a, sk), ra = vec4(a).mul(ck).msub(c, sk);
		c = rc;
		a = ra;
	}
	void rotate_around_y(float angle) {
	...
	void rotate_around_z(float ck, float sk) {
		vec4 ra = vec4(a).mul(ck).madd(b, sk), rb = vec4(b).mul(ck).msub(a, sk);
		a = ra;
		b = rb;
	}
	void rotate_around_z(float angle) {
	...
	void rotate(float angle, const vec& dir) {
		rotate(cosf(angle), sinf(angle), dir);
	}
	...
	void clip(const plane& p, const matrix4& m) {
		float x = ((p.x < 0 ? -1 : (p.x > 0 ? 1 : 0)) + m.c.x) / m.a.x,
			  y = ((p.y < 0 ? -1 : (p.y > 0 ? 1 : 0)) + m.c.y) / m.b.y, w = (1 + m.c.z) / m.d.z,
...
inline matrix3::matrix3(const matrix4& m) : a(m.a), b(m.b), c(m.c) {
}

inline matrix4x3::matrix4x3(const matrix4& m) : a(m.a), b(m.b), c(m.c), d(m.d) {
}
...
struct half {
	...
	bool operator==(const half& h) const {
		return val == h.val;
	}
	bool operator!=(const half& h) const {
		return val != h.val;
...
struct hvec2 {
	...
	template <class T>
	hvec2(T x, T y) : x(x), y(y) {
	}
	hvec2(const vec2& v) : x(v.x), y(v.y) {
	}

	bool operator==(const hvec2& h) const {
		return x == h.x && y == h.y;
	}
	bool operator!=(const hvec2& h) const {
		return x != h.x || y != h.y;
...
struct hvec {
	...
	template <class T>
	hvec(T x, T y, T z) : x(x), y(y), z(z) {
	}
	hvec(const vec& v) : x(v.x), y(v.y), z(v.z) {
	}

	bool operator==(const hvec& h) const {
...
struct hvec4 {
	...
	template <class T>
	hvec4(T x, T y, T z, T w) : x(x), y(y), z(z), w(w) {
	}
	template <class T>
	hvec4(const vec& v, T w = 0) : x(v.x), y(v.y), z(v.z), w(w) {
	}
	hvec4(const vec4& v) : x(v.x), y(v.y), z(v.z), w(v.w) {
	}

	bool operator==(const hvec4& h) const {
...
extern bool linecylinderintersect(const vec& from,
								  const vec& to,
								  const vec& start,
								  const vec& end,
								  float radius,
