...
extern const char* const attribnames[MAXATTRIBS] =
	{"vvertex", "vcolor", "vtexcoord0", "vtexcoord1", "vnormal", "vtangent", "vboneweight", "vboneindex"};
ucharbuf attribbuf;
...
void drawquads(int offset, int count) {
	...
	glDrawRangeElements_(
		GL_TRIANGLES, offset * 4, (offset + count) * 4 - 1, count * 6, GL_UNSIGNED_SHORT, (ushort*)0 + offset * 6);
}
...
void defattribs(const char* fmt) {
	for (;; fmt += 3) {
		...
		switch (fmt[0]) {
		case 'v':
			format = ATTRIB_VERTEX;
			break;
		case 'c':
			format = ATTRIB_COLOR;
			break;
		case 't':
			format = ATTRIB_TEXCOORD0;
			break;
		case 'T':
			format = ATTRIB_TEXCOORD1;
			break;
		case 'n':
			format = ATTRIB_NORMAL;
			break;
		case 'x':
			format = ATTRIB_TANGENT;
			break;
		case 'w':
			format = ATTRIB_BONEWEIGHT;
			break;
		case 'i':
			format = ATTRIB_BONEINDEX;
			break;
		default:
			return;
		}
...
void begin(GLenum mode, int numverts) {
	...
	if (glversion >= 300 && !intel_mapbufferrange_bug) {
		...
		if (vbooffset + len >= MAXVBOSIZE) {
			...
			glBindBuffer_(GL_ARRAY_BUFFER, vbo);
		void* buf = glMapBufferRange_(GL_ARRAY_BUFFER,
									  vbooffset,
									  len,
	...
	if (glversion >= 300) {
		if (buf == attribdata) {
			if (vbooffset + attribbuf.length() >= MAXVBOSIZE) {
				...
				glBindBuffer_(GL_ARRAY_BUFFER, vbo);
			void* dst = intel_mapbufferrange_bug
				? NULL
				: glMapBufferRange_(GL_ARRAY_BUFFER,
									vbooffset,
									attribbuf.length(),
									GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
			if (dst) {
