...
#define GLE_DEFATTRIB(name, type, defaultsize, defaultformat)                          \
	static inline void def##name(int size = defaultsize, int format = defaultformat) { \
		defattrib(type, size, format);                                                 \
	}
...
#define GLE_INITATTRIB(name, index, suffix, type)                     \
	static inline void name##suffix(type x) {                         \
		glVertexAttrib1##suffix##_(index, x);                         \
	}                                                                 \
	static inline void name##suffix(type x, type y) {                 \
		glVertexAttrib2##suffix##_(index, x, y);                      \
	}                                                                 \
	static inline void name##suffix(type x, type y, type z) {         \
		glVertexAttrib3##suffix##_(index, x, y, z);                   \
	}                                                                 \
	static inline void name##suffix(type x, type y, type z, type w) { \
		glVertexAttrib4##suffix##_(index, x, y, z, w);                \
	}
#define GLE_INITATTRIBF(name, index)                 \
	GLE_INITATTRIB(name, index, f, float)            \
	static inline void name(const vec& v) {          \
		glVertexAttrib3fv_(index, v.v);              \
	}                                                \
	static inline void name(const vec& v, float w) { \
		glVertexAttrib4f_(index, v.x, v.y, v.z, w);  \
	}                                                \
	static inline void name(const vec2& v) {         \
		glVertexAttrib2fv_(index, v.v);              \
	}                                                \
	static inline void name(const vec4& v) {         \
		glVertexAttrib4fv_(index, v.v);              \
	}
#define GLE_INITATTRIBN(name, index, suffix, type, defaultw)                     \
	static inline void name##suffix(type x, type y, type z, type w = defaultw) { \
		glVertexAttrib4N##suffix##_(index, x, y, z, w);                          \
	}

GLE_INITATTRIBF(vertex, ATTRIB_VERTEX)
GLE_INITATTRIBF(color, ATTRIB_COLOR)
GLE_INITATTRIBN(color, ATTRIB_COLOR, ub, uchar, 255)
static inline void color(const bvec& v, uchar alpha = 255) {
	glVertexAttrib4Nub_(ATTRIB_COLOR, v.x, v.y, v.z, alpha);
}
...
static inline void normal(float x, float y, float z) {
	glVertexAttrib4f_(ATTRIB_NORMAL, x, y, z, 0.0f);
}
static inline void normal(const vec& v) {
	glVertexAttrib4f_(ATTRIB_NORMAL, v.x, v.y, v.z, 0.0f);
}
static inline void tangent(float x, float y, float z, float w = 1.0f) {
	glVertexAttrib4f_(ATTRIB_TANGENT, x, y, z, w);
}
static inline void tangent(const vec& v, float w = 1.0f) {
	glVertexAttrib4f_(ATTRIB_TANGENT, v.x, v.y, v.z, w);
}
static inline void tangent(const vec4& v) {
	glVertexAttrib4fv_(ATTRIB_TANGENT, v.v);
}

#define GLE_ATTRIBPOINTER(name, index, defaultnormalized, defaultsize, defaulttype, prepare) \
	static inline void enable##name() {                                                      \
		prepare;                                                                             \
		glEnableVertexAttribArray_(index);                                                   \
	}                                                                                        \
	static inline void disable##name() {                                                     \
		glDisableVertexAttribArray_(index);                                                  \
	}                                                                                        \
	static inline void name##pointer(int stride,                                             \
									 const void* data,                                       \
									 GLenum type = defaulttype,                              \
									 int size = defaultsize,                                 \
									 GLenum normalized = defaultnormalized) {                \
		prepare;                                                                             \
		glVertexAttribPointer_(index, size, type, normalized, stride, data);                 \
	}

static inline void enableattrib(int index) {
	disable();
	glEnableVertexAttribArray_(index);
}
static inline void disableattrib(int index) {
	glDisableVertexAttribArray_(index);
}
...
static inline void bindebo(GLuint ebo) {
	disable();
	glBindBuffer_(GL_ELEMENT_ARRAY_BUFFER, ebo);
}
static inline void clearebo() {
...
#define GLE_ATTRIB(suffix, type)                                        \
	static inline void attrib##suffix(type x) {                         \
		attrib<type>(x);                                                \
	}                                                                   \
	static inline void attrib##suffix(type x, type y) {                 \
		attrib<type>(x, y);                                             \
	}                                                                   \
	static inline void attrib##suffix(type x, type y, type z) {         \
		attrib<type>(x, y, z);                                          \
	}                                                                   \
	static inline void attrib##suffix(type x, type y, type z, type w) { \
		attrib<type>(x, y, z, w);                                       \
	}
...
static inline void attrib(const vec& v, float w) {
	attribf(v.x, v.y, v.z, w);
}
static inline void attrib(const vec2& v) {
	attribf(v.x, v.y);
}
static inline void attrib(const vec4& v) {
	attribf(v.x, v.y, v.z, v.w);
}
static inline void attrib(const ivec& v) {
	attribi(v.x, v.y, v.z);
}
static inline void attrib(const ivec& v, int w) {
	attribi(v.x, v.y, v.z, w);
}
static inline void attrib(const ivec2& v) {
	attribi(v.x, v.y);
}
static inline void attrib(const ivec4& v) {
	attribi(v.x, v.y, v.z, v.w);
}
static inline void attrib(const bvec& b) {
	attribub(b.x, b.y, b.z);
}
static inline void attrib(const bvec& b, uchar w) {
	attribub(b.x, b.y, b.z, w);
}
static inline void attrib(const bvec4& b) {
	attribub(b.x, b.y, b.z, b.w);
}
